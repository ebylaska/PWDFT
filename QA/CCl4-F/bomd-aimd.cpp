...
extern int  lammps_pspw_aimd_minimizer(MPI_Comm, double*, double*, double*, std::ostream&);
...
int main(int argc, char* argv[])
{
   ...
   for (auto ii=0; ii<nion; ++ii)
   {
      ...
      symbol[ii] = ss[0];
      mass[ii]   = symboltomass(symbol[ii])*amu_to_mass;
   ...
   if (taskid==MASTER)
   {
      ...
      for (auto ii=0; ii<nion; ++ii)
            std::cout << "@ ii=" << ii << " " << symbol[ii] << "\trion: " << Ffmt(20,15) << rion1[3*ii] << " " << Ffmt(20,15) << rion1[3*ii+1] << " " << Ffmt(20,15) << rion1[3*ii+2]
   ...
   ierr += lammps_pspw_aimd_minimizer(MPI_COMM_WORLD,rion1,fion,&Eqm);
   ...
   if (taskid==MASTER) {
      ...
      for (auto ii=0; ii<nion; ++ii)
            std::cout << "@ ii=" << ii << " " << symbol[ii] << "\tfion: " << Ffmt(20,15) << fion[3*ii] << " " << Ffmt(20,15) << fion[3*ii+1] << " " << Ffmt(20,15) << fion[3*ii+2]
                      << " mass = "  << mass[ii] << std::endl;
      std::cout << "@" << std::endl;
   }
   ...
   for (auto it=0; it<200; ++it)
   {
      ...
      KE *= 0.5;
      if (taskid==MASTER) {
         std::cout << "@ it = " << Ifmt(5) << it+1 << " Energy+Kinetic = " << Efmt(20,15) << Eqm+KE << " Energy = " << Efmt(20,15) << Eqm << " Kinetic Energy = " << Efmt(20,15) << KE << std::endl;
