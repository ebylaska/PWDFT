...
extern int  lammps_pspw_qmmm_minimizer(MPI_Comm, double*, double*, double*, double*, double*, bool, bool, std::ostream&);
extern void lammps_pspw_input(MPI_Comm, std::string&, std::ostream&);
                                  ...
                                  const double qion[], const double rion[])
{
   ...
   for (auto qm=0; qm<nion_qm; ++qm)
      for (auto mm=nion_qm; mm<nion; ++mm)
         E += Q_Electrostatic_self(&rion[3*qm],qion[qm], 
                                   &rion[3*mm],qion[mm]);
   // mm interactions
   for (auto mm1=nion_qm; mm1<(nion-1); ++mm1)
      for (auto mm2=mm1+1; mm2<nion; ++mm2)
      {
         E += Q_Electrostatic_self(&rion[3*mm1],qion[mm1], 
                                   &rion[3*mm2],qion[mm2]);
      }
                              ...
                              const double qion[], const double rion[], double fion[])
{
   ...
   for (auto qm=0; qm<nion_qm; ++qm)
      for (auto mm=nion_qm; mm<nion; ++mm)
         Q_Electrostatic_Force_self(&rion[3*qm],qion[qm],&fion[3*qm],
                                    &rion[3*mm],qion[mm],&fion[3*mm]);

   // mm interactions
   for (auto mm1=nion_qm; mm1<(nion-1); ++mm1)
      for (auto mm2=mm1+1; mm2<nion; ++mm2)
         Q_Electrostatic_Force_self(&rion[3*mm1],qion[mm1],&fion[3*mm1],
                                    &rion[3*mm2],qion[mm2],&fion[3*mm2]);
}
...
int main(int argc, char* argv[])
{
   ...
   ierr += lammps_pspw_qmmm_minimizer(MPI_COMM_WORLD,rion1,uion,fion,qion,&Eqm,true,true,std::cout);

   Eqq = QMMM_electrostatic_energy(nion_qm,nion,qion,rion1);
