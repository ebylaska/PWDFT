...
int main(int argc, char* argv[])
{
   ...
   if (taskid==MASTER) {
      ...
      xyzfile->open("testnaqmm.xyz", std::ios::app);

      emotionfile = new (std::nothrow) std::ofstream;
      emotionfile->open("testnaqmm.emotion", std::ios::app);
   }
   ...
   double uion[nion],qion[nion],fdfion[3*nion],fion[3*nion],fion1[3*nion],fion0[3*nion];
   double fionq1[3*nion],fionq0[3*nion];
   double epsilon[nion],sigma[nion],mass[nion],dti[nion];
   double E,KE,Eqm,Ecoul,ELJ;

   std::string symbol[nion] = {"O","H","H","Na"};
   ...
   rion1[3] =  0.803054*ANGTOBOHR; rion1[4] =  0.369132*ANGTOBOHR; rion1[5] = -0.511660*ANGTOBOHR;
   rion1[6] = -0.325571*ANGTOBOHR; rion1[7] = -0.669574*ANGTOBOHR; rion1[8] = -0.488560*ANGTOBOHR;
   ...
   sigma[1] = 0.700000*ANGTOBOHR; epsilon[1] = 0.0;
   sigma[2] = 0.700000*ANGTOBOHR; epsilon[2] = 0.0;
   mass[0] = 16.0*1822.89; mass[1]= 2.0*1822.89; mass[2]=2.0*1822.89;

   // Na ion - MM 
   rion1[9] =  0.021259*ANGTOBOHR; rion1[10] =  0.506771*ANGTOBOHR; rion1[11] =  2.831278*ANGTOBOHR;
   qion[3]  = 1.0;
   uion[3]  = 0.0;
   ...
   get_QM_force(MPI_COMM_WORLD,nion_qm,nion,rion1,qion,fdfion);
   ...
   for (auto i=0; i<3*nion; ++i) fdfion[i] += fionq1[i];
   ...
   for (auto i=0; i<3*nion; ++i) fdfion[i] += fion1[i];
   ...
   for (auto ii=0; ii<nion; ++ii)
   {
      rion2[3*ii]   = rion1[3*ii]   + dt*rion0[3*ii]   + 0.5*dti[ii]*fdfion[3*ii];
      rion2[3*ii+1] = rion1[3*ii+1] + dt*rion0[3*ii+1] + 0.5*dti[ii]*fdfion[3*ii+1];
      rion2[3*ii+2] = rion1[3*ii+2] + dt*rion0[3*ii+2] + 0.5*dti[ii]*fdfion[3*ii+2];
   }
   ...
   if (taskid==MASTER)
   {
      ...
      std::cout << std::endl;
      for (auto ii=0; ii<nion; ++ii)
         std::cout << "ii=" << ii << " fdfion: "  << fdfion[3*ii]   << " " << fdfion[3*ii+1]   << " " << fdfion[3*ii+2]   << " qion=" << qion[ii]   << std::endl;
      std::cout << std::endl;
   ...
   for(auto it=0; it<nsteps; ++it)
   {
     ...
     //Ecoul += eqmm0;
      ...
      for (auto ii=0; ii<nion; ++ii)
      {
         rion2[3*ii]   = 2.0*rion1[3*ii]   - rion0[3*ii]   + dti[ii]*fdfion[3*ii];
         rion2[3*ii+1] = 2.0*rion1[3*ii+1] - rion0[3*ii+1] + dti[ii]*fdfion[3*ii+1];
         rion2[3*ii+2] = 2.0*rion1[3*ii+2] - rion0[3*ii+2] + dti[ii]*fdfion[3*ii+2];
      }
