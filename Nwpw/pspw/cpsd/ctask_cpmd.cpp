...

{
   ...
   if (oprint)
   {
      ...
      coutput << "          *****************************************************\n";
      coutput << "          *                                                   *\n";
      coutput << "          *     Car-Parrinello calculation for molecules,     *\n";
      coutput << "          *       microclusters, liquids, and materials       *\n";
      coutput << "          *                                                   *\n";
      coutput << "          *     [     extended Lagrangian molecular   ]       *\n";
      coutput << "          *     [        dynamics simulation          ]       *\n";
      coutput << "          *     [          C++ implementation         ]       *\n";
      coutput << "          *                                                   *\n";
      coutput << "          *            version #7.00   03/20/20               *\n";
      coutput << "          *                                                   *\n";
   ...
   mynose = new nwpw_Nose_Hoover(*myion,(mygrid->ne[0]+mygrid->ne[1]),w,*control);
   ...
   if (oprint)
   {
      ...
      for (ii=0; ii<myion->nion; ++ii)
         coutput << Ifmt(4) << ii+1 << " " << myion->symbol(ii)
                   ...
                   << " )"   << std::endl;
      coutput << "   G.C.\t( " << Ffmt(10,5) << myion->vgc(0) << " " << Ffmt(10,5) << myion->vgc(1) << " " << Ffmt(10,5) << myion->vgc(2) << " )" << std::endl;
      coutput << " C.O.M.\t( " << Ffmt(10,5) << myion->vcom(0) << " " << Ffmt(10,5) << myion->vcom(1) << " " << Ffmt(10,5) << myion->vcom(2) << " )" << std::endl;
      coutput << " number of constraints = " << Ifmt(5) << 0 << " ( DOF = " << Ifmt(6) << myion->ndof() << " )" << std::endl;
      coutput << std::endl;
      ...
      if (myion->fix_translation) coutput << "      translation constrained" << std::endl;
      if (myion->fix_rotation)    coutput << "      rotation constrained" << std::endl;
      coutput << "      time step =" << Ffmt(11,2) << control->time_step() << " ficticious mass =" << Ffmt(11,2) << control->fake_mass() << std::endl;
      ...
      coutput << "      cooling/heating rates  =" << Efmt(12,5) << control->elc_scaling() << " (psi) " << Efmt(12,5) << control->ion_scaling() << " (ion)" << std::endl;
      coutput << "      initial kinetic energy =" << Efmt(12,5) << eke0 << " (psi) " << Efmt(12,5) << myion->eki0 << " (ion)" << std::endl;
      coutput << "                                                 " << Efmt(12,5) << myion->ekg << " (C.O.M.)" << std::endl;
      coutput << "      after scaling          =" << Efmt(12,5) << eke1 << " (psi) " << Efmt(12,5) << myion->eki1 << " (ion)" << std::endl;
      coutput << "      increased energy       =" << Efmt(12,5) << eke1-eke0 << " (psi) " << Efmt(12,5) << myion->eki1-myion->eki0 << " (ion)" << std::endl;
      coutput << std::endl;
      ...
      if (SA) coutput << "      SA decay rate = " << Efmt(10,3) << sa_decay[0] << "  (elc) " << Efmt(10,3) << sa_decay[1]  << " (ion)" << std::endl;
...
//                 |**************************|
   ...
   if (oprint)
   {
      ...
      coutput << "     iter.          KE+Energy             Energy        KE_psi        KE_Ion   Temperature\n";
      coutput << "     -------------------------------------------------------------------------------------\n";
   }
...
int ctask_cpmd_stop(MPI_Comm comm_world0, std::ostream& coutput)
{
   ...
   psi_write(mygrid,&version,nfft,unita,&ispin,ne,psi2,control->output_movecs_filename(),coutput);
   psi_write(mygrid,&version,nfft,unita,&ispin,ne,psi0,control->output_v_movecs_filename(),coutput);
