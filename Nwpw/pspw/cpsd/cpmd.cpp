...
int cpmd(MPI_Comm comm_world0, std::string &rtdbstring) {
  ...
  if (oprint) {
    ...
    std::cout << " processor grid           : " << myparallel.np_i() << " x"
              << myparallel.np_j() << "\n";
    if (mygrid.maptype == 1)
      std::cout << " parallel mapping         : 1d-slab"
                << "\n";
    if (mygrid.maptype == 2)
      std::cout << " parallel mapping         : 2d-hilbert"
                << "\n";
    if (mygrid.maptype == 3)
      std::cout << " parallel mapping         : 2d-hcurve"
                << "\n";
    ...
    if (control.tile_factor() > 1)
      std::cout << " GPU tile factor          : " << control.tile_factor()
                << std::endl;
    ...
    for (ii = 0; ii < myion.nion; ++ii)
      std::cout << Ifmt(4) << ii + 1 << " " << myion.symbol(ii) << "\t( "
                ...
                << " )" << std::endl;
    std::cout << "   G.C.\t( " << Ffmt(10, 5) << myion.vgc(0) << " "
              << Ffmt(10, 5) << myion.vgc(1) << " " << Ffmt(10, 5)
              << myion.vgc(2) << " )" << std::endl;
    std::cout << " C.O.M.\t( " << Ffmt(10, 5) << myion.vcom(0) << " "
    ...
    std::cout << "                       Mandelung Wigner-Seitz ="
              << Ffmt(12, 8) << myewald.mandelung() << " (alpha=" << Ffmt(12, 8)
    ...
    std::cout << "      max iterations = " << Ifmt(10)
              << control.loop(0) * control.loop(1) << " (" << Ifmt(5)
              << control.loop(0) << " inner " << Ifmt(5) << control.loop(1)
              << " outer)" << std::endl;
    std::cout << std::endl;
    std::cout << " velocity scaling: " << std::endl;
    std::cout << "      cooling/heating rates  =" << Efmt(12, 5)
              << control.elc_scaling() << " (psi) " << Efmt(12, 5)
              << control.ion_scaling() << " (ion)" << std::endl;
    std::cout << "      initial kinetic energy =" << Efmt(12, 5) << eke0
              << " (psi) " << Efmt(12, 5) << myion.eki0 << " (ion)"
              << std::endl;
    std::cout << "                                                 "
              << Efmt(12, 5) << myion.ekg << " (C.O.M.)" << std::endl;
    std::cout << "      after scaling          =" << Efmt(12, 5) << eke1
              << " (psi) " << Efmt(12, 5) << myion.eki1 << " (ion)"
              << std::endl;
    std::cout << "      increased energy       =" << Efmt(12, 5) << eke1 - eke0
    ...
    if (SA)
      std::cout << "      SA decay rate = " << Efmt(10, 3) << sa_decay[0]
  ...
  if (oprint) {
    ...
    std::cout << "     iter.          KE+Energy             Energy        "
                 "KE_psi        KE_Ion   Temperature\n";
    std::cout << "     "
                 "-------------------------------------------------------------"
                 "------------------------\n";
  }
  ...
  if (control.loop(1) > 0) {
    ...
    while (!done) {
      ...
      if (oprint) {
        if (SA)
          std::cout << Ifmt(10) << icount * control.loop(0) << Efmt(19, 10)
                    << E[0] << Efmt(19, 10) << E[1] << Efmt(14, 5) << E[2]
                    << Efmt(14, 5) << E[3] << Ffmt(9, 1) << Te_new << Ffmt(9, 1)
                    << Tr_new << std::endl;
        ...
        //                            E[0],E[1],E[2],E[3],Te_new,Tr_new);
          ...
          std::cout << Ifmt(10) << icount * control.loop(0) << Efmt(19, 10)
                    << E[0] << Efmt(19, 10) << E[1] << Efmt(14, 5) << E[2]
                    << Efmt(14, 5) << E[3] << Ffmt(14, 2) << myion.Temperature()
                    << std::endl;
      ...
      if (control.out_of_time()) {
        ...
        if (oprint)
          std::cout << "         *** out of time. iteration terminated."
                    << std::endl;
      }
  ...
  if (oprint) {
    ...
    std::cout << " total orbital energy    : " << Efmt(19, 10) << E[4] << " ("
              << Efmt(15, 5) << E[4] / (mygrid.ne[0] + mygrid.ne[1])
              << " /electron)" << std::endl;
    ...
    std::cout << "                                            " << Efmt(12, 5)
              << myion.ekg << " (C.O.M.)" << std::endl;
    eke /= ((double)control.loop(1));
    eke *= 2.0 / kb / ((double)(mygrid.ne[0] + mygrid.ne[ispin - 1])) /
           ((double)mygrid.npack_all(1));
    std::cout << " Temperature         :   " << Ffmt(10, 1) << eke << " K (elc)"
              << std::endl;
    std::cout << " Temperature         :   " << Ffmt(10, 1)
              << myion.Temperature() << " K (ion)" << std::endl;
    std::cout << "                     :   " << Ffmt(10, 1)
              << myion.com_Temperature() << " K (C.O.M.)" << std::endl
              << std::endl;
    ...
    // E[5],E[5]/(mygrid.ne[0]+mygrid.ne[1])); printf(" K.S. V_l energy     :
    // %19.10le (%15.5le /electron)\n", E[6],E[6]/(mygrid.ne[0]+mygrid.ne[1]));
    // printf(" K.S. V_nl energy    : %19.10le (%15.5le /electron)\n",
    // E[7],E[7]/(mygrid.ne[0]+mygrid.ne[1])); printf(" K.S. V_Hart energy  :
    // %19.10le (%15.5le /electron)\n", E[8],E[8]/(mygrid.ne[0]+mygrid.ne[1]));
    // printf(" K.S. V_xc energy    : %19.10le (%15.5le /electron)\n",
    // E[9],E[9]/(mygrid.ne[0]+mygrid.ne[1])); if (mypsp.myapc->v_apc_on)
  ...
  psi_write(&mygrid, &version, nfft, unita, &ispin, ne, psi0,
            control.output_v_movecs_filename(), std::cout);
