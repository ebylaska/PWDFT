...
int cpsd(MPI_Comm comm_world0, std::string &rtdbstring) {
  ...
  */
  ...
  if (oprint) {
    ...
    std::cout << "      fixed step: time step =" << Ffmt(12, 2)
  ...
  if (oprint) {
    ...
    std::cout << "          >>> iteration started at " << util_date() << "  <<<"
              << std::endl;
    std::cout << "     iter.             Energy       DeltaE     DeltaPsi     "
                 "DeltaIon"
              << std::endl;
    std::cout
        << "     "
  ...
  while (!done) {
    ...
    if (oprint)
      std::cout << Ifmt(10) << icount * control.loop(0) << Efmt(19, 10) << E[0]
                << Efmt(13, 5) << deltae << Efmt(13, 5) << deltac << Efmt(13, 5)
                << deltar << std::endl;
    ...
    if ((deltae > 0.0) && (icount > 1)) {
      ...
      if (oprint)
        std::cout << "          *** arrived at the Maximum iteration.   "
                     "terminated ***\n";
    }
  }
  if (myparallel.is_master())
