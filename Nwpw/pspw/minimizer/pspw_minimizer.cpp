...
int pspw_minimizer(MPI_Comm comm_world0, std::string &rtdbstring,
  ...
  if (flag == 2) {
    ...
    if (lprint) {
      ...
      for (ii = 0; ii < myion.nion; ++ii)
        coutput << Ifmt(4) << ii + 1 << " " << myion.symbol(ii) << "\t( "
                << Ffmt(10, 5) << fion[3 * ii] << " " << Ffmt(10, 5)
                << fion[3 * ii + 1] << " " << Ffmt(10, 5) << fion[3 * ii + 2]
                << " )\n";
      coutput << std::endl << std::endl;
    }
    rtdbjson["pspw"]["fion"] = std::vector<double>(fion, &fion[3 * myion.nion]);
    for (ii = 0; ii < (3 * myion.nion); ++ii)
      fion[ii] *= -1.0;
    rtdbjson["pspw"]["gradient"] =
        std::vector<double>(fion, &fion[3 * myion.nion]);
