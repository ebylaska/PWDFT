...
int pspw_geovib(MPI_Comm comm_world0, std::string &rtdbstring,
  ...
  Grms = 0.0;
  for (ii = 0; ii < (myion.nion); ++ii) {
    gg = fion[3 * ii] * fion[3 * ii] + fion[3 * ii + 1] * fion[3 * ii + 1] +
  ...
  if ((Gmax <= tol_Gmax) && (Grms <= tol_Grms) && (Xrms <= tol_Xrms) &&
      (Xmax <= tol_Xmax))
    done = true;
  ...
  if (oprint) {
    ...
    if ((Gmax <= tol_Gmax) && (Grms <= tol_Grms) && (Xrms <= tol_Xrms) &&
      ...
      coutput << "                                            ok       ok      "
                 " ok       ok\n";
    } else if ((Gmax <= tol_Gmax) || (Grms <= tol_Grms) || (Xrms <= tol_Xrms) ||
               (Xmax <= tol_Xmax)) {
      std::string oktag("");
      coutput << "                                     ";
      oktag = "  ";
  ...
  while ((!done) && (it <= maxit)) {
    ...
    if (oprint) {
      ...
      for (auto ii = 0; ii < mymolecule.myion->nion; ++ii)
        coutput << " " << Ifmt(5) << ii + 1 << " "
                << mymolecule.myion->symbol(ii) << "  ( " << Ffmt(12, 6)
                << fion[3 * ii] << " " << Ffmt(12, 6) << fion[3 * ii + 1] << " "
    ...
    if (oprint) {
      ...
      if ((Gmax <= tol_Gmax) && (Grms <= tol_Grms) && (Xrms <= tol_Xrms) &&
        ...
        coutput << "                                            ok       ok    "
                   "   ok       ok\n";
      } else if ((Gmax <= tol_Gmax) || (Grms <= tol_Grms) ||
                 (Xrms <= tol_Xrms) || (Xmax <= tol_Xmax)) {
        std::string oktag = "";
        coutput << "                                     ";
        oktag = "  ";
        if (Gmax <= tol_Gmax)
