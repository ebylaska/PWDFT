 ...
 *                                        *
 ******************************************/
double cgsd_energy(Control2 &control, Molecule &mymolecule, bool doprint,
                   std::ostream &coutput) {
  Parallel *parall = mymolecule.mygrid->d3db::parall;
  ...
  bool oprint =
      (parall->is_master() && control.print_level("medium") && doprint);
  bool lprint = (parall->is_master() && control.print_level("low") && doprint);

  for (auto ii = 0; ii < 60; ++ii)
    E[ii] = 0.0;

  if (oprint) {
    if (minimizer == 1)
      coutput << "     ============ Grassmann conjugate gradient iteration "
                 "============"
              << std::endl;
    if (minimizer == 2)
      coutput << "     ================== Grassmann lmbfgs iteration "
                 "=================="
              << std::endl;
    if (minimizer == 4)
      coutput << "     ============= Stiefel conjugate gradient iteration "
                 "============="
              << std::endl;
    if (minimizer == 5)
      coutput << "     ============= Kohn-Sham scf iteration (potential) "
                 "=============="
              << std::endl;
    if (minimizer == 7)
      coutput << "     =================== Stiefel lmbfgs iteration "
                 "==================="
              << std::endl;
    if (minimizer == 8)
      coutput << "     ============== Kohn-Sham scf iteration (density) "
                 "==============="
              << std::endl;

    coutput << "          >>> iteration started at " << util_date()
            << "  <<<\n";
    coutput << "     iter.                 Energy          DeltaE        "
               "DeltaRho\n";
    coutput
        << "     "
           "----------------------------------------------------------------\n";
  ...
  if (minimizer == 1) {
    ...
    while ((icount < it_out) && (!converged)) {
      ...
      deltae_old = deltae;
      total_energy =
          cgsd_cgminimize(mymolecule, mygeodesic12.mygeodesic1, E, &deltae,
                          &deltac, bfgscount, it_in, tole, tolc);
      ++bfgscount;
      if (oprint)
        coutput << Ifmt(10) << icount * it_in << Efmt(25, 12) << total_energy
                << Efmt(16, 6) << deltae << Efmt(16, 6) << deltac << std::endl;
    ...
    while ((icount < it_out) && (!converged)) {
      ...
      deltae_old = deltae;
      total_energy =
          cgsd_bfgsminimize(mymolecule, mygeodesic12.mygeodesic1, psi_lmbfgs, E,
    ...
    while ((icount < it_out) && (!converged)) {
      ...
      deltae_old = deltae;
      total_energy =
          cgsd_bfgsminimize2(mymolecule, mygeodesic12.mygeodesic2, psi_lmbfgs2,
  ...
  if (oprint) {
    if (converged)
      coutput << "     *** tolerance ok. iteration terminated" << std::endl;
    if (!converged)
      coutput << "     *** arrived at the Maximum iteration.  terminated"
              << std::endl;
