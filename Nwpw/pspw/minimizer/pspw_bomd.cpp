...
int pspw_bomd(MPI_Comm comm_world0, std::string &rtdbstring,
  ...
  if (oprint) {
    ...
    coutput << "     iter.          KE+Energy             Energy        KE_Ion "
               "  Temperature\n";
    coutput << "     "
               "---------------------------------------------------------------"
               "--------\n";
  ...
  if (control.bo_steps(1) > 0) {
    ...
    while (!done) {
      ...
      if (oprint) {
        if (SA) {
          if (mynose.on())
            coutput << Ifmt(10) << (icount * it_in) << Efmt(19, 10)
                    << EV + eki + mynose.r_energy() << Efmt(19, 10) << EV
                    << Efmt(14, 5) << eki << Ffmt(9, 1) << Tr_new << std::endl;
          else
            coutput << Ifmt(10) << (icount * it_in) << Efmt(19, 10) << EV + eki
                    << Efmt(19, 10) << EV << Efmt(14, 5) << eki << Ffmt(9, 1)
                    << Tr_new << std::endl;
        } else {
          if (mynose.on())
            coutput << Ifmt(10) << (icount * it_in) << Efmt(19, 10)
                    << EV + eki + mynose.r_energy() << Efmt(19, 10) << EV
                    << Efmt(14, 5) << eki << Ffmt(14, 2) << myion.Temperature()
                    << std::endl;
          else
            coutput << Ifmt(10) << (icount * it_in) << Efmt(19, 10) << EV + eki
                    << Efmt(19, 10) << EV << Efmt(14, 5) << eki << Ffmt(14, 2)
                    << myion.Temperature() << std::endl;
        }
      }
