...
//                 |**************************|
   ...
   if (oprint)
   {
      ...
      if (myion.fix_translation) coutput << "      translation constrained\n";
      if (myion.fix_rotation)    coutput << "      rotation constrained\n";
      ...
      if (control.bo_algorithm()==0) coutput << "      integration algorithm = position Verlet\n";
      if (control.bo_algorithm()==1) coutput << "      integration algorithm = velocity Verlet\n";
      if (control.bo_algorithm()==2) coutput << "      integration algorithm = leap frog\n";


      coutput << std::endl;
      coutput << " scaling parameters:   " << std::endl;
      coutput << "      cooling/heating rate   =" << Efmt(12,5) << control.ion_scaling() << " (ion)"    << std::endl;
      coutput << "      initial kinetic energy =" << Efmt(12,5) << myion.eki0            << " (ion)"    << std::endl;
      coutput << "                           "    << Efmt(15,5) << myion.ekg             << " (C.O.M.)" << std::endl;
      coutput << "      after scaling          =" << Efmt(12,5) << myion.eki1            << " (ion)"    << std::endl;
      coutput << "      increased energy       =" << Efmt(12,5) << myion.eki1-myion.eki0 << " (ion)"    << std::endl;
      coutput << std::endl;
      ...
      if (SA) coutput << "      SA decay rate = " << Ffmt(10,3) << sa_decay[1] << "(ion)\n";
   ...
   if (control.bo_steps(1)>0)
   {
      ...
      while (!done)
      {
         ...
         {   
            ...
            if (oprint) coutput << "         *** out of time. iteration terminated." << std::endl;
         }
         // Check for Completion
         else if (icount>=it_out)
         {
            done = true;
            if (oprint) coutput << "         *** arrived at the Maximum iteration.   terminated." << std::endl;
         }
      } // end outer loop 
