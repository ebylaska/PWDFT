 ...
 *                                          *
 ********************************************/
/*static void generate_rho_ion(Pneb *mypneb, Strfac *mystrfac, double rc, double
*rho_ion)
{
 ...
 *                                          *
 ********************************************/
Dielectric_Operator::Dielectric_Operator(Pneb *mygrid, Strfac *mystrfacin,
                                         Control2 &control) {
  mypneb = mygrid;
  mystrfac = mystrfacin;
...
void Dielectric_Operator::generate_dpotential(const double *w, double *dwdeps) {
  ...
  mypneb->cr_fft3d(w_z);
  for (auto i = 0; i < n2ft3d; ++i)
    dwdeps[i] = (w_x[i] * epsilon_x[i] + w_y[i] * epsilon_y[i] +
                 w_z[i] * epsilon_z[i]) /
                (fourpi * epsilon[i]);
  ...
  std::cout << "esum_xyz =" << esumx << " " << esumy << " " << esumz
            << std::endl;
  ...
  std::cout << "aesum_xyz =" << asumx << " " << asumy << " " << asumz
            << std::endl;
}
