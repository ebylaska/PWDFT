...
Coulomb2_Operator::Coulomb2_Operator(Pneb *mygrid, Control2& control)
{
   ...
   double volume = dunita[0]*dunitg[0] + dunita[1]*dunitg[1] + dunita[2]*dunitg[2];
   for (auto i=0; i<9; ++i)
   ...
   if (mypneb->PGrid::lattice->fast_erf())
   {
      ...
      for (auto i=0; i<dnx; ++i)
      {
         ...
         if (p==taskid)
         {
            ...
            auto y = i1*dunita[1]/fdnx + j1*dunita[4]/fdny + k1*dunita[7]/fdnz;
            auto z = i1*dunita[2]/fdnx + j1*dunita[5]/fdny + k1*dunita[8]/fdnz;
...
erf(x) \approx 1-{\frac {1}{\left(1+a_{1}x+a_{2}x^{2}+a_{3}x^{3}+a_{4}x^{4}\right)^{4}}}, for x>=0
(maximum error: 5×10−4)
where a1 = 0.278393, a2 = 0.230389, a3 = 0.000972, a4 = 0.078108

erf(x) \approx 1-\left(a_{1}t+a_{2}t^{2}+a_{3}t^{3}\right)e^{-x^{2}},\quad t={\frac {1}{1+px}}, for x>=0.
(maximum error: 2.5×10−5)
where p = 0.47047, a1 = 0.3480242, a2 = −0.0958798, a3 = 0.7478556

Approximation used by Ryoichi:
erf(x) \approx 1-{\frac {1}{\left(1+a_{1}x+a_{2}x^{2}+\cdots +a_{6}x^{6}\right)^{16}}}, for x>= 0
...
erf(x) \approx 1-\left(a_{1}t+a_{2}t^{2}+\cdots +a_{5}t^{5}\right)e^{-x^{2}},\quad t={\frac {1}{1+px}}}
