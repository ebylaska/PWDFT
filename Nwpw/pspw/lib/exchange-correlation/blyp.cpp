 ...
 *                                                *
 **************************************************/
void gen_BLYP_BW_unrestricted(const int n2ft3d, double *dn_in, double *agr_in,
                              const double x_parameter,
                              const double c_parameter, double *xce, double *fn,
                              double *fdn) {
  /* local variables */
  ...
  for (int j = 0; j < n2ft3d; ++j) {
    ...
    if ((dn_in[j] + dn_in[j + n2ft3d]) < DNS_CUT) {
      ...
      if (dn_in[j] < DNS_CUT) {
        ...
        fdnxup =
            -nup_thrd * (4.00 / 3.00) * (lda_c + beta * (F1up - chiup2 * F2up));
        fdagrxup = -beta * chiup * F2up;

        if ((fdnxup - xeup) > 0.00) {
          gen_PBE96_x_unrestricted(nup, agrup, &xeup_pbe, &fdnxup_pbe,
                                   &fdagrxup_pbe);
          Becke_smalln_correction(nup, nup_thrd, 1.00, beta, lda_c, chiup,
      ...
      if (dn_in[j + n2ft3d] < DNS_CUT) {
        ...
        fdnxdn =
            -ndn_thrd * (4.00 / 3.00) * (lda_c + beta * (F1dn - chidn2 * F2dn));
        fdagrxdn = -beta * chidn * F2dn;
        if ((fdnxdn - xedn) > 0.00) {
          gen_PBE96_x_unrestricted(ndn, agrdn, &xedn_pbe, &fdnxdn_pbe,
                                   &fdagrxdn_pbe);
          Becke_smalln_correction(ndn, ndn_thrd, 1.00, beta, lda_c, chidn,
    ...
    if ((dn_in[j] + dn_in[j + n2ft3d]) < DNS_CUT) {
      ...
      F_uu = gamma_uu * n13d_m +
             d3 * (gamma_u + gamma_u) * n_mfrthrd * n13d2_m -
             (4.00 / 9.00) * d * gamma * n_mfrthrd * n_m * n13d2_m +
             (2.00 / 9.00) * d * d * gamma * n_mfrthrd * n_mfrthrd * n13d3_m;

      F_dd = gamma_dd * n13d_m +
             d3 * (gamma_d + gamma_d) * n_mfrthrd * n13d2_m -
             (4.00 / 9.00) * d * gamma * n_mfrthrd * n_m * n13d2_m +
             (2.00 / 9.00) * d * d * gamma * n_mfrthrd * n_mfrthrd * n13d3_m;

      F_du = gamma_du * n13d_m +
             d3 * (gamma_u + gamma_d) * n_mfrthrd * n13d2_m -
             (4.00 / 9.00) * d * gamma * n_mfrthrd * n_m * n13d2_m +
             (2.00 / 9.00) * d * d * gamma * n_mfrthrd * n_mfrthrd * n13d3_m;
...
void gen_BLYP_BW_restricted(const int n2ft3d, double *rho_in, double *agr_in,
  ...
  for (int i = 0; i < n2ft3d; ++i) {
    ...
    /* calc. becke exchange energy density, fnx, fdnx */
      ...
      if ((fdnx - xe) > 0.00) {
        ...
        Becke_smalln_correction(n, n_thrd, 2.00, beta, lda_c, chi, chi2, chiSQ,
                                K, F1, F2, xe_pbe, fdnx_pbe, fdagrx_pbe, &xe,
                                &fdnx, &fdagrx);
      }
    }
