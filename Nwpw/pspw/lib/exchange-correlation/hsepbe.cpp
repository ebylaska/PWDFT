...
*/
static void FsrxHSE(double s, double nu, double *Fxhse, double *d10Fxhse,
                    double *d01Fxhse) {
  ...
  hden = one + hb1 * s + hb2 * s2 + hb3 * s3 + hb4 * s4 + hb5 * s5 + hb6 * s6 +
         hb7 * s7 + hb8 * s8 + hb9 * s9;
  H = hnum / hden;
  ...
  /* Calculate EG(s) */
  EGs = -(two / five) * CC * Fs * lambda -
        (four / r15) * BB * pow(lambda, two) -
  ...
  *Fxhse = Fxhse1 + Fxhse2 + Fxhse3 + Fxhse4 + Fxhse5 + Fxhse6;
  ...
  /* Calculate the first derivative of H with respect to the reduced density
   * gradient, s.*/
  ...
  faczeta = 0.0;
  if (std::abs(zeta) > tollz)
    faczeta = faczeta + d1zeta / sqrt(zeta);
  faczeta = faczeta - d1eta / sqrt(eta);
  d1EGs =
      -(two / five) * CC * (d1Fs * lambda + Fs * d1lambda) -
      (eight / r15) * BB * lambda * d1lambda -
      (r18 / five) * AA * lambda * lambda * d1lambda -
      (r14 / five) * srpi * d1lambda * pow(lambda, f52) -
      (r42 / five) * (pow(lambda, f52)) * d1lambda * (sqrt(zeta) - sqrt(eta)) -
      (six / five) * (pow(lambda, (seven / two))) * faczeta;
  ...
  d1Fxhse4 = -(eight / (nine * L3)) *
             ((d1EGs - three * EGs * tmp9) * tmp7 + EGs * tmp8);

  d1Fxhse5 = two * d1zeta * log(one - DD / Ms) +
             two * zeta * DD * d1Ms / (Ms * Ms * (one - DD / Ms));

  d1Fxhse6 = -two * d1eta * log(one - (DD - AA) / Ns) -
             two * eta * (DD - AA) * d1Ns / (Ns * Ns * (one - (DD - AA) / Ns));
  ...
  *d10Fxhse = d1Fxhse1 + d1Fxhse2 + d1Fxhse3 + d1Fxhse4 + d1Fxhse5 + d1Fxhse6;
  ...
  d1Fxhse1 = -((AA * (nu + sqrt(eta + nu2)) * zeta) /
               (sqrt(eta + nu2) * sqrt(nu2 + zeta) * (nu + sqrt(nu2 + zeta)) *
                (sqrt(eta + nu2) + sqrt(nu2 + zeta))));

  d1Fxhse2 = -((AA * eta * (nu / sqrt(eta + nu2) + nu / sqrt(nu2 + zeta))) /
               ((nu + sqrt(eta + nu2)) *
  ...
  d1Fxhse5 = (five * EGs *
              (pow(lambda, two) + four * nu3 * (nu + sqrt(lambda + nu2)) +
               lambda * nu * (five * nu + three * sqrt(lambda + nu2)))) /
             (three * pow((lambda + nu2), four) *
              pow((nu + sqrt(lambda + nu2)), three));

  d1Fxhse6 =
      (two * DD * zeta * (nu + sqrt(nu2 + zeta))) /
      (sqrt(lambda + nu2) * sqrt(nu2 + zeta) *
       (-DD + lambda + (nu + sqrt(lambda + nu2)) * (nu + sqrt(nu2 + zeta))));

  d1Fxhse7 = (two * (AA - DD) * eta * (nu + sqrt(eta + nu2))) /
             (sqrt(eta + nu2) * sqrt(lambda + nu2) *
              (AA - DD + lambda + nu2 + nu * sqrt(eta + nu2) +
               nu * sqrt(lambda + nu2) + sqrt(eta + nu2) * sqrt(lambda + nu2)));
...
static void LSDT(double a, double a1, double b1, double b2, double b3,
                 double b4, double srs, double *ec, double *ec_rs) {
   ...
   to nup, ndn, |grad nup|, |grad ndn|, and |grad n|.
    ...
    Exit - xce(*)  : PBE96 energy density
         ...
         - fdn(*,3): d(n*xce)/d|grad nup|, d(n*xce)/d|grad ndn|
                     d(n*xce)/d|grad n|
*/
void gen_HSE_BW_unrestricted(const int n2ft3d, double *dn_in, double *agr_in,
  ...
  for (int i = 0; i < n2ft3d; ++i) {
    ...
    ec_lda = ecu * (1.00 - F * z4) + ecp * F * z4 - eca * F * (1.00 - z4) / FZZ;

    ec_lda_rs = ecu_rs * (1.00 - F * z4) + ecp_rs * F * z4 -
                eca_rs * F * (1.00 - z4) / FZZ;

    ec_lda_zet =
        (4.00 * (zet * zet * zet) * F + FZ * z4) * (ecp - ecu + eca * iFZZ) -
        FZ * eca * iFZZ;
    ...
    Hpbe_phi = 3.00 * Hpbe / phi + (GAMMA * phi3 / Ipbe) * Ipbe_A * A_phi;

    Hpbe_t = (GAMMA * phi3 / Ipbe) * Ipbe_t;

    ec_lda_nup = ec_lda_zet - zet * ec_lda_zet + rs_n * ec_lda_rs;
    ec_lda_ndn = -ec_lda_zet - zet * ec_lda_zet + rs_n * ec_lda_rs;

    Hpbe_nup = ec_lda_nup * Hpbe_ec_lda + phi_zet * zet_nup * Hpbe_phi +
               t_nup * Hpbe_t;

    Hpbe_ndn = ec_lda_ndn * Hpbe_ec_lda + phi_zet * zet_ndn * Hpbe_phi +
               t_ndn * Hpbe_t;
...
*/
void gen_HSE_BW_restricted(const int n2ft3d, double *rho_in, double *agr_in,
                           const double x_parameter, const double c_parameter,
                           double *xce, double *fn, double *fdn) {
  ...
  for (int i = 0; i < n2ft3d; ++i) {
    ...
    ec = ec_lda + H;
    fnc = ec - (onethird * rs * ec_lda_rs) - (onethird * rs * Hrs) -
          (sevensixths * t * Ht);
    fdnc = 0.50 * Ht / ks;
