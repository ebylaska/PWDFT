...
static void Becke_smalln_correction(double n, double n_thrd, double fac,
                                    double beta, double lda_c, double chi,
                                    double chi2, double chiSQ, double K,
                                    double F1, double F2, double pbex,
                                    double dfpbednx, double dfpbedagrx,
  ...
  dsdx = 0.00;
  if (x < 100.00)
    dsdx = pow((1.00 / cosh(x)), 2);

  tmp1 = 6.00 * beta * chi / chiSQ;
  tmp2 = (1.00 + chi * K);
  dchi = -frthrd * chi / nf;
  dF1 = -frthrd * chi2 * F2 / nf;
  dF2 = ((-tmp1 / (1.00 + chi2) + K) / tmp2 - 2.00 * F2 * (tmp1 + K)) * dchi /
        tmp2;
  dfdnxdagr = -beta * (chi * dF2 + F2 * dchi);
  ddf0 = (thrd / nf) * (*fdnx) -
         frthrd * beta * nf_thrd * (dF1 - 2.00 * chi * F2 * dchi - chi2 * dF2);
  ...
  *fdnx = (1.00 - s) * (*fdnx) + s * dfpbednx + dsdn * nf * (-xeold + pbex);
  *fdagrx =
      (1.00 - s) * (*fdagrx) + s * dfpbedagrx + dsdagr * nf * (-xeold + pbex);
}
...
void gen_B3LYP_BW_unrestricted(const int n2ft3d, double *dn_in, double *agr_in,
                               const double x_parameter,
                               const double c_parameter, double *xce,
                               double *fn, double *fdn) {
  ...
  for (int j = 0; j < n2ft3d; ++j) {
    ...
    ff = (pow((1.00 + xi), frthrd) + pow((1.00 - xi), frthrd) - 2.00) /
         (2.00 * AA);
    dff = frthrd * (pow((1.00 + xi), thrd) - pow((1.00 - xi), thrd)) /
          (2.00 * AA);
    ...
    fnxup_dirac =
        ux_p + ff * (ux_f - ux_p) + (+1.00 - xi) * dff * (ex_f - ex_p);
    fnxdn_dirac =
        ux_p + ff * (ux_f - ux_p) + (-1.00 - xi) * dff * (ex_f - ex_p);
    ...
    dxxp = 2.00 * x + bp;
    ec_p = cp1 * log(pow(x, 2) / xxp) + cp2 * log((x + cp3) * (x + cp3) / xxp) +
           cp4 * atan(cp5 / (x + cp6));
    uc_p = ec_p - one6th * x *
                      (dp1 / x + dp2 / (x + cp3) + dp4 * dxxp / xxp +
                       dp5 / ((x + dp6) * (x + dp6) + dp7));

    xxf = pow(x, 2) + bf * x + cft;
    dxxf = 2.00 * x + bf;
    ec_f = cf1 * log(pow(x, 2) / xxf) + cf2 * log((x + cf3) * (x + cf3) / xxf) +
           cf4 * atan(cf5 / (x + cf6));
    uc_f = ec_f - one6th * x *
                      (df1 / x + df2 / (x + cf3) + df4 * dxxf / xxf +
                       df5 / ((x + df6) * (x + df6) + df7));
    ce_vosko = ec_p + ff * (ec_f - ec_p);
    fncup_vosko =
        uc_p + ff * (uc_f - uc_p) + (+1.00 - xi) * dff * (ec_f - ec_p);
    fncdn_vosko =
        uc_p + ff * (uc_f - uc_p) + (-1.00 - xi) * dff * (ec_f - ec_p);
    ...
    if ((dn_in[j] + dn_in[j + n2ft3d]) < DNS_CUT) {
      ...
      if (dn_in[j] < DNS_CUT) {
        ...
        F2up = (2.00 + chiup * Kup - 6.00 * BETA * chiup2 / chiupSQ) /
               pow((1.00 + chiup * Kup), 2.00);
        fdnxup = -pow(nup, (thrd)) * (4.00 / 3.00) *
                 (LDA_C + BETA * (F1up - chiup2 * F2up));
        fdagrxup = -BETA * chiup * F2up;
        if ((fdnxup - xeup) > 0.00) {
          gen_PBE96_x_unrestricted(nup, agrup, &xeup_pbe, &fdnxup_pbe,
                                   &fdagrxup_pbe);

          Becke_smalln_correction(nup, pow(nup, thrd), 1.0, BETA, LDA_C, chiup,
                                  chiup2, chiupSQ, Kup, F1up, F2up, xeup_pbe,
                                  fdnxup_pbe, fdagrxup_pbe, &xeup, &fdnxup,
                                  &fdagrxup);
        }
      }
      /*************END UP*****************/
      ...
      if (dn_in[j + n2ft3d] < DNS_CUT) {
        ...
        F2dn = (2.00 + chidn * Kdn - 6.00 * BETA * chidn2 / chidnSQ) /
               pow((1.00 + chidn * Kdn), 2.00);
        fdnxdn = -pow(ndn, (thrd)) * (4.00 / 3.00) *
                 (LDA_C + BETA * (F1dn - chidn2 * F2dn));
        fdagrxdn = -BETA * chidn * F2dn;
        if ((fdnxdn - xedn) > 0.00) {
          gen_PBE96_x_unrestricted(ndn, agrdn, &xedn_pbe, &fdnxdn_pbe,
                                   &fdagrxdn_pbe);
          Becke_smalln_correction(ndn, pow(ndn, thrd), 1.00, BETA, LDA_C, chidn,
                                  chidn2, chidnSQ, Kdn, F1dn, F2dn, xedn_pbe,
                                  fdnxdn_pbe, fdagrxdn_pbe, &xedn, &fdnxdn,
                                  &fdagrxdn);
        }
      }
      /*************END DOWN******************/
    ...
    fclda_u = -a * F_u * n - a * F -
              2.00 * a * b * G_u * Cf * (pow(2.00, twthrd)) *
                  (nup_etthrd + ndn_etthrd) -
              2.00 * a * b * G * Cf * (pow(2.00, twthrd)) * (8.00 / 3.00) *
                  nup_fvthrd;
    ...
    H0 = (a * b / 2.00) * (G + (1.00 / 3.00) * (nup * G_d + ndn * G_u) +
                           (1.00 / 4.00) * (nup * G_u + ndn * G_d));
    H0_u = (a * b / 2.00) *
           (G_u + (1.00 / 3.00) * (G_d + nup * G_du + ndn * G_uu) +
            (1.00 / 4.00) * (G_u + nup * G_uu + ndn * G_du));
    H0_d = (a * b / 2.00) *
           (G_d + (1.00 / 3.00) * (G_u + ndn * G_du + nup * G_dd) +
            (1.00 / 4.00) * (G_d + ndn * G_dd + nup * G_du));

    Hu = (a * b / 18.00) *
         (G + (15.00 / 4.00) * nup * G_u - (9.00 / 4.00) * ndn * G_d -
          (3.00) * nup * G_d + (3.00 / 2.00) * ndn * G_u);

    Hu_u = (a * b / 18.00) *
           (G_u + (15.00 / 4.00) * (G_u + nup * G_uu) -
            (9.00 / 4.00) * (ndn * G_du) - (3.00) * (G_d + nup * G_du) +
            (3.00 / 2.00) * (ndn * G_uu));

    Hu_d = (a * b / 18.00) *
           (G_d + (15.00 / 4.00) * (nup * G_du) -
            (9.00 / 4.00) * (G_d + ndn * G_dd) - (3.00) * (nup * G_dd) +
            (3.00 / 2.00) * (G_u + ndn * G_du));

    Hd = (a * b / 18.00) *
         (G + (15.00 / 4.00) * ndn * G_d - (9.00 / 4.00) * nup * G_u -
          (3.00) * ndn * G_u + (3.00 / 2.00) * nup * G_d);

    Hd_d = (a * b / 18.00) *
           (G_d + (15.00 / 4.00) * (G_d + ndn * G_dd) -
            (9.00 / 4.00) * (nup * G_du) - (3.00) * (G_u + ndn * G_du) +
            (3.00 / 2.00) * (nup * G_dd));

    Hd_u = (a * b / 18.00) *
           (G_u + (15.00 / 4.00) * (ndn * G_du) -
            (9.00 / 4.00) * (G_u + nup * G_uu) - (3.00) * (ndn * G_uu) +
    ...
    fdn[j + n2ft3d] =
        x_parameter * 0.90 * fdagrxdn + c_parameter * 0.81 * fc_agrdn;
    fdn[j + 2 * n2ft3d] = c_parameter * 0.81 * fc_agr;

    xce[j] = xce[j] + x_parameter * 0.100 * xe_dirac +
             c_parameter * 0.190 * ce_vosko;
    fn[j] = fn[j] + x_parameter * 0.100 * fnxup_dirac +
            c_parameter * 0.190 * fncup_vosko;
    fn[j + n2ft3d] = fn[j + n2ft3d] + x_parameter * 0.100 * fnxdn_dirac +
                     c_parameter * 0.190 * fncdn_vosko;
  }
}
...
void gen_B3LYP_BW_restricted(const int n2ft3d, double *rho_in, double *agr_in,
  ...
  for (int i = 0; i < n2ft3d; ++i) {
    ...
    /* vosko correlation */
    xx = 1.00 / (x * (x + bp) + cpt);
    ce_vosko = cp1 * log(xx * pow(x, 2)) + cp2 * log(xx * pow((x + cp3), 2)) +
               cp4 * atan(cp5 / (x + cp6));
    fnc_vosko =
        ce_vosko - one6th * x *
                       (dp1 / x + dp2 / (x + dp3) + dp4 * xx * (2.00 * x + bp) +
                        dp5 / (pow((x + dp6), 2) + dp7));
    ...
    if (rho_in[i] < DNS_CUT) {
      ...
      F2 = (2.00 + chi * K - (chi2)*6.00 * BETA / chiSQ) /
      ...
      if ((fdnx - xe) > 0.00) {
        ...
        Becke_smalln_correction(n, pow(n, thrd), 2.00, BETA, LDA_C, chi, chi2,
                                chiSQ, K, F1, F2, xe_pbe, fdnx_pbe, fdagrx_pbe,
                                &xe, &fdnx, &fdagrx);
      }
    }
    ...
    P_n = thrd_d * Fc_n * n_mfrthrd -
          ...
          5.00 * thrd * n_m * n_m - 4.00 * thrd * thrd * c * n_mfrthrd * n_m;

    Gc_n = Gc * P;
    Gc_nn = Gc * P * P + Gc * P_n;

    fc_lda = -a * Fc * n - abCf * Gc * (n_fr * n_fr);
    fdnc_lda = -a * Fc_n * n - a * Fc - abCf * Gc_n * n_fr * n_fr -
    ...
    xce[i] = xce[i] + x_parameter * 0.100 * xe_dirac +
             c_parameter * 0.190 * ce_vosko;
    fn[i] = fn[i] + x_parameter * 0.100 * fnx_dirac +
            c_parameter * 0.190 * fnc_vosko;
  }
}
