 ...
 *******************************************/
static void
vpp_read(PGrid *mygrid, char *fname, char *comment, int *psp_type, int *version,
 ...
 *                                         *
 *******************************************/
static void vpp_write(
    PGrid *mygrid, char *fname, char *comment, int psp_type, int version,
        ...
        for (l = 0; l < nprj[ia]; ++l) {
          ...
          if (move) {
            for (n = 0; n < nn; ++n) {
              ...
              sum[3 * n + 3 * nn * (l + nprjall)] =
                  mypneb->tt_pack_idot(1, Gx, xtmp);
              sum[3 * n + 1 + 3 * nn * (l + nprjall)] =
                  mypneb->tt_pack_idot(1, Gy, xtmp);
              sum[3 * n + 2 + 3 * nn * (l + nprjall)] =
                  mypneb->tt_pack_idot(1, Gz, xtmp);
            }
          }
        }
        nprjall += nprj[ia];
    ...
    if (move) {
      ...
      for (jj = jstart; jj < jend; ++jj) {
        ...
        for (l = 0; l < nprj[ia]; ++l) {
          fion[3 * jj] +=
              (3 - ispin) * 2.0 *
              DDOT_PWDFT(nn, &sw2[ll * nn], one, &sum[3 * nn * ll], three);
          fion[3 * jj + 1] +=
              (3 - ispin) * 2.0 *
              DDOT_PWDFT(nn, &sw2[ll * nn], one, &sum[1 + 3 * nn * ll], three);
          fion[3 * jj + 2] +=
              (3 - ispin) * 2.0 *
              DDOT_PWDFT(nn, &sw2[ll * nn], one, &sum[2 + 3 * nn * ll], three);
          ++ll;
        }
      }
    }
    ...
    if (nprj[ia] > 0) {
      ...
      if (move) {
        for (l = 0; l < nprj[ia]; ++l) {
          ...
          fion[3 * ii] +=
              (3 - ispin) * 2.0 * DDOT_PWDFT(nn, &sw2[l * nn], one, sum, three);
          fion[3 * ii + 1] += (3 - ispin) * 2.0 *
                              DDOT_PWDFT(nn, &sw2[l * nn], one, &sum[1], three);
          fion[3 * ii + 2] += (3 - ispin) * 2.0 *
                              DDOT_PWDFT(nn, &sw2[l * nn], one, &sum[2], three);
        }
      }
