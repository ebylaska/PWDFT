...
void psi_Hv4(Pneb *mygrid, 
           Kinetic_Operator *myke,
           Pseudopotential  *mypsp,
           double *psi, double *psi_r,
           double *vsr_l, double *vlr_l, 
           double *vc, double *xcp,
           double *Hpsi,
           bool move, double *fion)
{
   int indx1 = 0;
 ...
 *************************************
   ...
   This routine is used for DFPT and calculates 
    ...
    Entry - psi1_k,psi1_r - orbitals in k-space and r-space
            vall_r        - local psp + coulomb potential and xc and e-field in r-space
            hml           - <psi|H|psi> matrix
    Exit - Hpsi1 - gradient in k-space
*/  

void psi1_H_DFPT(Pneb *mygrid, 
                 Kinetic_Operator *myke,
                 Pseudopotential  *mypsp,
                 double *psi1, double *psi1_r,
                 double *vall_r, double *hml,
                 double *Hpsi1)

{
   ...
   for (int ms=0; ms<ispin; ++ms)
   {
      for (int i=0; i<(mygrid->neq[ms]); ++i)
      {
         ...
         mygrid->cc_pack_daxpy(1,(scal1),vpsi1,Hpsi1+indx1); // generates  -Hpsi1
...
void gen_vall_DFPT(Pneb *mygrid,
           Pseudopotential  *mypsp,
           double *vl, double *vc, double *xcp,
           double *vall_r)
{
 ...
 *************************************
   ...
   This routine calculates vall_r = iFFT[vsr_l] + vlr_l + vc + vlr + xcp + Vfield
...
void gen_vall_v4_DFPT(Pneb *mygrid, 
           Pseudopotential  *mypsp,
           double *vsr_l, double *vlr_l,
           double *vc, double *xcp,
           double *vall_r) 
