 ...
 *                                          *
 ********************************************/
void DFPT_Operators::start(double *psi, double *psi_r, double *dnall, double *dEpertdpsi_r)
{
   psi0   = psi;
   psi0_r = psi_r;
   dnall0 = dnall;
...
void DFPT_Operators::run()
{
   ...
   if (periodic)  mycoulomb12->mycoulomb1->vcoulomb(dng1,vc1);
   if (aperiodic)
   { 
      mygrid->rrr_Sum(dn1,dn1+(ispin-1)*n2ft3d,rho1);
   ...
   {
      mygrid->rrr_SMulAdd( eps,dn1+ms*n2ft3d,dnall0+ms*n2ft3d,dnallp+ms*n2ft3d);
      mygrid->rrr_SMulAdd(-eps,dn1+ms*n2ft3d,dnall0+ms*n2ft3d,dnallm+ms*n2ft3d);
   }
   ...
   for (int ms=0; ms<ispin; ++ms)
   {
      mygrid->arrr_Minus((0.5/eps), 
                         xcpp+ms*n2ft3d,
                         xcpm+ms*n2ft3d,
                         vxc1+ms*n2ft3d);
   }

   /* generate b1 -  b1(ms,i) = rc_fft[(vxc1(ms)+vc1)*psi0(ms,i) + dEpertdpsi(ms,i)] */
   indx1 = 0;
   for (int ms=0; ms<ispin; ++ms)
      for (int i=0; i<(mygrid->neq[ms]); ++i)
      {
         mygrid->rrrrr_SumMulAdd(vc1,vxc1+ms*n2ft3d,psi0_r+indx2,dEpertdpsi0_r+indx2,tmp);
