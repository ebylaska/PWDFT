...
std::array<double, 9> read_unita(const std::string& rtdbstring)
{
    ...
    for (int i = 0; i < 9; ++i) A[i] = u.at(i).get<double>();
    return A;
}
...
std::vector<double> read_coords_cart(const std::string& rtdbstring)
{
    ...
    std::vector<double> r;
    r.reserve(c.size());
    for (const auto& v : c) r.push_back(v.get<double>());
    return r;
}

std::vector<double> read_gradient_cart(const json& result)
{
    if (!result.contains("gradient")) return {};
                       ...
                       const std::vector<double>& coords_cart)
{
    ...
    for (double x : coords_cart) geometry["coords"].push_back(x);
...
{
    return {
        r[0]*A_inv[0] + r[1]*A_inv[3] + r[2]*A_inv[6],
                               ...
                               const std::vector<FracSymOp>& ops)
{
    ...
    for (int op = 0; op < n_ops; ++op)
    {
        ...
        for (int i = 0; i < n_atoms; ++i)
        {
            ...
            for (int k = 0; k < 3; ++k) g_out[i][k] += rotated[k];
        }
    }
...
void symmetrize_positions(std::vector<FracCoord>& f,
                          const AtomPermutation& perm,
                          const std::vector<FracSymOp>& ops)
{
    if (!perm.valid() || ops.empty()) return;
    ...
    for (int op = 0; op < n_ops; ++op)
    {
        ...
        {
            const int j = perm.perm[op][i];
            const FracCoord image = s.apply(f[j]);
            for (int k = 0; k < 3; ++k) f_out[i][k] += image[k];
        }
    }
...
struct LBFGS {
    ...
    std::vector<double> apply(const std::vector<double>& g) const
    {
        ...
        for (int k = n_stored - 1; k >= 0; --k)
        {
            const int idx = (next_slot - 1 - k + m) % m;
            double yq = 0.0;
            for (int i = 0; i < N; ++i) yq += y_hist[idx][i] * q[i];
            const double beta = rho_hist[idx] * yq;
            const double coef = alpha[k] - beta;
            for (int i = 0; i < N; ++i) q[i] += coef * s_hist[idx][i];
        }
                   ...
                   const AtomContext& ctx)
{
    ...
    for (int istep = 0; istep < max_steps; ++istep)
    {
        ...
        for (int i = 0; i < n_atoms; ++i)
        {
            for (int k = 0; k < 3; ++k) x[3*i + k] = f[i][k];
        ...
        const json result = compute_egs_values(2, comm, minimizer,
                                               rtdb_eval, coutput);
        ...
        {
            coutput << tag << " atom_minimizer: gradient size mismatch ("
                    << g_cart.size() << " vs " << N << "); aborting.\n";
            return 1;
        }
        ...
        {
            double tn2 = 0.0;
            for (int i = 0; i < N; ++i) tn2 += trial_dx[i]*trial_dx[i];
            ...
            const json trial_result = compute_egs_values(1, comm, minimizer,
                                                         trial_rtdb, coutput);
        ...
        if (accepted)
        {
            ...
            double rho = 0.0;
            if (pred_red > 1.0e-14) rho = act_red / pred_red;

            double an2 = 0.0;
            for (int i = 0; i < N; ++i) an2 += accepted_dx[i]*accepted_dx[i];
            ...
            if (oprint)
                coutput << tag << " Method        : "
                        << (used_lbfgs ? "L-BFGS" : "gradient fallback")
                        << '\n'
                        << tag << " Action        : accepted\n"
                        << tag << " rho           : " << rho << '\n'
                        << tag << " Trust radius  : " << trust_radius << '\n'
