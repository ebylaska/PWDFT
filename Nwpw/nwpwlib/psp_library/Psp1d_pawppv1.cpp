...
static void pawppv1_derivative_ngrid(const int ng, const double log_amesh,
  ...
  df[0] = aa *
          (-147.00 * f[0] + 360.00 * f[1] - 450.00 * f[2] + 400.00 * f[3] -
           225.00 * f[4] + 72.00 * f[5] - 10.00 * f[6]) /
          r[0];
  df[1] = aa *
          (-10.00 * f[0] - 77.00 * f[1] + 150.00 * f[2] - 100.00 * f[3] +
           50.00 * f[4] - 15.00 * f[5] + 2.00 * f[6]) /
          r[1];
  df[2] = aa *
          (+2.00 * f[0] - 24.00 * f[1] - 35.00 * f[2] + 80.00 * f[3] -
           30.00 * f[4] + 8.00 * f[5] - 1.00 * f[6]) /
          r[2];
  df[ng - 3] = aa *
               (-2.00 * f[ng - 1] + 24.00 * f[ng - 2] + 35.00 * f[ng - 3] -
                80.00 * f[ng - 4] + 30.00 * f[ng - 5] - 8.00 * f[ng - 6] +
                1.00 * f[ng - 7]) /
               r[ng - 3];
  df[ng - 2] = aa *
               (+10.00 * f[ng - 1] + 77.00 * f[ng - 2] - 150.00 * f[ng - 3] +
                100.00 * f[ng - 4] - 50.00 * f[ng - 5] + 15.00 * f[ng - 6] -
                2.00 * f[ng - 7]) /
               r[ng - 2];
  df[ng - 1] = aa *
               (+147.00 * f[ng - 1] - 360.00 * f[ng - 2] + 450.00 * f[ng - 3] -
                400.00 * f[ng - 4] + 225.00 * f[ng - 5] - 72.00 * f[ng - 6] +
                10.00 * f[ng - 7]) /
               r[ng - 1];
  for (auto i = 3; i < (ng - 3); ++i)
    df[i] = aa *
            (-1.00 * f[i - 3] + 9.00 * f[i - 2] - 45.00 * f[i - 1] +
             45.00 * f[i + 1] - 9.00 * f[i + 2] + 1.00 * f[i + 3]) /
            r[i];
}
...
Psp1d_pawppv1::Psp1d_pawppv1(Parallel *myparall, const char *psp_name) {
  ...
  if (std::abs(zion - zv) > 1.0e-9) {
    double ecorez = util_log_coulomb0_energy(core_ae, zion - zv, rgrid, n1dgrid,
                                             log_amesh, zion);
    double ecorecore =
        util_log_coulomb_energy(core_ae, zion - zv, rgrid, n1dgrid, log_amesh);
...
#define matindx4(i1, j1, i2, j2, l, nbasis)                                    \
  (i1 + j1 * nbasis + i2 * nbasis * nbasis + j2 * nbasis * nbasis * nbasis +   \
   l * nbasis * nbasis * nbasis * nbasis)
#define matovlp(na, nb, l, ii, nmax, lmax)                                     \
  ((na - 1) + (nb - 1) * nmax + l * nmax * nmax +                              \
   (ii - 1) * nmax * nmax * (lmax + 1))
...
void Psp1d_pawppv1::vpp_generate_paw_matrices(Parallel *myparall, double *Gijl,
  ...
  std::memset(comp_charge_matrix, 0,
              nbasis * nbasis * (2 * lmax + 1) * sizeof(double));
  for (auto i = 0; i < nbasis; ++i)
    for (auto j = 0; j <= i; ++j) {
      ...
      for (auto l = 0; l < (2 * lmax + 1); ++l) {
        double d =
            util_log_integrate_def(2 * l + 2, f1, l, rgrid, log_amesh, icut);
  ...
  for (auto l = 0; l < (2 * lmax + 1); ++l) {
    ...
    for (auto i = 0; i < nbasis; ++i) {
      for (auto j = 0; j <= i; ++j) {
        ...
        d = util_log_multipole_energy(l, icut, rgrid, k1, f1, k2, f2,
                                      log_amesh);
  ...
  std::memset(hartree_matrix, 0,
              nbasis * nbasis * nbasis * nbasis * (2 * lmax + 1) *
                  sizeof(double));
    ...
    for (auto j = 0; j <= i; ++j) {
      ...
      for (auto i2 = 0; i2 < nbasis; ++i2)
        for (auto j2 = 0; j2 <= i2; ++j2) {
          ...
          for (auto l = 0; l < (2 * lmax + 1); ++l) {
            d = ((double)(2 * l + 1)) * util_double_factorial(2 * l + 1) *
                pow(sigma, (2 * l + 1));
            vgl = 4.00 * sqrt(twopi) / d;

            if ((l <= sum_l) && (l >= dif_l) && (l <= sum_l2) &&
                (l >= dif_l2)) {
              d = util_log_multipole_energy(l, icut, rgrid, k1, f1, k2, f2,
                                            log_amesh) -
                  util_log_multipole_energy(l, icut, rgrid, k1, f3, k2, f4,
                                            log_amesh);

              hartree_matrix[matindx4(i, j, i2, j2, l, nbasis)] =
                  d -
              ...
              hartree_matrix[matindx4(j, i, j2, i2, l, nbasis)] =
                  d -
                  2.00 * comp_pot_matrix[matindx2(j, i, l, nbasis)] *
                      comp_charge_matrix[matindx2(j2, i2, l, nbasis)] -
                  vgl * comp_charge_matrix[matindx2(j, i, l, nbasis)] *
  ...
  for (auto i = 0; i < nbasis; ++i) {
    ...
    for (auto k = 0; k < icut; ++k)
      f1[k] = 0.50 * (dphi_ae[k + i * n1dgrid] * dphi_ae[k + i * n1dgrid] -
              ...
              0.50 * la * (la + 1) *
                  (phi_ae[k + i * n1dgrid] * phi_ae[k + i * n1dgrid] -
                   phi_ps[k + i * n1dgrid] * phi_ps[k + i * n1dgrid]) /
                  pow(rgrid[k], 2);
      ...
      if (la == lb) {
        for (auto k = 0; k < icut; ++k)
          f1[k] = 0.50 * (dphi_ae[k + i * n1dgrid] * dphi_ae[k + j * n1dgrid] -
                  ...
                  0.50 * la * (la + 1) *
                      (phi_ae[k + i * n1dgrid] * phi_ae[k + j * n1dgrid] -
                       phi_ps[k + i * n1dgrid] * phi_ps[k + j * n1dgrid]) /
                      pow(rgrid[k], 2);
  ...
  for (auto i = 0; i < nbasis; ++i) {
    int la = lps[i];
    int na = nps[i] - la;
    Gijl[matovlp(na, na, la, 4, nmax, lmax)] +=
        q * comp_pot_matrix[matindx2(i, i, 0, nbasis)] +
        d * Gijl[matovlp(na, na, la, 2, nmax, lmax)];
    for (auto j = 0; j < i; ++j) {
      int lb = lps[j];
      int nb = nps[j] - lb;
      if (la == lb) {
        Gijl[matovlp(na, nb, la, 4, nmax, lmax)] +=
            q * comp_pot_matrix[matindx2(i, j, 0, nbasis)] +
            d * Gijl[matovlp(na, nb, la, 2, nmax, lmax)];

        Gijl[matovlp(nb, na, la, 4, nmax, lmax)] +=
            q * comp_pot_matrix[matindx2(j, i, 0, nbasis)] +
  ...
  for (auto i = 0; i < nbasis; ++i) {
    ...
    for (auto j = 0; j < i; ++j) {
      ...
      if (la == lb) {
        for (auto k = 0; k < icut; ++k)
          f1[k] =
              phi_ps[k + i * n1dgrid] * phi_ps[k + j * n1dgrid] * (-v_ps[k]);
  ...
  if (zcore > 0.0) {
    ...
    for (auto i = 0; i < nbasis; ++i) {
      ...
      d = util_log_multipole_energy(0, icut, rgrid, power_f, f1, 2, f3,
                                    log_amesh);
    ...
    for (auto i = 0; i < nbasis; ++i) {
      ...
      d = zcore *
          util_log_integrate_def(power_f, f1, 0, rgrid, log_amesh, icut);
      Gijl[matovlp(na, na, la, 5, nmax, lmax)] -= d;
      ...
      for (auto j = 0; j < i; ++j) {
        ...
        if (la == lb) {
          for (auto k = 0; k < icut; ++k)
            f1[k] =
                phi_ae[k + i * n1dgrid] * phi_ae[k + j * n1dgrid] / rgrid[k];
  ...
  DAXPY_PWDFT(nnn, rone, &Gijl[3 * nnn], one, Gijl,
              one); // ** Vsrloc - short range pseudo  **
  DAXPY_PWDFT(nnn, rone, &Gijl[4 * nnn], one, Gijl,
              one); // ** vcore - valence-core   **
}
 ...
 *                                         *
 *******************************************/
void Psp1d_pawppv1::vpp_generate_ray(Parallel *myparall, int nray,
                                     double *G_ray, double *vl_ray,
                                     double *vlpaw_ray, double *vnl_ray) {
  ...
  for (auto k1 = (1 + myparall->taskid()); k1 < nray; k1 += myparall->np()) {
    ...
    for (auto ib = 0; ib < nbasis; ++ib) {
      ...
      if (la == 3) {
        ...
        vnl_ray[k1 + ib * nray] =
            p3 *
            util_log_integrate_def(power_f, f, 0, rgrid, log_amesh, n1dgrid) /
            q;
      }
      // ::::::::::::::::::::::::::::::  d-wave  ::::::::::::::::::::::::::::::
      if (la == 2) {
    ...
    if (version == 3) {
      vl_ray[k1] =
          -zv * (forpi / (q * q)) * exp(-0.25 * (q * sigma) * (q * sigma));
    } else if (version == 4) {
      vl_ray[k1] = -zv * (forpi / (q * q)) *
                   (exp(-0.25 * (q * sigma) * (q * sigma)) -
    ...
    vlpaw_ray[k1] =
        forpi * util_log_integrate_def(0, f, 0, rgrid, log_amesh, n1dgrid) / q;
  }
  ...
  vlpaw_ray[0] =
      forpi * util_log_integrate_def(0, f, 0, rgrid, log_amesh, n1dgrid);
  ...
  for (auto ib = 0; ib < nbasis; ++ib) {
    ...
    if (la == 0) {
      ...
      vnl_ray[ib * nray] =
          p0 * util_log_integrate_def(power_f, f, 0, rgrid, log_amesh, n1dgrid);
    }
  }
 ...
 *                                         *
 *******************************************/
void Psp1d_pawppv1::vpp_generate_spline(PGrid *mygrid, int nray, double *G_ray,
  ...
  yp1 = (-50.00 * vlpaw_ray[1] + 96.00 * vlpaw_ray[2] - 72.00 * vlpaw_ray[3] +
         32.00 * vlpaw_ray[4] - 6.00 * vlpaw_ray[5]) /
        (24.00 * dG);
  util_spline(&(G_ray[1]), &(vlpaw_ray[1]), nray - 1, yp1, 0.00,
              &(vlpaw_splineray[1]), tmp_splineray);

  for (auto i = 0; i < nbasis; ++i)
    util_spline(G_ray, &(vnl_ray[i * nray]), nray, 0.00, 0.00,
                &(vnl_splineray[i * nray]), tmp_splineray);
  ...
  for (auto k = 0; k < npack0; ++k) {
    ...
    if (q > 1.0e-9) {
      ...
      vl[k] = util_splint(&(G_ray[1]), &(vl_ray[1]), &(vl_splineray[1]),
                          nray - 1, nx, q);
      vlpaw[k] = util_splint(&(G_ray[1]), &(vl_ray[1]), &(vl_splineray[1]),
                             nray - 1, nx, q);
    } else {
      vl[k] = vl_ray[0];
      vlpaw[k] = vl_ray[0];
...
/****************************************************
 *                                                  *
 *     Psp1d_pawppv1::cpp_generate_paw_matrices     *
 *                                                  *
 ****************************************************/
#define matindx2(i, j, l, nbasis) (i + j * nbasis + l * nbasis * nbasis)
#define matindx4(i1, j1, i2, j2, l, nbasis)                                    \
...
void Psp1d_pawppv1::cpp_generate_paw_matrices(Parallel *myparall, double *Gijl,
                                              double *comp_charge_matrix,
                                              double *comp_pot_matrix,
                                              double *hartree_matrix) 
{
...
/*******************************************
 *                                         *
 *     Psp1d_pawppv1::cpp_generate_ray     *
 *                                         *
 *******************************************/
void Psp1d_pawppv1::cpp_generate_ray(Parallel *myparall, int nray,
                                     double *G_ray, double *vl_ray,
                                     double *vlpaw_ray, double *vnl_ray) 
{
   double pi = 4.00 * atan(1.0);
