...
void p_xpansion(double p[]) {
  ...
  for (i = 0; i <= match; ++i) {
    ...
    r10 = r8 * r2;
    r12 = r10 * r2;
    p[i] = c[0] + c[1] * r2 + c[2] * r4 + c[3] * r6 + c[4] * r8 + c[5] * r10 +
           c[6] * r12;
  }
} /* p_xpansion */
...
void dp_xpansion(double dp[]) {
  ...
  for (i = 0; i <= match; ++i) {
    ...
    r9 = r7 * r2;
    r11 = r9 * r2;
    dp[i] = 2.0 * c[1] * r1 + 4.0 * c[2] * r3 + 6.0 * c[3] * r5 +
            8.0 * c[4] * r7 + 10.0 * c[5] * r9 + 12.0 * c[6] * r11;
  }
} /* dp_xpansion */
...
void ddp_xpansion(double ddp[]) {
  ...
  for (i = 0; i <= match; ++i) {
    ...
    r8 = r6 * r2;
    r10 = r8 * r2;
    ddp[i] = 2.0 * c[1] + 12.0 * c[2] * r2 + 30.0 * c[3] * r4 +
             56.0 * c[4] * r6 + 90.0 * c[5] * r8 + 132.0 * c[6] * r10;
  }
} /* ddp_xpansion */
...
void dddp_xpansion(double dddp[]) {
  ...
  for (i = 0; i <= match; ++i) {
    ...
    r7 = r5 * r2;
    r9 = r7 * r2;
    dddp[i] = 24.0 * c[2] * r1 + 120.0 * c[3] * r3 + 336.0 * c[4] * r5 +
              720.0 * c[5] * r7 + 1320.0 * c[6] * r9;
  }
} /* dddp_xpansion */
...
void ddddp_xpansion(double ddddp[]) {
  ...
  for (i = 0; i <= match; ++i) {
    ...
    r6 = r4 * r2;
    r8 = r6 * r2;
    ddddp[i] = 24.0 * c[2] + 360.0 * c[3] * r2 + 1680.0 * c[4] * r4 +
               5040.0 * c[5] * r6 + 11880.0 * c[6] * r8;
  }
} /* ddddp_xpansion */
...
void psp_xpansion() {
  ...
  for (i = 0; i <= match; ++i) {
    Vl[i] = el + dp[i] * (l + 1.0) / r[i] + 0.5 * ddp[i] + 0.5 * dp[i] * dp[i];
  }
...
void dpsi_xpansion() {
  ...
  for (i = 0; i <= match; ++i)
    wl_prime[i] =
        (al * r[i]) * ((l + 1.0) + r[i] * dp[i]) * pow(r[i], l) * exp(p[i]);
...
void get_c0_c10() {
  ...
  while ((tolerance > SMALL) && (iteration <= 50)) {
    ...
    if (psp_core_charge < 0.0)
      psp_core_charge = fabs(psp_core_charge); /*debug*/
...
double get_c0_c12(double gamma) {
  ...
  while ((tolerance > SMALL) && (iteration <= 50)) {
    ...
    psp_core_charge = Norm_LogGrid(match, l + 1.0, wl);
    if (psp_core_charge < 0.0)
      psp_core_charge = fabs(psp_core_charge);
...
void init_xpansion(int l_in, int match_in, double occupation_in, double el_in,
                   double *Vall_in, double *ul_in, double *ul_prime_in,
                   double *Vl_in, double *wl_in, double *wl_prime_in) {
  int i, iteration;
  ...
  ddVall_match =
      (-1.0 / (r[match] * r[match] * al)) * Derivative7_4(match, Vall) +
  ...
  poly[2] = 2.0 * Vall_match - 2.0 * el - (2.0 * (l + 1.0) / rc[1]) * poly[1] -
            poly[1] * poly[1];
  poly[3] = 2.0 * dVall_match + (2.0 * (l + 1.0) / rc[2]) * poly[1] -
            (2.0 * (l + 1.0) / rc[1]) * poly[2] - 2.0 * poly[1] * poly[2];
  poly[4] = 2.0 * ddVall_match - (4.0 * (l + 1.0) / rc[3]) * poly[1] +
            (4.0 * (l + 1.0) / rc[2]) * poly[2] -
            (2.0 * (l + 1.0) / rc[1]) * poly[3] - 2.0 * poly[2] * poly[2] -
            2.0 * poly[1] * poly[3];
  ...
  iteration = 0;
  constraint_mid = 1.0;
  while ((fabs(dgamma) > SMALL) && (constraint_mid != 0.0) &&
         (iteration <= 80)) {
    ++iteration;
