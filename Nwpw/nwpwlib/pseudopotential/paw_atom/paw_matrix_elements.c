    ...
    for (j = 0; j <= nbasis - 1; j++) {
      if (orb_l[i] == orb_l[j]) {
        for (k = 0; k <= Ngrid - 1; k++) {
          ...
          f2[k] = 0.5 * (phi_prime[i][k] * phi_prime[j][k]) /
                      pow(rgrid[k] * log_amesh, 2.0) +

                  0.5 * orb_l[i] * (orb_l[i] + 1) / pow(rgrid[k], 2.0) *
                      phi[i][k] * phi[j][k];
        ...
        for (k = 0; k < Ngrid; ++k) {
          ...
          f2[k] = 0.5 * (phi_ps_prime[i][k] * phi_ps_prime[j][k]) /
                      pow(rgrid[k] * log_amesh, 2.0) +

                  0.5 * orb_l[i] * (orb_l[i] + 1) / pow(rgrid[k], 2.0) *
                      phi_ps[i][k] * phi_ps[j][k];
...
double **paw_get_pointer_dH_ae() { return dH_ae; }

double **paw_get_pointer_dH_ps() { return dH_ps; }

double **paw_get_pointer_dO_ae() { return dO_ae; }

double **paw_get_pointer_dO_ps() { return dO_ps; }
