...
double paw_find_sigma_comp() {
  ...
  tmp = paw_bisection(paw_boundary_function, rc1, rc2,
                      0.0001 * comp_charge_tolerance);
...
****************************************/
double *paw_find_comp_charge_potential(double Zion, double charge,
                                       double ps_charge) {
  int k;
  int Ngrid;
  double comp_charge;
  ...
  for (k = 0; k <= Ngrid - 1; k++)
    comp_charge_density[k] = comp_charge * 4 /
                             (sqrt(PI) * pow(sigma_comp, 3.0)) *
...
double paw_get_sigma_comp() { return sigma_comp; }

double paw_get_comp_charge_radius() { return r_comp; }
