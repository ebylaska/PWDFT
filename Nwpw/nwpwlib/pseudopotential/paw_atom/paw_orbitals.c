  ...
  while ((!converged) && (it < max_iter)) {
    ...
    for (i = 0; i <= Nvalence - 1; i++) {
      ...
      if (Solver_Type == Schrodinger) {
        paw_R_Schrodinger(n[i], l[i], Vi, &Etmp, psi[i], psi_prime[i]);
      } else {
        paw_R_Pauli(n[i], l[i], Zion, Vi, &Etmp, psi[i], psi_prime[i]);
      }
...
void paw_solve_unoccupied_orbitals() {
  ...
  for (i = Nvalence; i <= Nbound - 1; i++) {
    ...
    if (state == 0) {
      ...
      if (Solver_Type == Schrodinger) {
        status = paw_R_Schrodinger(n[i], l[i], V, &eigenvalue[i], psi[i],
                                   psi_prime[i]);
      } else {
        status = paw_R_Pauli(n[i], l[i], Zion, V, &eigenvalue[i], psi[i],
                             psi_prime[i]);
      }
    }
...
void paw_solve_scattering_orbitals() {
  ...
  for (i = Nbound; i < Ntotal; i++) {
    ...
    if (Solver_Type == Schrodinger) {
      paw_R_Schrodinger_Fixed_E(l[i], V, i_match, eigenvalue[i], psi[i],
                                psi_prime[i]);
    } else {
      paw_R_Pauli_Fixed_E(n[i], l[i], Zion, V, i_match, eigenvalue[i], psi[i],
                          psi_prime[i]);
    }
...
void paw_print_orbital_information(FILE *fp) {
  ...
  for (i = 0; i < Ntotal; ++i) {
    fprintf(fp, "%d\t%s\t%.2lf\t\t%f\t%s\n", n[i], paw_spd_Name(l[i]), fill[i],
            eigenvalue[i], paw_orbital_type_name(orb_type[i]));
  }
...
void paw_print_orbitals_to_file(char *atom_name) {
  ...
  if (paw_debug()) {
    ...
    fprintf(fp, "set title \" Kohn-Sham bound states for %s \" \n", atom_name);
    ...
    for (i = 0; i <= Ntotal - 1; i++) {
      ...
      sprintf(script_filename, "%s%s_%s_orb.plt", paw_sdir(), atom_name,
              nl_name);
      ...
      fprintf(fp, "plot \"%s\" using 1:%d   title \"%s\"  ", data_filename,
              i + 2, title);
...
int paw_get_l(int i) { return l[i]; }

int paw_get_orb_type(int i) { return orb_type[i]; }

int *paw_get_pointer_l_array() { return l; }

double paw_get_e(int i) { return eigenvalue[i]; }

double paw_get_fill(int i) { return fill[i]; }

double *paw_get_pointer_fill_array() { return fill; }

double *paw_get_psi(int i) { return psi[i]; }

double **paw_get_pointer_psi_array() { return psi; }

double **paw_get_pointer_psi_prime_array() { return psi_prime; }

double *paw_get_psi_prime(int i) { return psi_prime[i]; }

int paw_get_Ntotal() { return Ntotal; }
...
/****************************************
 ...
 Date & Time		  : 3/31/99 3:35:46 PM
****************************************/
double *paw_get_pointer_density() { return rho; }
