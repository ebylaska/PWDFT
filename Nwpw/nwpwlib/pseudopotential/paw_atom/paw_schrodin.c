...
int paw_R_Schrodinger(int n, int l, double *v, double *Eig, double *u,
                      double *uprime) {
  int i, j, iteration, node, match, Ninf, Ngrid;

  double E, de, Emax, Emin, log_amesh, log_amesh2, gamma, L2, sum, a, scale,
      m1scale, uout, upout, upin, *r, *f_upp, *r2, *upp;
...
int paw_R_Schrodinger_Fixed_E(int l, double *v, int match, double E, double *u,
                              double *uprime) {
  int i, j, Ngrid;

  double log_amesh, log_amesh2, gamma, L2, r2, *r, *f_upp, *upp;
...
****************************************/
int paw_R_Schrodinger_Fixed_Logderiv(int n, int l, double *v, int match,
                                     double u_logderiv, double *Eig, double *u,
                                     double *uprime)
...
int paw_R_Schrodinger_Fixed_Logderiv1(int n, int l, double *v, int match,
                                      double u_logderiv, double *Eig, double *u,
                                      double *uprime)
...
void paw_R_Schrodinger_Fixed_E1(int l, double *v, double *f, int match,
  ...
  while (i < match && fabs(u[i]) < max_u) {
    ...
    for (j = 0; j < Corrector_Iterations; ++j) {
      upp[i + 1] = log_amesh * uprime[i + 1] + f_upp[i + 1] * u[i + 1] -
                   2.0 * log_amesh2 * r[i + 1] * r[i + 1] * f[i + 1];
      uprime[i + 1] = paw_Corrector_Out(i, uprime, upp);
      u[i + 1] = paw_Corrector_Out(i, u, uprime);
    }
