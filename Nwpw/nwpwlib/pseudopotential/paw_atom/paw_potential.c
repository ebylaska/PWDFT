...
****************************************/
void paw_init_potential() { Vi = paw_alloc_LogGrid(); }
...
****************************************/
double *paw_get_kohn_sham_potential() { return Vi; }
...
double paw_find_rc_pot(double rcut_in) {
  ...
  rc2 = 2 * rcut_in;
  tmp =
      paw_bisection(paw_function_for_rc_pot, rc1, rc2, 0.00001 * pot_tolerance);
...
****************************************/
void paw_init_paw_potential(int a_nbasis, double c0, double a_r_ref,
                            double *a_r_potential, double *V_ks) {
  ...
  */
  ...
  V_prime =
      0.5 * (V_ks[ic + 1] - V_ks[ic - 1]) / (rc * paw_log_amesh_LogGrid());
  ...
  for (k = 0; k < ic; ++k)
    V_ref[k] = a + b * rgrid[k] * rgrid[k] +
               d * rgrid[k] * rgrid[k] * rgrid[k] * rgrid[k];
...
****************************************/
double *paw_get_paw_potential(int i) { return V_paw[i]; }

double *paw_get_pointer_pseudopotential() { return V_pseudo; }
...
****************************************/
void paw_update_paw_potential(int *conv_status, int i, double eig,
                              double eig_ps, double *w) {
...
****************************************/
double *paw_get_ref_pot() { return V_ref; }
...
double paw_get_potential_matching_radius() { return r_ref; }
...
void paw_print_paw_potential_to_file(char *atom_name) {
  ...
  if (paw_debug()) {
    ...
    for (i = 0, j = 4; i <= nbasis - 1; i++, j = j + 1) {
      ...
      sprintf(script_filename, "%s%s_%s_pot.plt", paw_sdir(), atom_name,
              nl_name);
