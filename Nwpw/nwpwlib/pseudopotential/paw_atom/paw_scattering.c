...
void paw_init_paw_scattering_set() { paw_scattering_initialized = False; }
...
****************************************/
void paw_solve_paw_scattering(int l, double r, double e, double *psi,
                              double *psi_prime) {
  ...
  paw_R_Schrodinger_Fixed_E(l, V, i_end_point, e, u, u_prime);
  ...
  for (i1 = 0; i1 <= nbasis - 1; i1++) {
    ...
    if (l == orb_l[i1]) {
      paw_R_Schrodinger_Fixed_E1(l, V, prj_ps[i1], i_end_point, e, w[i1],
                                 w_prime[i1]);
    }
  }
  ...
  for (i1 = 0; i1 <= nbasis - 1; i1++) {
    for (j1 = 0; j1 <= nbasis - 1; j1++) {
      ...
      A[i1][j1] =
          dH_ae[i1][j1] - e * dO_ae[i1][j1] - dH_ps[i1][j1] + e * dO_ps[i1][j1];
    }
  }
