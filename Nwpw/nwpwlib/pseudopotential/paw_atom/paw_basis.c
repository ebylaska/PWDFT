...
****************************************/
void paw_init_paw_basis(char *a_nodal_constraint, char *a_projector_method,
                        int a_nbasis, int *a_n, int *a_l, double *r_match) {
  ...
  for (i = 0; i < nbasis; ++i) {
    ...
    if (fabs(phi[i][i_match]) < SMALL) {
      printf("error, your orbital matching sphere is to close to the node of "
             "%d%d orbital \n",
             prin_n[i], orb_l[i]);
      exit(1);
    } else {
...
void paw_generate_pseudo_orbitals() {
  ...
  for (i = 0; i <= nbasis - 1; i++) {
    ...
    while ((iteration <= max_iter) && (!converged)) {
      ...
      */
      ...
      status = paw_R_Schrodinger_Fixed_Logderiv(prin_n_ps[i], orb_l[i], V,
                                                match, log_deriv[i], &e_ps[i],
                                                phi_ps[i], phi_ps_prime[i]);
  ...
  for (i = 0; i <= nbasis - 1; i++) {
    ...
    for (k = 0; k <= Ngrid - 1; k++) {
      f[k] = 0.5 * phi_prime[i][k] / (rgrid[k] * log_amesh) * phi_prime[i][k] /
                 (rgrid[k] * log_amesh) -
  ...
  for (i = 0; i <= nbasis - 1; i++) {
    for (j = 0; j <= nbasis - 1; j++) {
      ...
      if (orb_l[i] == orb_l[j]) {
        ...
        if (fabs(norm) > norm_error)
          norm_error = fabs(norm);
      }
    }
  }
  ...
  for (i = 0; i <= nbasis - 1; i++) {
    for (j = 0; j <= i; j++) {
      ...
      for (k = 0; k <= Ngrid - 1; k++) {
        ...
        phi_ps_prime[i][k] =
            phi_ps_prime[i][k] + U_inv[j][i] * phi_ps0_prime[j][k];
      }
    }
  }
...
void paw_solve_pseudo_orbitals() {
  ...
  for (i = 0; i <= nbasis - 1; i++) {
    ...
    paw_solve_paw_scattering(orb_l[i], 3 * r_orbital[i], e_ps[i], psi_ps[i],
                             psi_ps_prime[i]);

    if (paw_debug())
      printf("%d%s log derivative   %le  %le \n", prin_n[i],
             paw_spd_Name(orb_l[i]),
             psi_ps_prime[i][i_end_point] /
                 (psi_ps[i][i_end_point] * rgrid[i_end_point] *
                  paw_log_amesh_LogGrid()),
             phi0_prime[i][i_end_point] /
                 (phi0[i][i_end_point] * rgrid[i_end_point] *
                  paw_log_amesh_LogGrid()));
...
****************************************/
double *paw_get_pointer_paw_ps_density() { return rho_ps; }
...
/****************************************
 ...
 Date & Time		  : 4/9/99 1:13:57 PM
****************************************/
double *paw_get_pointer_paw_density() { return rho; }

double paw_get_paw_kinetic_energy() { return ekin; }
/****************************************
...
void paw_print_basis_to_file(char *atom_name) {
  ...
  if (paw_debug()) {
    ...
    for (k = 0; k <= Ngrid - 1; k++) {
      ...
      for (i = 0; i <= nbasis - 1; i++)
        fprintf(fp, "\t%le \t%le \t%le ", phi[i][k], phi_ps[i][k],
                prj_ps[i][k]);
    ...
    for (i = 0, j = 2; i <= nbasis - 1; i++, j = j + 3) {
      ...
      sprintf(script_filename, "%s%s_%s_paw.plt", paw_sdir(), atom_name,
              nl_name);
      ...
      fprintf(fp, "plot \"%s\" using 1:%d title \" all-electron orbital \" ,  ",
              data_filename, j);
      fprintf(fp, "\"%s\" using 1:%d title \" pseudo orbital \" ,  ",
              data_filename, j + 1);
...
void paw_print_basis_test_to_file(char *atom_name) {
  ...
  if (paw_debug()) {
    ...
    for (i = 0, j = 2; i <= nbasis - 1; i++, j = j + 2) {
      ...
      sprintf(script_filename, "%s%s_%s_test.plt", paw_sdir(), atom_name,
              nl_name);
      ...
      fprintf(fp, "set xlabel \"r (a0)\" \n");
      fprintf(fp,
              "set title \" %s paw_basis test for %s\\n (e=%f Hartree)\" \n",
              nl_name, atom_name, e_ps[i]);

      sprintf(title, "%s (%f)", nl_name, e_ps[i]);

      fprintf(fp,
              "plot \"%s\" using 1:%d title \" original pseudo orbital \" ,  ",
              data_filename, j);
      fprintf(fp,
              "\"%s\" using 1:%d title \" solution of PAW Hamiltonian \"  with "
              "points\n",
              data_filename, j + 1);
...
int paw_get_nbasis() { return nbasis; }

int *paw_get_pointer_paw_l_array() { return orb_l; }

int *paw_get_pointer_paw_n_array() { return prin_n; }

int *paw_get_pointer_paw_n_ps_array() { return prin_n_ps; }

double *paw_get_pointer_paw_e_array() { return e_ps; }

double **paw_get_pointer_paw_psi_array() { return phi; }

double **paw_get_pointer_paw_psi_prime_array() { return phi_prime; }

double **paw_get_pointer_paw_psi_ps_array() { return phi_ps; }

double **paw_get_pointer_paw_psi_ps_prime_array() { return phi_ps_prime; }

double **paw_get_pointer_paw_prj_ps_array() { return prj_ps; }

double **paw_get_pointer_paw_prj_ps0_array() { return prj_ps0; }

int paw_get_max_i_r_orbital() { return max_i_r_orbital; }
double paw_get_r_orbital(int i) { return r_orbital[i]; }

int paw_projectors_are_done() { return projectors_done; }

double **paw_get_pointer_paw_psi_ps_unscr_array() { return phi_ps0; }
...
/****************************************
 ...
 Date & Time		  : 5/15/00
****************************************/
void paw_scattering_test(double e1, double e2, int number_points, int l,
                         double r) {
  ...
  for (i = 0; i <= number_points - 1; i++) {
    ...
    paw_R_Schrodinger_Fixed_E(l, V_ks, i_end_point, e_test, psi1, psi1_prime);

    log_grid_ae[i] =
        psi1_prime[i_end_point - 1] /
        (psi1[i_end_point - 1] * rgrid[i_end_point - 1] * log_amesh);
    log_grid_paw[i] =
        psi_prime[i_end_point - 1] /
        (psi[i_end_point - 1] * rgrid[i_end_point - 1] * log_amesh);
  }

  if (paw_debug()) {
    sprintf(data_filename, "%s%s_%s_scat_test.dat", paw_sdir(),
    ...
    sprintf(script_filename, "%s%s_%s_scat_test.plt", paw_sdir(),
            paw_get_atom_name(), paw_spd_Name(l));
    printf("script_filename: %s\n", script_filename);
    fp = fopen(script_filename, "w+");
