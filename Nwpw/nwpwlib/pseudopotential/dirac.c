...
void R_Dirac(int n, int l, int s2, double Z, const double *v, int *mch,
             double *Eig, double *u, double *uprime) {
  int i, j, iteration, node, match, Ninf, Ngrid;

  double E, de, kappa, Emax, Emin, log_amesh, log_amesh2, fss, gamma, L2, L0,
      r2, sum, a, scale, m1scale, uout, upout, upin, *r, *f_upp, *dv, *frp, *fr,
      *upp;
  ...
  while (iteration < Max_Iterations) {
    ...
    for (i = 0; i < Ngrid; ++i) {
      ...
      fr[i] =
          -log_amesh2 * r2 *
          (fss * (v[i] - E) * (v[i] - E) +
           0.5 * fss * dv[i] * kappa / (r[i] * (1.0 + 0.5 * fss * (E - v[i]))));
...
void R_Dirac_Fixed_E(int n, int l, int s2, double Z, const double *v, int match,
                     double E, double *u, double *uprime) {
  int i, j, node, Ngrid;

  double log_amesh, kappa, m1scale, log_amesh2, fss, gamma, L2, L0, r2, sum, *r,
      *f_upp, *dv, *frp, *fr, *upp;
  ...
  for (i = 0; i < Ngrid; ++i) {
    ...
    fr[i] =
        ...
        (fss * (v[i] - E) * (v[i] - E) +
         0.5 * fss * dv[i] * kappa / (r[i] * (1.0 + 0.5 * fss * (E - v[i]))));
    frp[i] =
        -log_amesh * r[i] * 0.5 * fss * dv[i] / (1.0 + 0.5 * fss * (E - v[i]));
  }
...
void R_Dirac_FixedLogDeriv(int n, int l, int s2, double Z, const double *v,
                           int match, double u_logderiv, double *Eig, double *u,
                           double *uprime) {
  ...
  fprintf(stderr, "Error R_Dirac_Fixed_LogDeriv: More than %d iterations\n",
          Max_Iterations);
  *Eig = E;
