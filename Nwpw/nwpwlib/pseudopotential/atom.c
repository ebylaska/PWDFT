  ...
  if (Solver_Type != Dirac && Solver_Type != ZORA) {
    ...
    for (i = 0; i < ncvh; ++i) {
      ...
      if (!lx) {
        ...
        fill[2 * i + 1] = (2.0 * lx + 2.0) * (fillx * 0.5 / (2. * lx + 1.));
      }
    }
...
void solve_Atom() 
{
  ...
  while ((!converged) && (it < Max_Iterations)) {
    ...
    for (i = 0; i < Ncv; ++i) {
      ...
      if (Solver_Type == Schrodinger) {
        R_Schrodinger(n[i], l[i], Vall, &turning_point[i], &Etmp, r_psi[i],
                      r_psi_prime[i]);
      } else if (Solver_Type == Pauli) {
        R_Pauli(n[i], l[i], Zion, Vall, &turning_point[i], &Etmp, r_psi[i],
                r_psi_prime[i]);
      } else if (Solver_Type == Dirac) {
        R_Dirac(n[i], l[i], s2[i], Zion, Vall, &turning_point[i], &Etmp,
                r_psi[i], r_psi_prime[i]);
      } else if (Solver_Type == ZORA) {
        R_ZORA(n[i], l[i], s2[i], Zion, Vall, &turning_point[i], &Etmp,
               r_psi[i], r_psi_prime[i]);
      }
      /**********************************************************/
  ...
  Total_E += E_Hartree - P_Hartree + E_exchange - P_exchange + E_correlation -
             P_correlation;
...
void solve_Scattering_State_Atom(int nt, int lt, double et, double rmax) {
  ...
  if (Solver_Type == Schrodinger) {
    R_Schrodinger_Fixed_E(nt, lt, Vall, turning_point[Ncv], et, r_psi[Ncv],
                          r_psi_prime[Ncv]);
  } else if (Solver_Type == Pauli) {
    R_Pauli_Fixed_E(nt, lt, Zion, Vall, turning_point[Ncv], et, r_psi[Ncv],
                    r_psi_prime[Ncv]);
  } else if (Solver_Type == Dirac) {
    s2[Ncv] = 1;
    s2[Ncv + 1] = -1;
    ...
    R_Dirac_Fixed_E(nt, lt, 1, Zion, Vall, turning_point[Ncv], et, r_psi[Ncv],
                    r_psi_prime[Ncv]);
    R_Dirac_Fixed_E(nt, lt, -1, Zion, Vall, turning_point[Ncv + 1], et,
    ...
    turning_point[Ncv] = turning_point[Ncv + 1] = rint(log(rmax / r0) / al);
    peak[Ncv] = peak[Ncv + 1] = rmax;
    R_ZORA_Fixed_E(nt, lt, 1, Zion, Vall, turning_point[Ncv], et, r_psi[Ncv],
                   r_psi_prime[Ncv]);
    R_ZORA_Fixed_E(nt, lt, -1, Zion, Vall, turning_point[Ncv + 1], et,
                   r_psi[Ncv + 1], r_psi_prime[Ncv + 1]);
  }
...
int solver_Type_Atom() { return Solver_Type; }

void print_Atom(FILE *fp) {
  int i, st;
...
double eigenvalue_Atom(int i) { return eigenvalue[i]; }

double *rho_Atom() { return rho; }

double *rho_core_Atom() { return rho_core; }

double *rho_valence_Atom() { return rho_valence; }

double *Vall_Atom() { return Vall; }

double *r_psi_Atom(int i) { return r_psi[i]; }

double *r_psi_prime_Atom(int i) { return r_psi_prime[i]; }

int n_Atom(int i) { return n[i]; }

int l_Atom(int i) { return l[i]; }

int s_Atom(int i) { return s2[i]; };

int lmax_Atom() { return lmax; }

double fill_Atom(int i) { return fill[i]; }

int Ncore_Atom() { return Ncore; }

int Nvalence_Atom() { return Nvalence; }

double peak_Atom(int i) { return peak[i]; }

int turning_point_Atom(int i) { return turning_point[i]; }

double Zion_Atom() { return Zion; }

double Amass_Atom() { return amass; }
...
int state_RelAtom(int nt, int lt, int st) {
  int i;
  i = 0;
...
char *name_Atom() { return atom_name; }
