...
//void hgh_parse(debug_ptr, lmax_ptr, locp_ptr, rlocal_ptr, sdir_name, n9,
//               dir_name, n0, in_filename, n1, out_filename, n2) 
void hgh_parse(int *debug_ptr, int *lmax_ptr, int *locp_ptr, double *rlocal_ptr,
               char sdir_name[], int *n9, char dir_name[], int *n0,
               char in_filename[], int *n1, char out_filename[], int *n2)
{
  ...
  if (!get_string(fp, atom))
    printf("NO ATOM NAME\n");
  if (!get_int(fp, &Zion))
    printf("NO ZION\n");

  if (!get_float(fp, &rloc))
    printf("NO rlocN\n");
  if (!get_float(fp, &C1))
    C1 = 0.0;
  if (!get_float(fp, &C2))
  ...
  if (!done) {
    if (!get_float(fp, &(r[i])))
      r[i] = 0.0;
    if (!get_float(fp, &(H[0][i])))
      H[0][i] = 0.0;
    if (!get_float(fp, &(H[1][i])))
      H[1][i] = 0.0;
    if (!get_float(fp, &(H[2][i])))
      H[2][i] = 0.0;
    argc = to_eoln(fp);
    done = get_end(fp);
    ++i;
    while (!done) {
      ...
      if (!done) {
        if (!get_float(fp, &(K[0][i])))
          K[0][i] = 0.0;
        if (!get_float(fp, &(K[1][i])))
          K[1][i] = 0.0;
        if (!get_float(fp, &(K[2][i])))
          K[2][i] = 0.0;
        argc = to_eoln(fp);
        done = get_end(fp);
      }
