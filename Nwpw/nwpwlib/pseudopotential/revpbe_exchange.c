/* revpbe_exchange.c - 6/9/95
   ...
   THis GGA was developed by Norskov
...
//void R_revPBE_Exchange(rho, Vx, Ex, Px)
void R_revPBE_Exchange(double rho[], double Vx[], double  *Ex, double *Px)
{
  int i;
  double mu, kappa;
  ...
  for (i = 0; i < Ngrid; ++i) {
    ...
    if (n > 1.0e-18) {
      ...
      ux_p = -(3.0 / 4.0) * n_onethird *
             (fourthird * F - v * Fs - (u - fourthird * s * s * s) * Fss);
