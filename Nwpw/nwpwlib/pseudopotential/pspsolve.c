...
int *n2;
{
  ...
  if (isRelativistic_Atom()) {
    ...
    rpspsolve(print_ptr, debug_ptr, lmax_ptr, locp_ptr, rlocal_ptr, sdir_name,
              n9, dir_name, n0, in_filename, n1, out_filename, n2);
    return;
  }
  init_Psp(infile);
  solve_Psp();
  ...
  if (NormConserving_Psp()) {
    for (p = 0; p < Nvalence; ++p) {
      ...
      if (fill_Psp(p) == 0.0) {
        ...
        rmax = 20.0;
        solve_Scattering_State_Atom(n_Psp(p), l_Psp(p), eigenvalue_Psp(p),
                                    rmax);
    ...
    if (print) {
      printf(
          "  + Appending pseudowavefunctions and aewavefunctions: %s thru %s\n",
  ...
  if (Vanderbilt_Psp()) {
    ...
    for (l = 0; l <= lmax_Psp(); ++l) {
      ...
      for (j = 0; j < ns_Psp(l); ++j)
        for (i = 0; i < ns_Psp(l); ++i)
          fprintf(fp, "%20.15le ", 0.5 * (D0_Psp(i, j, l) + D0_Psp(j, i, l)));
      fprintf(fp, "\n");

      /* output q    */
      for (j = 0; j < ns_Psp(l); ++j)
        for (i = 0; i < ns_Psp(l); ++i)
          fprintf(fp, "%20.15le ", 0.5 * (q_Psp(i, j, l) + q_Psp(j, i, l)));
      fprintf(fp, "\n");
  ...
  if (debug) {
    ...
    for (p = 0; p <= (Ncore_Atom() + Nvalence_Atom()); ++p) {
      sprintf(name, "%s.%1d%s", name_Atom(), n_Atom(p), spd_Name(l_Atom(p)));
      full_filename[0] = '\0';
      strncpy(full_filename, sdir_name, m9);
      full_filename[m9] = '\0';
      strcat(full_filename, "/");
