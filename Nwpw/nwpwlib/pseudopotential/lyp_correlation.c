...
void R_LYP_Correlation(double rho[], double Vc[], double *Ec, double *Pc)
{
  ...
  double n;
  double agr, agr2, lap;
  double rho_onethird, rho_twothirds, rho_fourthirds, rho_fivethirds,
      rho_seventhirds, rho_eightthirds, rho3;
  ...
  for (i = 0; i < Ngrid; ++i) {
    ...
    if (n > 1.2e-18) {
      ...
      ddF1 = (d / 3.0) * (2.0 * F1 * dF1 * rho_fourthirds -
                          (4.0 / 3.0) * F12 * rho_seventhirds);

      G1 = F1 * rho_fivethirds * e_c_rho;
      dG1 = dF1 * rho_fivethirds * e_c_rho -
            (5.0 / 3.0) * F1 * rho_eightthirds * e_c_rho +
            (c / 3.0) * F1 * rho3 * e_c_rho;
      ddG1A = ddF1 * rho_fivethirds * e_c_rho;
      ddG1B = dF1 * ((2.0 * c / 3.0) * rho3 * e_c_rho -
                     (10.0 / 3.0) * rho_eightthirds * e_c_rho);
      ddG1C = F1 * ((40.0 / 9.0) * rho_eleventhirds * e_c_rho -
                    (14.0 * c / 9.0) * rho4 * e_c_rho +
                    (c * c / 9.0) * rho_thirteenthirds * e_c_rho);
      ...
      T3 =
          -1.0 * (a * b / 4.0) *
          (ddG1 * n * agr2 + dG1 * (3.0 * agr2 + 2 * n * lap) + 4.0 * G1 * lap);
      T4 = -1.0 * (a * b / 72.0) *
           (3.0 * ddG1 * n * agr2 + dG1 * (5.0 * agr2 + 6.0 * n * lap) +
            4.0 * G1 * lap);
      ...
      //             ce = -1.0*(a*F1/n)*(n +
      //             b*rho_twothirds*(Cf*rho_fivethirds2-2.0*tw+(1.0/9.0)*tw+(1.0/18.0)*lap)*e_c_rho);
      tw = agr2 / (8.0 * n) - lap / 8.0;
      ce = (-1.0 * a / (1 + d * rho_onethird)) *
           (n + b * rho_twothirds *
                    (Cf * rho_fivethirds2 - 2.0 * tw + (1.0 / 9.0) * tw +
                     (1.0 / 18.0) * lap) *
                    e_c_rho);
