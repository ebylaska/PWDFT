...
static double *peak;
static double Zion;
static double Total_E, E_Hartree, P_Hartree, E_exchange, P_exchange,
...
void init_Psp(char *filename) {
  ...
  if (w != NIL) {
    w = get_word(fp);
    if (strcmp("hamann", w) == 0)
      Solver_Type = Hamann;
    if (strcmp("troullier-martins", w) == 0)
      Solver_Type = Troullier;
    if (strcmp("vanderbilt", w) == 0)
  ...
  if (Solver_Type == Troullier)
    strcpy(comment, "Troullier-Martins pseudopotential");
  if (Solver_Type == Vanderbilt)
    strcpy(comment, "Vanderbilt pseudopotential");
  fp = fopen(filename, "r+");
  w = get_word(fp);
  ...
  if (Solver_Type == Hamann)
    ...
    Suggested_Param_Vanderbilt(&Nvalence, n, l, eigenvalue, fill, rcut, &rlocal,
                               &clocal);
...
void solve_Psp() {
  ...
  if (Solver_Type == Hamann)
    solve_Hamann(Nvalence, n, l, eigenvalue, fill, rcut, r_psi, r_psi_prime,
                 rho, rho_semicore, V_psp, &Total_E, &E_Hartree, &P_Hartree,
                 &E_exchange, &P_exchange, &E_correlation, &P_correlation,
                 kb_expansion, r_psi_extra, r_psi_prime_extra);
  else if (Solver_Type == Troullier)
    solve_Troullier(Nvalence, n, l, eigenvalue, fill, rcut, r_psi, r_psi_prime,
                    rho, rho_semicore, V_psp, &Total_E, &E_Hartree, &P_Hartree,
                    &E_exchange, &P_exchange, &E_correlation, &P_correlation,
                    kb_expansion, r_psi_extra, r_psi_prime_extra);
  else if (Solver_Type == Vanderbilt)
    solve_Vanderbilt(Nvalence, n, l, eigenvalue, fill, rcut, rlocal, clocal, ns,
                     indx_il, indx_ijl, r_hard_psi, r_psi, r_psi_prime, rho,
                     rho_semicore, V_psp, Vlocal, D0, q, &Total_E, &E_Hartree,
                     &P_Hartree, &E_exchange, &P_exchange, &E_correlation,
                     &P_correlation);
...
double E_Psp() { return Total_E; } /*E_Atom*/

double eigenvalue_Psp(int i) { return eigenvalue[i]; }

double *rho_Psp() { return rho; }

double *rho_semicore_Psp() { return rho_semicore; }

double *drho_semicore_Psp() { return drho_semicore; }

double r_semicore_Psp() { return r_semicore; }

double *Beta_Psp(int i, int l) { return V_psp[indx_il[i][l]]; }

double *r_psi_il_Psp(int i, int l) { return r_psi[indx_il[i][l]]; }
double *r_hard_psi_il_Psp(int i, int l) { return r_hard_psi[indx_il[i][l]]; }

int ns_Psp(int l) { return ns[l]; }

double D0_Psp(int i, int j, int l) { return D0[indx_ijl[i][j][l]]; }

double q_Psp(int i, int j, int l) { return q[indx_ijl[i][j][l]]; }

double *Vlocal_Psp() { return Vlocal; }

double *V_Psp(int i) { return V_psp[i]; }

double *r_psi_Psp(int i) { return r_psi[i]; }

double *r_psi_extra_Psp(int i) { return r_psi_extra[i]; }

int n_Psp(int i) { return n[i]; }

int l_Psp(int i) { return l[i]; }

int lmax_Psp() { return lmax; }

double fill_Psp(int i) { return fill[i]; }

int Nvalence_Psp() { return Nvalence; }

int kb_extra_Psp() { return kb_extra; }

int kb_expansion_Psp(int i) { return kb_expansion[i]; }

double peak_Psp(int i) { return peak[i]; }

double rcut_Psp(int i) { return rcut[i]; }

double rcut_il_Psp(int i, int l) { return rcut[indx_il[i][l]]; }

double Zion_Psp() { return Zion; }
...
char *comment_Psp() { return comment; }
