...
//void qmmm_parse(debug_ptr, lmax_ptr, locp_ptr, rlocal_ptr, sdir_name, n9,
//                dir_name, n0, in_filename, n1, out_filename, n2, atom, n3) 
void qmmm_parse(int *debug_ptr, int *lmax_ptr, int *locp_ptr, double *rlocal_ptr,
                ...
                char in_filename[], int *n1, char out_filename[], int *n2, char atom[], int *n3)
{
  ...
  if (!get_string(fp, atom))
    printf("NO Atom name\n");
  if (!get_float(fp, &Zion))
    printf("NO Zion\n");
  if (!get_int(fp, &n_sigma))
    printf("NO n_sigma\n");
  if (!get_float(fp, &rc))
    printf("NO rc\n");
  fclose(fp);
  ...
  fp = fopen(outfile, "w+");
  fprintf(fp, "%s\n", atom_out);
  fprintf(fp, "%lf %lf %d   %d %d %lf\n", Zion, 0.0, lmax, lmax_out, locp_out,
          rlocal_out);
  fprintf(fp, "%lf\n", rc);
  fprintf(fp, "%d %lf\n", nrl, drl);
  fprintf(fp, "%s\n", comment);
