...
void solve_RelPsp() {
  ...
  if (Solver_Type == Troullier) {
    solve_RelTroullier(Nvalence, n, l, spin, eigenvalue, fill, rcut, r_psi,
                       r_psi_prime, rho, rho_semicore, V_psp, &Total_E,
                       &E_Hartree, &P_Hartree, &E_exchange, &P_exchange,
                       &E_correlation, &P_correlation);
  } else {
    solve_RelHamann(Nvalence, n, l, spin, eigenvalue, fill, rcut, r_psi,
                    r_psi_prime, rho, rho_semicore, V_psp, &Total_E, &E_Hartree,
...
void print_RelPsp(FILE *fp) {
  ...
  for (i = 0; i < Nvalence; ++i) {
    fprintf(fp, "%d\t%s\t%.1lf\t%.2lf\t\t%le\t%le\t%le\n", (l[i] + 1),
            spd_Name(l[i]), 0.5 * spin[i], fill[i], eigenvalue[i], rcut[i],
            peak[i]);
  }
  fprintf(fp, "----------------------------------------------------------------"
              "------------\n");
...
double E_RelPsp() { return Total_E; } /*E_Atom */

double eigenvalue_RelPsp(int i) { return eigenvalue[i]; }

double *rho_RelPsp() { return rho; }

double *rho_semicore_RelPsp() { return rho_semicore; }

double *drho_semicore_RelPsp() { return drho_semicore; }

double r_semicore_RelPsp() { return r_semicore; }

double *Beta_RelPsp(int i, int l) { return V_psp[indx_il[i][l]]; }

double *r_psi_il_RelPsp(int i, int l) { return r_psi[indx_il[i][l]]; }

double *r_hard_psi_il_RelPsp(int i, int l) { return r_hard_psi[indx_il[i][l]]; }

int ns_RelPsp(int l) { return ns[l]; }

double D0_RelPsp(int i, int j, int l) { return D0[indx_ijl[i][j][l]]; }

double q_RelPsp(int i, int j, int l) { return q[indx_ijl[i][j][l]]; }

double *Vlocal_RelPsp() { return Vlocal; }

double *V_RelPsp(int i) { return V_psp[i]; }

double *r_psi_RelPsp(int i) { return r_psi[i]; }

int n_RelPsp(int i) { return n[i]; }

int l_RelPsp(int i) { return l[i]; }

int lmax_RelPsp() { return lmax; }

double fill_RelPsp(int i) { return fill[i]; }

int Nvalence_RelPsp() { return Nvalence; }

double peak_RelPsp(int i) { return peak[i]; }

double rcut_RelPsp(int l) { return rcut[2 * l]; }

double rcut_il_RelPsp(int i, int l) { return rcut[indx_il[i][l]]; }

double Zion_RelPsp() { return Zion; }

int state_RelPsp(int nt, int lt, int st) {
  int i;
  i = 0;
...
char *comment_RelPsp() { return comment; }
