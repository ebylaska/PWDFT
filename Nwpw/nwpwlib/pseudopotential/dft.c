...
void init_DFT(char *filename) {
  ...
  if (w != NIL) {
    w = get_word(fp);
    if (strcmp("on", w) == 0)
      Hartree_Type = Hartree_On;
    if (strcmp("off", w) == 0)
      Hartree_Type = Hartree_Off;
  }
  fclose(fp);
  ...
  if (w != NIL) {
    w = get_word(fp);
    if (strcmp("dirac", w) == 0)
      Exchange_Type = Exchange_Dirac;
    if (strcmp("pbe96", w) == 0)
      Exchange_Type = Exchange_PBE96;
    if (strcmp("becke", w) == 0)
      Exchange_Type = Exchange_Becke;
    if (strcmp("revpbe", w) == 0)
      Exchange_Type = Exchange_revPBE;
    if (strcmp("off", w) == 0)
      Exchange_Type = Exchange_Off;
  }
  fclose(fp);
...
void set_Exchange_DFT(int exchange) { Exchange_Type = exchange; }

void set_Correlation_DFT(int correlation) { Correlation_Type = correlation; }

void set_Hartree_DFT(int hartree) { Hartree_Type = hartree; }
...
void R_Screening_Cut(double *Vx) {
  ...
  if (screening_cut > 0.0) {
    ...
    for (k = 0; k < n1; ++k)
      Vx[k] = m * r[k] + b;
  } else if ((blyp_screening_cut > 0.0) && (Exchange_Type == Exchange_Becke) &&
             (Correlation_Type == Correlation_LYP)) {
    r = r_LogGrid();
