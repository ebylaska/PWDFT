...
void R_ZORA(int n, int l, int s2, double Z, const double *v, int *mch,
  ...
  while (iteration < Max_Iterations) {
    ...
    for (i = 0; i < Ngrid; ++i) {
      ...
      fr[i] = -log_amesh2 * r2 *
              (fss * v[i] * (v[i] - E) +
               0.5 * fss * dv[i] * kappa / (r[i] * (1.0 - 0.5 * fss * v[i])));
      frp[i] =
          -log_amesh * r[i] * 0.5 * fss * dv[i] / (1.0 - 0.5 * fss * (v[i]));
    }
...
void R_ZORA_Fixed_E(int n, int l, int s2, double Z, const double *v, int match,
  ...
  for (i = 0; i < Ngrid; ++i) {
    ...
    fr[i] = -log_amesh2 * r2 *
            (fss * (v[i]) * (v[i] - E) +
             0.5 * fss * dv[i] * kappa / (r[i] * (1.0 - 0.5 * fss * (v[i]))));
    frp[i] = -log_amesh * r[i] * 0.5 * fss * dv[i] / (1.0 - 0.5 * fss * (v[i]));
  }
