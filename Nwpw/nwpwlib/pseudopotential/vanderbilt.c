                      ...
                      double *eh_psp, double *ph_psp, double *ex_psp, double *px_psp, double *ec_psp, double *pc_psp)
{
  ...
  double *ul, *ul_prime;
  double *wl, *wl_prime;
  double *r, *Vh, *Vx, *Vc, *rho_valence, *Vall, *Vl;
  double *chi;
  ...
  for (l = 0; l < (num_psp); ++l) {
    ...
    R_Schrodinger_Fixed_E(n_psp[l], l_psp[l], Vall, match_R, el, ul, ul_prime);
    ...
    for (it = 1; it <= MAXIT; ++it) {
      ...
      if (init_xpansion2(l_psp[l], match, el, Vall, ul, ul_prime, chi, wl,
                         wl_prime)) {
        psi_xpansion2();
        dpsi_xpansion2();
        chi_xpansion2();
      ...
      if (it == MAXIT)
        printf("%s\t%lf  %lf  %lf  %lf  %lf  %lf\n", spd_Name(l_psp[l]),
               rcut_psp[l], r[match], e_psp[l], el, fabs(ul[match] / wl[match]),
    ...
    for (i = 0; i < ns[l]; ++i) {
      ...
      for (j = 0; j < ns[l]; ++j) {
        ...
        D0[indx_ijl[i][j][l]] = B[indx_ijl[i][j][l]] +
                                e_psp[indx_il[j][l]] * q[indx_ijl[i][j][l]] -
                                D_shift;
      }
    }
  }
  ...
  for (k = 0; k < Ngrid; ++k)
    rho_psp[k] = (rho_valence[k]) * Vx[k];
  px = Integrate_LogGrid(rho_psp);
  for (k = 0; k < Ngrid; ++k)
    rho_psp[k] = (rho_valence[k]) * Vc[k];
