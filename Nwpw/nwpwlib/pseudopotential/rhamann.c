...
void Suggested_Param_RelHamann(int *num_states_psp, int n_psp[], int l_psp[],
...
*/
#define ALAM 3.5
void solve_RelHamann(int num_psp, int *n_psp, int *l_psp, int *s_psp,
                     double *e_psp, double *fill_psp, double *rcut_psp,
                     double **r_psi_psp, double **r_psi_prime_psp,
                     double *rho_psp, double *rho_semicore, double **V_psp,
  ...
  if (debug_print()) {
    ...
    printf("l\ts\trcore     rmatch    E in       E psp      norm test slope "
           "test\n");
  }
  for (p = 0; p < num_psp; ++p) {
    w2l = w1l = r_psi_psp[p];
    ...
    sx = sx * gamma * gamma;
    sf = sf * gamma * gamma;
    del = (-sx + (sx / fabs(sx)) * sqrt(sx * sx - sf * (gamma * gamma - 1.0))) /
          sf;
    ...
    for (k = 0; k < Ngrid; ++k) {
      if ((fabs(w2l[k]) > SMALL) || (r[k] < 0.1)) {
        /* hacking */
        V2l[k] += (gamma * del * pow(r[k], nu1) * Fcut[k] / (2.0 * w2l[k])) *
    ...
    if (debug_print()) {
      printf("%d\t%d/2\t%lf  %lf  %lf  %lf  %lf  %lf\n", l_psp[p], s_psp[p],
             rcut_psp[p], r[match], e_psp[p], e2l, gamma, gpr);
    }

    /* Extend scattering states to Ngrid */
    if (fill_psp[p] == 0.0) {
      R_Schrodinger_Fixed_E((l_psp[p] + 1), l_psp[p], V2l, (Ngrid - 1), e2l,
                            w2l, w2l_prime);
    }
