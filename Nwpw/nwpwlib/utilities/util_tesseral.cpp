...
std::complex<double> util_rSphericalHarmonic3(const int l, const int m, const double x, const double y, const double z)
{
   std::complex<double> rSphericalHarmonic3;
   std::complex<double> ctmp;
   int mod_m = abs(m);

   if (mod_m>l) std::cout << "Parameter out of order in function rSphericalHarmonic3" << std::endl;
   ...
   if (r>1.0e-9)
   {
      ...
      coeff = sqrt(coeff);
      rSphericalHarmonic3 = ctmp*coeff*util_rlegendre_lm(l,mod_m,cos_theta)*std::pow(r,l);
   }
   else
   {
      if (l==0) 
...
void util_Tesseral3_vector_lm(const int l, const int m, const int nfft3d, 
                              const double x[], const double y[], const double z[], double Tlm[])
{
   double coeff;
   double twopi = 8.00*atan(1.0);
   int mod_m = abs(m);

   if (mod_m> l)  std::cout << "Parameter out of order in function Tesseral3_vector_lm" << std::endl;
...
/*
   Calculates the tesseral harmonic for x,y,z such that
     ...
     where   cos_theta = z/r and phi = atan2(y,x)
*/
void util_Tesseral3_rgrid_lm(const int l, const int m, const int nfft3d, const double rgrid[], double Tlm[])
{
   double coeff;
...
/*
   ...
   where   cos_theta = z/r and phi = atan2(y,x)
*/
void util_Tesseral3_rgrid_lm_rl(const int l, const int m, const int nfft3d, const double rgrid[], double rTlm[])
{
   double coeff;
   double twopi = 8.0*atan(1.0);
   int mod_m = abs(m);
   ...
   for (auto k=0; k<nfft3d; ++k)
   {
      ...
      if (r>1.0e-9)
      {
         ...
         rTlm[k] = std::pow(r,l)*tmp2*coeff*util_rlegendre_lm(l,mod_m,cos_theta);
      }
      else
      {
         rTlm[k] = 0.0;
...
/*
  Calculates the 1/r**(l+1) times the tesseral harmonic for x,y,z such that
    ...
    where   cos_theta = z/r and phi = atan2(y,x)
*/
void util_Tesseral3_rgrid_lm_over_rl(const int l, const int m, const int nfft3d, const double rgrid[], double rTlm[])
{
   double coeff;
   double twopi = 8.0*atan(1.0);
   int mod_m = abs(m);
   ...
   for (auto k=0; k<nfft3d; ++k)
   {
      ...
      if (r>1.0e-9) 
      {
         ...
         rTlm[k] = tmp2*coeff*util_rlegendre_lm(l,mod_m,cos_theta)/std::pow(r,(l+1));
      }
      else
      {
         rTlm[k] = 0.0;
...
/*
  Calculates the tesseral harmonic for x,y,z such that
    ...
    where   cos_theta = z/r and phi = atan2(y,x)
*/
double util_Tesseral3_lm(const int l, const int m, const double x, const double y, const double z)
{
    int  mod_m = abs(m);
    double tmp2;
    double r = sqrt(x*x + y*y + z*z);
...
/*
  Calculates the tesseral harmonic divided by r**(l+1) for x,y,z such that
    ...
    where   cos_theta = z/r and phi = atan2(y,x)
*/
double util_Tesseral3_lm_over_rl(const int l, const int m, const double x, const double y, const double z)
{
   int mod_m = abs(m);
   double tmp2;
   double r = sqrt(x*x + y*y + z*z);
...
/*
  Calculates the derivative of tesseral harmonic for x,y,z wrt to x,y,z,such that
    ...
    where   cos_theta = z/r and phi = atan2(y,x)
*/
void util_dTesseral3_lm(const int l, const int m, const double x, const double y, const double z, double& dTx, double& dTy, double& dTz)
{
...
/*
  Calculates the derivative of tesseral harmonic for x,y,z wrt to x,y,z,such that
    ...
    where   cos_theta = z/r and phi = atan2(y,x)
*/
void util_dTesseral3_lm_over_rl(const int l, const int m, const double x, const double y, const double z, double& dTx, double& dTy, double& dTz)
{
   double coeff;
   double twopi = 8.0*atan(1.0);
   int mod_m = abs(m);

   if (mod_m>l) std::cout << "parameter out of order in function dTesseral3_lm_over_rl" << std::endl;
...
void dTesseral3_rgrid_lm_rl(const int l, const int m, const int nfft3d, const double rgrid[], double drTlm[])
{
   int mod_m = abs(m);
   double coeff;
   double twopi = 8.0*atan(1.0);

   if (mod_m>l) std::cout << "parameter out of order in function dTesseral3_rgrid_lm_rl" << std::endl;
   ...
   for (auto k=0; k<nfft3d; ++k)
   {
      ...
      if (r>1.0e-9)
      {
         ...
         drTlm[3*k]   = Tlm*x/r + rl*(f1*cos_theta*cos(phi) - f2*sin(phi));
         drTlm[3*k+1] = Tlm*y/r + rl*(f1*cos_theta*sin(phi) + f2*cos(phi));
         drTlm[3*k+2] = Tlm*z/r - rl*f1*sqrt(1.0-cos_theta*cos_theta);
      }
      else
      {
         drTlm[3*k]   = 0.0;
...
/*
  Calculates the tesseral harmonic for x,y,z such that
    ...
    where   cos_theta = z/r and phi = atan2(y,x)
*/
double util_Tesseral_lm(const int l,const int m, const double cos_theta, const double phi)
{
   double tmp2;
   int mod_m = abs(m);
...
/*
  Calculates the spherical harmonic for x,y,z such that
    ...
    where   cos_theta = z/r and phi = atan2(y,x)
*/
std::complex<double> util_YSpherical_lm(const int l, const int m, const double cos_theta, const double phi)
{
   return( util_ytheta_lm(l,m,cos_theta) * std::complex<double> (cos(m*phi),sin(m*phi)) );
}
...
/*
  Calculates the derivative of tesseral harmonic for x,y,z wrt to x,y,z,such that
    ...
    where   cos_theta = z/r and phi = atan2(y,x)
*/
void util_dTesseral_lm(const int l, const int m, const double cos_theta, const double phi, double& dTx, double& dTy, double& dTz)
{
   int mod_m = abs(m);
   double tmp2,f2;
...
/*
  Calculates the derivative of tesseral harmonic for x,y,z wrt to x,y,z,such that
    ...
    where   cos_theta = z/r and phi = atan2(y,x)
*/
void util_dYspherical_lm(const int l, const int m, const double cos_theta, const double phi, 
                         std::complex<double>& dYx, std::complex<double>& dYy, std::complex<double>& dYz)
{
   std::complex<double> f1,f2;

   // *** derivative with respect to theta ***
   f1 = util_dytheta_lm(l,m,cos_theta) * std::complex<double> (cos(m*phi),sin(m*phi));
   ...
   {
     f2 = util_ytheta_lm_div(l,m,cos_theta) * std::complex<double> (cos(m*phi),sin(m*phi)) * std::complex<double> (0.0,1.0*m);
   }
