...
double util_linesearch(const double t0, const double f0, const double df0,
  ...
  if (stoptype == 1)
    ...
    notfinished = (std::fabs(df[indx[0]] / df_first) > tolerance) &&
                  (iteration <= maxiter);
  ...
  while (notfinished) {
    ...
    if (secant) {
      deltat = -df[indx[0]] * (t[indx[1]] - t[indx[0]]) /
               (df[indx[1]] - df[indx[0]]);
