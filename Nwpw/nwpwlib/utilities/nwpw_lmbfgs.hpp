...
public:
  ...
  void lmbfgs(double *x, double *g, double *q) {
    ...
    std::memcpy(sylist + (2 * (indx[m + 1])) * nsize, x,
                nsize * sizeof(double));
    std::memcpy(sylist + (2 * (indx[m + 1]) + 1) * nsize, g,
                nsize * sizeof(double));

    DAXPY_PWDFT(nsize, mrone, x, one, &sylist[(2 * indx[m]) * nsize], one);
    DAXPY_PWDFT(nsize, mrone, g, one, &sylist[(2 * indx[m] + 1) * nsize], one);

    double sum = DDOT_PWDFT(nsize, &sylist[(2 * indx[m] + 1) * nsize], one,
                            &sylist[(2 * indx[m]) * nsize], one);
    if (std::fabs(sum) > 1.0e-11) {
      ...
      for (auto k = m - 1; k >= 0; --k) {
        alpha[k] =
            rho[indx[k]] *
            DDOT_PWDFT(nsize, &sylist[(2 * indx[k]) * nsize], one, q, one);
        double tscal = -alpha[k];
        DAXPY_PWDFT(nsize, tscal, &sylist[(2 * indx[k] + 1) * nsize], one, q,
                    one);
      }
      ...
      if (m > 200) {
        double sumsy =
            DDOT_PWDFT(nsize, &sylist[(2 * indx[m - 1]) * nsize], one,
                       &sylist[(2 * indx[m - 1] + 1) * nsize], one);
        double sumyy =
            DDOT_PWDFT(nsize, &sylist[(2 * indx[m - 1] + 1) * nsize], one,
                       &sylist[(2 * indx[m - 1] + 1) * nsize], one);
        if (sumyy > 1.0e-10) {
      ...
      for (auto k = 0; k < m; ++k) {
        double beta =
            rho[indx[k]] *
            DDOT_PWDFT(nsize, &sylist[(2 * indx[k] + 1) * nsize], one, q, one);
