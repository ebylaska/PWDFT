...
double util_legendre_lm(const int l, const int m, const double x) {
  ...
  // *** find P(mod_m+1,mod_m) ***
    ...
    if (l == (mod_m + 1))
      ...
      for (auto i = (mod_m + 2); i <= l; ++i) {
        tmp = (x * (2 * i - 1) * p_mp1m - (i + mod_m - 1) * p_mm) /
              ((double)(i - mod_m));
        p_mm = p_mp1m;
        p_mp1m = tmp;
      }
  ...
  // *** negative m - this routine is only call with negative m from dtheta_lm
  // and ddtheta_lm ***
  if (m < 0) {
    double coeff = 1.0;
...
double util_dlegendre_lm(const int l, const int m, const double x) {
  ...
  // *** check the arguments ***
  if ((m > l) || (std::abs(x) > 1.0))
    std::cout << "Parameter ot of range in dlegendre_lm" << std::endl;
...
double util_rlegendre_lm_div(const int l, const int m, const double x) {
  ...
  // *** check the arguments ***
  if ((m < 0) || (m > l) || (std::abs(x) > 1.0) || (m == 0))
    std::cout << "parameter ot of range in rlegendre_lm_div" << std::endl;
  ...
  if (m == l)
    ...
    if (l == (m + 1))
      ...
      for (auto i = (m + 2); i <= l; ++i) {
        tmp =
            (x * (2 * i - 1) * p_mp1m - (i + m - 1) * p_mm) / ((double)(i - m));
        p_mm = p_mp1m;
        p_mp1m = tmp;
      }
      f = tmp;
...
double util_legendre_lm_div(const int l, const int m, const double x) {
  ...
  // *** check the arguments ***
  if ((m < 0) || (m > l) || (std::abs(x) > 1.0) || (m == 0))
    std::cout << "parameter ot of range in legendre_lm_div" << std::endl;
...
double util_theta_lm(const int l, const int m, const double cos_theta) {
  ...
  if (m > l)
    std::cout << "parameter out of order in function theta_lm" << std::endl;
...
double util_rtheta_lm(const int l, const int m, const double cos_theta) {
  ...
  if (mod_m > l)
    std::cout << "parameter out of order in function rtheta_lm" << std::endl;
...
double util_ytheta_lm(const int l, const int m, const double cos_theta) {
  ...
  if (mod_m > l)
    std::cout << "parameter out of order in function ytheta_lm" << std::endl;
...
double util_theta_lm_div(const int l, const int m, const double cos_theta) {
  ...
  if (m > l)
    std::cout << "parameter out of order in function theta_lm_div" << std::endl;
...
double util_rtheta_lm_div(const int l, const int m, const double cos_theta) {
  ...
  if (mod_m > l)
    std::cout << "parameter out of order in function rtheta_lm_div"
              << std::endl;
...
double util_drtheta_lm(const int l, const int m, const double cos_theta) {
  ...
  if (mod_m > l)
    std::cout << "parameter out of order in function drtheta_lm" << std::endl;
  ...
  if (sin_theta <= 1.0e-9) {
    ...
    if (mod_m < l) {
      f = coeff * (mod_m * (cos_theta / sin_theta) *
                   ...
                   util_rlegendre_lm(l, mod_m + 1, cos_theta));
    } else {
      f = coeff * (mod_m * (cos_theta / sin_theta) *
                   util_rlegendre_lm(l, mod_m, cos_theta));
    }
...
/*
     Purpose : calculates drtheta_cos_theta_lm for a scalar cos_theta
               such that
                                                                             {cos(|m|*phi)
   m>0 dT_lm(cos_theta,phi)/d_cos_theta  = drtheta_cos_theta_lm(cos_theta)*{1
   m==0 {sin(|m|*phi)   m<0
*/
double util_drtheta_cos_theta_lm(const int l, const int m,
  ...
  if (sin_theta <= 1.0e-9) {
    ...
    if (mod_m < l) {
      f = -coeff * (mod_m * (cos_theta / (sin_theta * sin_theta)) *
                        util_rlegendre_lm(l, mod_m, cos_theta) -
                    util_rlegendre_lm(l, mod_m + 1, cos_theta));
    } else {
      f = -coeff * (mod_m * (cos_theta / (sin_theta * sin_theta)) *
...
double util_dytheta_lm(const int l, const int m, const double cos_theta) {
  ...
  if (m > l)
    std::cout << "Parameter out of order in function dytheta_lm" << std::endl;
