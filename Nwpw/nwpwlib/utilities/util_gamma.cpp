...
double util_gamma(const double x) { return (exp(util_ln_gamma(x))); }
...
double util_gcf(const double a, const double x) {
  ...
  while ((i <= ITMAX) && (!done)) {
    ...
    double del = d * c;
    h *= del;
    if (std::abs(del - 1.00) < EPS)
      done = true;
    ++i;
  }
  if (i > ITMAX)
    std::cout << "a too large, ITMAX too small in util_gcf" << std::endl;
...
double util_gammp(const double a, const double x) {
  if ((x < 0.0) || (a <= 0.0)) {
    std::cout << "bad arguments in util_gammp" << std::endl;
    return 0.0;
  }
