...
// #endif
   ...
   // BATCHED VERSIONS NOT WORKING (NUMERICAL INACCURACY)
...
void d3db::cr_fft3d_sycl(double *a)
{
    ...
    int i,j,k,jj,kk,indx,indx0,nxh,nxh2,nxhy,nxhy2,nxhz,nxhz2;
    nxh  = nx/2+1;
    nxhy = nxh*ny;
    nxhz = nxh*nz;
    nxh2  = nx+2;
	...
	for (int q=0; q<nq3; ++q)
	{
	    compute_backward(*desc_z, &a_dev[indx]);
	    indx += 2*nz;
	}
        //compute_backward(*desc_z, a_dev); // BATCHED VERSION NOT WORKING
	get_syclQue()->memcpy(a, a_dev, n2ft3d*sizeof(double));
	get_syclQue()->wait();
	...
	for (int q=0; q<nq2; ++q)
	{
	    compute_backward(*desc_y, &a_dev[indx]);
	    indx += (2*ny);
	}
        //compute_backward(*desc_y, a_dev); // BATCHED VERSION NOT WORKING
	get_syclQue()->memcpy(a, a_dev, n2ft3d*sizeof(double));
	get_syclQue()->wait();
	...
	for (int q=0; q<nq1; ++q)
	{
	    compute_backward(*desc_x, &a_dev[indx]);
	    indx += nxh2;
	}
        //compute_backward(*desc_x, a_dev); // BATCHED VERSION NOT WORKING
	get_syclQue()->memcpy(a, a_dev, n2ft3d*sizeof(double));
	get_syclQue()->wait();
...
void d3db::rc_fft3d_sycl(double *a)
{
	...
	for (q=0; q<nq1; ++q)
	{
	    compute_forward(*desc_x, &a_dev[indx]);
	    indx += nxh2;
	}
	get_syclQue()->memcpy(a, a_dev, n2ft3d*sizeof(double));
	get_syclQue()->wait();
	...
	for (q=0; q<nq3; ++q)
	{
	    compute_forward(*desc_z, &a_dev[indx]);
	    indx += 2*nz;
	}
	get_syclQue()->memcpy(a, a_dev, n2ft3d*sizeof(double));
	get_syclQue()->wait();
