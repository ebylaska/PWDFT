   ...
   : Mapping3(inmaptype,inparall->np_i(),inparall->taskid_i(),nx,ny,nz)
{
   ...
   if (maptype==1)
   {
      iq_to_i1 = new (std::nothrow) int*[1]; iq_to_i1[0] = new (std::nothrow) int[(nx/2+1)*ny*nq]();
      iq_to_i2 = new (std::nothrow) int*[1]; iq_to_i2[0] = new (std::nothrow) int[(nx/2+1)*ny*nq]();
      i1_start = new (std::nothrow) int*[1]; i1_start[0] = new (std::nothrow) int[nz+1]();
      i2_start = new (std::nothrow) int*[1]; i2_start[0] = new (std::nothrow) int[nz+1]();
      index1 = 0;
      index2 = 0;
      ...
      for (auto nb=0; nb<2; ++nb)
      {
         ...
         p_i2_start[nb] = new (std::nothrow) int*[1](); p_i2_start[nb][0] = new (std::nothrow) int[nz+1]();

         p_jq_to_i1[nb] = new (std::nothrow) int*[1](); p_jq_to_i1[nb][0] = new (std::nothrow) int[(nx/2+1)*ny*nq]();
         p_jq_to_i2[nb] = new (std::nothrow) int*[1](); p_jq_to_i2[nb][0] = new (std::nothrow) int[(nx/2+1)*ny*nq]();
         p_jz_to_i2[nb] = new (std::nothrow) int*[1](); p_jz_to_i2[nb][0] = new (std::nothrow) int[(nx/2+1)*ny*nq]();
         p_j1_start[nb] = new (std::nothrow) int*[1](); p_j1_start[nb][0] = new (std::nothrow) int[nz+1]();
         p_j2_start[nb] = new (std::nothrow) int*[1](); p_j2_start[nb][0] = new (std::nothrow) int[nz+1]();
      }
...
void d3db::rrrr_periodic_gradient(const double *rho, double *grx, double *gry, double *grz)
{
   int nx2 = nx+2;
   ...
   if (maptype==1)
   {
      ...
      for (auto q=0; q<nq; ++q)
      {
         ...
         for (auto i=0; i<nx; ++i)
         {
            ...
            for (auto j=0; j<ny; ++j)
               df[i+j*nx2] =  one_over_60*(  -1.0*f[im3+j*nx2]
                                          +   9.0*f[im2+j*nx2]
                                          -  45.0*f[im1+j*nx2]
                                          +  45.0*f[ip1+j*nx2]
                                          -   9.0*f[ip2+j*nx2]
      ...
      for (auto q=0; q<nq; ++q)
      {
         ...
         for (auto j=0; j<ny; ++j)
         {
            ...
            for (auto i=0; i<nx; ++i)
               df[i+j*nx2] =  one_over_60*(  -1.0*f[i+jm3*nx2]
                                          +   9.0*f[i+jm2*nx2]
                                          -  45.0*f[i+jm1*nx2]
                                          +  45.0*f[i+jp1*nx2]
                                          -   9.0*f[i+jp2*nx2]
      ...
      for (auto q=0; q<nq; ++q)
      {
         ...
         for (auto k=0; k<nz; ++k)
         {
            ...
            for (auto i=0; i<nx; ++i)
               df[i+k*nx2] =  one_over_60*(  -1.0*f[i+km3*nx2]
                                          +   9.0*f[i+km2*nx2]
                                          -  45.0*f[i+km1*nx2]
                                          +  45.0*f[i+kp1*nx2]
                                          -   9.0*f[i+kp2*nx2]
...
void d3db::rrrr_periodic_laplacian(const double *rho, double *grxx, double *gryy, double *grzz)
{
   int nx2 = nx+2;
   ...
   if (maptype==1)
   {
      ...
      for (auto q=0; q<nq; ++q)
      {
         ...
         for (auto i=0; i<nx; ++i)
         {
            ...
            for (auto j=0; j<ny; ++j)
               ddf[i+j*nx2] =  one_over_180*(   2.0*f[im3+j*nx2]
                                            -  27.0*f[im2+j*nx2]
                                            + 270.0*f[im1+j*nx2]
                                            - 490.0*f[i  +j*nx2]
                                            + 270.0*f[ip1+j*nx2]
      ...
      for (auto q=0; q<nq; ++q)
      {
         ...
         for (auto j=0; j<ny; ++j)
         {
            ...
            jp3 = j+3; if (jp3>=ny) jp3 -= ny;
            for (auto i=0; i<nx; ++i)
               ddf[i+j*nx2] =  one_over_180*(   2.0*f[i+jm3*nx2]
                                            -  27.0*f[i+jm2*nx2]
                                            + 270.0*f[i+jm1*nx2]
                                            - 490.0*f[i+  j*nx2]
                                            + 270.0*f[i+jp1*nx2]
      ...
      for (auto q=0; q<nq; ++q)
      {
         ...
         for (auto k=0; k<nz; ++k)
         {
            ...
            kp3 = k+3; if (kp3>=nz) kp3 -= nz;
            for (auto i=0; i<nx; ++i)
               ddf[i+k*nx2] =  one_over_180*(   2.0*f[i+km3*nx2]
                                            -  27.0*f[i+km2*nx2]
                                            + 270.0*f[i+km1*nx2]
                                            - 490.0*f[i+  k*nx2]
                                            + 270.0*f[i+kp1*nx2]
