 ...
 *      Author: bylaska
 *
 *      Implements the H-Curve 2d mapping by Niedermeier, Reinhardt and Sanders.
 * This program is based on the Java applet of Reinhard.
 *      (http://www2-fs.informatik.uni-tuebingen.de/~reinhard/hcurve.html)
 *
 *      R. Niedermeier , K. Reinhardt and P. Sanders . Towards optimal locality
 * in mesh-indexings. Proceedings of the 11th International Symposium on
 * Fundamentals of Computation Theory, number 1279 in LNCS, pages 364--375,
 * Krakow, Poland, September 1997. Springer.
...
int xcor(Rect_Type *r, int i) {
  ...
  if (r->x == 1) {
    ...
    if (r->y == 1) {
      ...
      if (2 * r->lo->y == r->y) {
        if (i < r->lo->g - r->lo->h) {
          return r->lo->x - 1 - xcor(r->lo, i + r->lo->h);
        } else {
          if (i < r->lu->g + r->lo->g - r->lo->h) {
            return r->lu->x - 1 -
                   xcor(r->lu, r->lu->g + r->lo->g - r->lo->h - i - 1);
          } else {
            ...
            if (i < r->lu->g + r->lo->h) {
              ...
              if (i < r->lu->g + r->ru->g + r->lo->h) {
                ...
                if (i < r->g - r->lo->g + r->lo->h) {
                  return r->lo->x +
                         xcor(r->ro, r->g - r->lo->g + r->lo->h - i - 1);
                } else {
...
int ycor(Rect_Type *r, int i) {
  if (r->x == 1) {
    ...
    if (r->y == 1) {
      ...
      if (2 * r->lo->y == r->y) {
        if (i < r->lo->g - r->lo->h) {
          return r->lo->y - 1 - ycor(r->lo, i + r->lo->h);
        } else {
          if (i < r->lu->g + r->lo->g - r->lo->h) {
            return r->lo->y +
                   ycor(r->lu, r->lu->g + r->lo->g - r->lo->h - i - 1);
          } else {
            if (i < r->ru->g + r->lu->g + r->lo->g - r->lo->h) {
          ...
          if (i < r->lo->h) {
            ...
            if (i < r->lu->g + r->lo->h) {
              ...
              if (i < r->lu->g + r->ru->g + r->lo->h) {
                ...
                if (i < r->g - r->lo->g + r->lo->h) {
                  return r->ro->y - 1 -
                         ycor(r->ro, r->g - r->lo->g + r->lo->h - i - 1);
                } else {
...
void hcurve2d_map(const int sizex, const int sizey, int map[]) {
  int i, ii, jj;
  Rect_Type *r;
  ...
  bfel = new int[2 * nx1];
  bfeld = new int[2 * nx1 * ny1];
  for (i = 0; i < 2 * nx1; ++i)
    bfel[i] = 0;
  for (i = 0; i < (2 * nx1 * ny1); ++i)
    bfeld[i] = 0;

  fel = new Rect_Type[2 * nx1];
  feld = new Rect_Type[2 * nx1 * ny1];
