...
static size_t hdbm_fread(void *pointer, size_t size, size_t num_items,
                         FILE *stream) {
  io_stats.reads++;
...
static size_t hdbm_fwrite(const void *pointer, size_t size, size_t num_items,
                          FILE *stream) {
  io_stats.writes++;
...
static int hdbm_fseek(FILE *stream, long int offset, int whence) {
  int result;
...
/*
  ...
  */
{
  return (db >= 0 && db < MAXHDBM && hash_tables[db].active);
}
  ...
  */
{
  ...
  r->dptr = 0;
  r->dsize = dsize; /* Note that datum can have non-zero
  ...
  */
{
  ...
  int i;
  unsigned char *ptr = (unsigned char *)d.dptr;
  ...
  while (!hdbm_fseek(file, rec_ptr, SEEK_SET) &&
    ...
    if (fe.active) {
      if (!datum_fread(file, rec_ptr + sizeof(fe), fe.key_size, &key)) {
        (void)fprintf(stderr, "hdbm_load: header present but key missing?\n");
        return 0;
      }

      value.dptr = 0;
      value.dsize = fe.val_size;
  ...
  */
{
  ...
  *db = -1; /* Invalid value */
  ...
  if (use_file) {
    ...
    if (!(hash_tables[i].ffl = (fflentry *)hdbm_malloc(
    ...
    if (ftell(hash_tables[i].file) > 0) {
      ...
      if (!hdbm_load(i, hash_tables[i].file)) {
        ...
        (void)fprintf(stderr, "hdbm_open: failed to load file %s\n", name);
        return 0;
      }
    } else {
      ...
      if (hdbm_fwrite(cookie, sizeof(cookie), (size_t)1, hash_tables[i].file) !=
          1) {
        fprintf(stderr, "hdbm_open: failed to write cookie to %s\n", name);
        return 0;
      }
    }
  } else {
  ...
  */
{
  ...
  for (available = hdbm_first_key(db, &key); available;
  ...
  */
{
  ...
  char *tmp = (char *)hdbm_malloc((size_t)l);
  ...
  if (!hdbm_file_copy(filename, tmp)) {
    (void)fprintf(stderr, "hdbm_file_compress: copy to %s failed\n", tmp);
    return 0;
  }

  if (!hdbm_file_copy(tmp, filename)) {
    (void)fprintf(stderr, "hdbm_file_compress: copy from %s failed\n", tmp);
    (void)fprintf(stderr, "hdbm_file_compress: copy manually to recover\n");
    return 0;
  }
...
{
  if (!hdbm_check(db)) {
    (void)fprintf(stderr, "hdbm_close: invalid db handle passed %d\n", db);
    return 0;
  }
  ...
  */
{
  ...
  file_entry fe;
  long act_ptr = e->rec_ptr + (long)(((char *)&fe.active) - ((char *)&fe));
  int ind;
  int flse;
  ...
  */
{
  ...
  if (hdbm_fwrite((char *)e->key.dptr, (size_t)1, (size_t)e->key.dsize, file) !=
      e->key.dsize) {
  ...
  if (hdbm_fwrite((char *)e->value.dptr, (size_t)1, (size_t)e->value.dsize,
                  file) != e->value.dsize) {
...
static void search(entry **table, datum key, int *p_index, entry **p_cur,
                   entry **p_prev)
/*
  ...
  */
{
  ...
  if (cur) {
    if (hash_tables[db].file)
      return datum_fread(hash_tables[db].file, cur->val_ptr, cur->value.dsize,
                         value);
    else
  ...
  */
{
  ...
  for (available = hdbm_first_key(dbfrom, &key); available;
  ...
  */
{
  ...
  for (cur_bin = 0; (hash_tables[db].nentry[cur_bin] == 0) && (cur_bin < NBINS);
       cur_bin++)
    ;
...
int hdbm_print_table(hdbm db, void (*print_key)(datum),
                     void (*print_value)(datum))
/*
  ...
  */
{
  ...
  if (!print_key)
    print_key = datum_print_hex;
  if (!print_value)
    print_value = datum_print_hex;
