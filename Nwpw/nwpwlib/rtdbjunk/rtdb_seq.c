...
static int ma_nametype(const char *maname) {
  ...
  if (strcmp(maname, "char") == 0)
    ma_type = rtdb_char;
  if (strcmp(maname, "int") == 0)
    ma_type = rtdb_int;
  if (strcmp(maname, "long") == 0)
    ma_type = rtdb_long;
  if (strcmp(maname, "float") == 0)
    ma_type = rtdb_float;
  if (strcmp(maname, "double") == 0)
    ma_type = rtdb_double;
  return ma_type;
}
...
*/
{
  ...
  for (status = hdbm_first_key(db, &key); status;
...
*/
{
  ...
  if (strlen(name) + sizeof(info_header) + 1 > sizeof(info_buf)) {
    (void)fprintf(stderr, "rtdb_delete: info entry buffer needs to be %d\n",
...
static int rtdb_seq_put_info(const int handle, const char *name,
                             const int ma_type, const int nelem)
/*
...
*/
{
  ...
  if (!status) {
    (void)fprintf(stderr, "rtdb_seq_put_info: put failed for \"%s\" in %s\n",
...
int rtdb_seq_get_info(const int handle, const char *name, int *ma_type,
...
*/
{
  ...
  if (!rtdb_seq_get_info(handle, name, &ma_type_from_db, &nelem_from_db,
                         date)) {
  ...
  if (nelem_actual != nelem_from_db) {
    (void)fprintf(stderr,
                  ...
                  name, rtdb[handle].filename, nelem_from_db, nelem_actual);
    return 0;
  }
...
*/
{
  ...
  if (!hdbm_file_copy(rtdb[handle].filename, suffix)) {
    (void)fprintf(stderr, "rtdb_seq_copy: copy from %s failed\n", suffix);
    return 0;
  }
  return 1;
}
int rtdb_seq_getfname(const int handle, char fname[36])
/*
  Get rtdb file name
  handle   = handle to RTDB
