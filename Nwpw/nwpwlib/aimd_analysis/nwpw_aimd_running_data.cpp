...
#define xyzstream(S,X,Y,Z,VX,VY,VZ)	std::left << std::setw(3) << (S) << E124 << (X) << E124 << (Y) << E124 << (Z) << E124 << (VX) << E124 << (VY) << E124 << (VZ)

#define hionstream(t,N,V,a1x,a1y,a1z,a2x,a2y,a2z,a3x,a3y,a3z)	E156 << (t) << std::setw(6) << (N) << E156 << (V) << E156 << (a1x) << E156 << (a1y) << E156 << (a1z) << E156 << (a2x) << E156 << (a2y) << E156 << (a2z) << E156 << (a3x) << E156 << (a3y) << E156 << (a3z) 

#define ionstream(II,S1,S2,X,Y,Z,VX,VY,VZ)	std::setw(6) << (II) << std::setw(3) << (S1) << std::setw(5) << (S2) << E156 << (X) << E156 << (Y) << E156 << (Z) << E156 << (VX) << E156 << (VY) << E156 << (VZ) 
                                               ...
                                               double *Ein, double *hmlin, double *psiin, double *dnin)
{
   ...
   if (myparall->is_master())
   {
      ...
      if (fs::exists(emotion_filename))
      {
         ...
         while(std::getline(emotiontmp, line))
         {
            ...
            if (!(iss >> emotion_time_shift >> w >> a >> b >> c >> d)) { break; } // error
   ...
   if (calculate_mulliken && (myparall->is_master()))
   {
      ...
      eigmotion->open(const_cast<char*> (eigmotion_filename.data()), std::ios::app);
...
void nwpw_aimd_running_data::update_iteration(const int icount)
{
   ...
   if (xyz_open)  
   {
      ...
      *xyz << myion->nion << std::endl << hxyzstream(a1x,a1y,a1z,a2x,a2y,a2z,a3x,a3y,a3z) << std::endl;

      for (auto ii=0; ii<myion->nion; ++ii)
         *xyz << xyzstream(myion->symbol(ii), myion->rion(0,ii)*AACONV, myion->rion(1,ii)*AACONV, myion->rion(2,ii)*AACONV, 
                                              myion->vion(0,ii)*AACONV, myion->vion(1,ii)*AACONV, myion->vion(2,ii)*AACONV) << std::endl;
   }
   ...
   if (ion_motion_open) 
   {
      ...
      *ion_motion << hionstream(current_time,myion->nion,omega,a1x,a1y,a1z,a2x,a2y,a2z,a3x,a3y,a3z) << std::endl;

      for (auto ii=0; ii<myion->nion; ++ii)
         *ion_motion << ionstream(ii,myion->symbol(ii), myion->symbol(ii), 
                                     myion->rion(0,ii), myion->rion(1,ii), myion->rion(2,ii), 
                                     myion->vion(0,ii), myion->vion(1,ii), myion->vion(2,ii)) << std::endl;
   }
   ...
   if (emotion_open) 
   {
      ...
      if (use_nose_output)
         *emotion << E1910 << current_time
                  ...
                  << E1910 << qave
                  << E1910 << qvar
                  << E1910 << myion->Temperature() 
                  << E1910 << pressure << std::endl;
      else
         *emotion << E1910 << current_time
                  << E1910 << E[0]
                  << E1910 << E[1]
