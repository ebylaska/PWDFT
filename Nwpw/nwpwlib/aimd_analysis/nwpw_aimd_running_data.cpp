 ...
 *                                                  *
 ****************************************************/
nwpw_aimd_running_data::nwpw_aimd_running_data(Control2 &control,
                                               Parallel *inparall, Pneb *inpneb,
                                               Ion *inion, double *Ein,
                                               double *hmlin, double *psiin,
                                               double *dnin) {
  myparall = inparall;
  myion = inion;
  mypneb = inpneb;
  ...
  ion_motion_filename =
      control.permanent_dir_str + "/" + control.ion_motion_filename;
  ...
  dipole_motion_filename =
      control.permanent_dir_str + "/" + control.dipole_motion_filename;
  eigmotion_filename =
      control.permanent_dir_str + "/" + control.eigmotion_filename;
  ...
  if (myparall->is_master()) {
    ...
    if (fs::exists(xyz_bakfile))
      fs::copy(xyz_bakfile, xyz_filename);
    if (fs::exists(motion_bakfile))
      fs::copy(motion_bakfile, ion_motion_filename);
    if (fs::exists(emotion_bakfile))
      fs::copy(emotion_bakfile, emotion_filename);

    // make new backup files
    if (fs::exists(xyz_filename))
      fs::copy(xyz_filename, xyz_bakfile);
    if (fs::exists(ion_motion_filename))
      fs::copy(ion_motion_filename, motion_bakfile);
    if (fs::exists(emotion_filename))
      fs::copy(emotion_filename, emotion_bakfile);
    ...
    ion_motion->open(const_cast<char *>(ion_motion_filename.data()),
                     std::ios::app);
  ...
  if (calculate_fei && (myparall->is_master())) {
    ...
    if (fs::exists(fei_bakfile))
      fs::copy(fei_bakfile, fei_filename);

    // make a new backup file
    if (fs::exists(fei_filename))
      fs::copy(fei_filename, fei_bakfile);
  ...
  if (calculate_cif && (myparall->is_master())) {
    // check for a backup file
    if (fs::exists(cif_bakfile))
      fs::copy(cif_bakfile, cif_filename);

    // make a new backup file
    if (fs::exists(cif_filename))
      fs::copy(cif_filename, cif_bakfile);
  ...
  if (calculate_dipole && (myparall->is_master())) {
    // check for a backup file
    if (fs::exists(dipole_bakfile))
      fs::copy(dipole_bakfile, dipole_motion_filename);

    // make a new backup file
    if (fs::exists(dipole_motion_filename))
      fs::copy(dipole_motion_filename, dipole_bakfile);

    // dipole_motion file
    dipole_motion_open = true;
    dipole_motion = new (std::nothrow) std::ofstream;
    dipole_motion->open(const_cast<char *>(dipole_motion_filename.data()),
                        std::ios::app);
  }
  ...
  if (calculate_mulliken && (myparall->is_master())) {
    // check for backup files
    if (fs::exists(eigmotion_bakfile))
      fs::copy(eigmotion_bakfile, eigmotion_filename);
    if (fs::exists(omotion_bakfile))
      fs::copy(omotion_bakfile, omotion_filename);
    if (fs::exists(hmotion_bakfile))
      fs::copy(hmotion_bakfile, hmotion_filename);

    // make new backup files
    if (fs::exists(eigmotion_filename))
      fs::copy(eigmotion_filename, eigmotion_bakfile);
    if (fs::exists(omotion_filename))
      fs::copy(omotion_filename, omotion_bakfile);
    if (fs::exists(hmotion_filename))
      fs::copy(hmotion_filename, hmotion_bakfile);
...
void nwpw_aimd_running_data::update_iteration(const int icount) {
  ...
  if (xyz_open) {
    ...
    *xyz << myion->nion << std::endl
    ...
    for (auto ii = 0; ii < myion->nion; ++ii)
      *xyz << xyzstream(
                  myion->symbol(ii), myion->rion(0, ii) * AACONV,
                  myion->rion(1, ii) * AACONV, myion->rion(2, ii) * AACONV,
                  myion->vion(0, ii) * AACONV, myion->vion(1, ii) * AACONV,
                  myion->vion(2, ii) * AACONV)
           << std::endl;
  }
  ...
  if (ion_motion_open) {
    ...
    *ion_motion << hionstream(current_time, myion->nion, omega, a1x, a1y, a1z,
                              a2x, a2y, a2z, a3x, a3y, a3z)
                << std::endl;

    for (auto ii = 0; ii < myion->nion; ++ii)
      *ion_motion << ionstream(ii, myion->symbol(ii), myion->symbol(ii),
                               myion->rion(0, ii), myion->rion(1, ii),
                               myion->rion(2, ii), myion->vion(0, ii),
                               myion->vion(1, ii), myion->vion(2, ii))
                  << std::endl;
  }
...
void nwpw_aimd_running_data::remove_bakfiles() {
  if (xyz_open)
    if (fs::exists(xyz_bakfile))
      fs::remove(xyz_bakfile);
  if (ion_motion_open)
    if (fs::exists(motion_bakfile))
      fs::remove(motion_bakfile);
  if (emotion_open)
    if (fs::exists(emotion_bakfile))
      fs::remove(emotion_bakfile);

  if (fei_open)
    if (fs::exists(fei_bakfile))
      fs::remove(fei_bakfile);
  if (cif_open)
    if (fs::exists(cif_bakfile))
      fs::remove(cif_bakfile);

  if (dipole_motion_open)
    if (fs::exists(dipole_bakfile))
      fs::remove(dipole_bakfile);

  if (eigmotion_open)
    if (fs::exists(eigmotion_bakfile))
      fs::remove(eigmotion_bakfile);
  if (omotion_open)
    if (fs::exists(omotion_bakfile))
      fs::remove(omotion_bakfile);
  if (hmotion_open)
    if (fs::exists(hmotion_bakfile))
      fs::remove(hmotion_bakfile);
}
