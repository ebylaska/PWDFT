                                               ...
                                               double *Ein, double *hmlin, double *psiin, double *dnin) 
{
   ...
   cif_shift_cell     = control.CIF_shift_cell();
...
void nwpw_aimd_running_data::update_iteration(const int icount) 
{
   ...
   if (ion_motion_open) 
   {
      ...
      *ion_motion << hionstream(current_time,myion->nion,omega,
      ...
      for (auto ii=0; ii<myion->nion; ++ii)
        *ion_motion << ionstream(ii+1,myion->symbol(ii),myion->symbol(ii),
   ...
   if (cif_open) { 
      ...
      double bz1 = (a2x*a3y - a2y*a3x)/omega;

      double bx2 = (a3y*a1z - a3z*a1y)/omega;
      double by2 = (a3z*a1x - a3x*a1z)/omega;
      double bz2 = (a3x*a1y - a3y*a1x)/omega;

      double bx3 = (a1y*a2z - a1z*a2y)/omega;
      double by3 = (a1z*a2x - a1x*a2z)/omega;
      double bz3 = (a1x*a2y - a1y*a2x)/omega;

      // determine a,b,c,alpha,beta,gmma
      double pi = 4.0*std::atan(1.0);
      double aa = std::sqrt(a1x*a1x + a1y*a1y +a1z*a1z);
      double bb = std::sqrt(a2x*a2x + a2y*a2y +a2z*a2z);
      double cc = std::sqrt(a3x*a3x + a3y*a3y +a3z*a3z);

      double da = (a2x-a3x)*(a2x-a3x)
                + (a2y-a3y)*(a2y-a3y)
                + (a2z-a3z)*(a2z-a3z);
      double alpha = (bb*bb + cc*cc - da)/(2.0*bb*cc);
      alpha = std::acos(alpha)*180.0/pi;

      double db = (a3x-a1x)*(a3x-a1x)
                + (a3y-a1y)*(a3y-a1y)
                + (a3z-a1z)*(a3z-a1z);
      double beta = (cc*cc + aa*aa - db)/(2.0*cc*aa);
      beta = std::acos(beta)*180.0/pi;

      double dg = (a1x-a2x)*(a1x-a2x)
                + (a1y-a2y)*(a1y-a2y)
                + (a1z-a2z)*(a1z-a2z);
      double gmma = (aa*aa + bb*bb - dg)/(2.0*aa*bb);
      gmma = std::acos(gmma)*180.0/pi;

      *cif << "data_nwchemex_pwdft" << std::endl << std::endl;
      *cif << "_audit_creation_date   " << util_date() << std::endl;
      *cif << "_audit_creation_method    generated by pwdft module of NWChemEx" << std::endl;
      *cif<< std::endl << std::endl;
      *cif << "_cell_length_a   " << Ffmt(16,4) << aa * 0.529177 << std::endl;
      *cif << "_cell_length_b   " << Ffmt(16,4) << bb * 0.529177 << std::endl;
      *cif << "_cell_length_c   " << Ffmt(16,4) << cc * 0.529177 << std::endl;
      *cif << "_cell_angle_alpha" << Ffmt(16,4) << alpha << std::endl;
      *cif << "_cell_angle_beta " << Ffmt(16,4) << beta  << std::endl;
      *cif << "_cell_angle_gamma" << Ffmt(16,4) << gmma  << std::endl;
      *cif << std::endl;
      *cif << "_symmetry_space_group_name_H-M     P1  " << std::endl;
      *cif << std::endl;
      *cif << "loop_" << std::endl;
      *cif << "_atom_site_type_symbol" << std::endl;
      //stream << 1242 FORMAT('_atom_site_label')
      *cif << "_atom_site_fract_x" << std::endl;
      *cif << "_atom_site_fract_y" << std::endl;
      *cif << "_atom_site_fract_z" << std::endl;

      for (auto ii=0; ii<myion->nion; ++ii)
      {
         std::string sym1 = myion->symbol(ii);
         if (sym1.length() == 1) sym1 += " ";
         sym1 += "      ";
         double xx = myion->rion(0,ii);
         double yy = myion->rion(1,ii);
         double zz = myion->rion(2,ii);
         double f1 = xx*bx1 + yy*by1 + zz*bz1 + shift;
         double f2 = xx*bx2 + yy*by2 + zz*bz2 + shift;
         double f3 = xx*bx3 + yy*by3 + zz*bz3 + shift;

         *cif << sym1 << std::fixed << std::setprecision(6)
              << std::setw(14) << f1
              << std::setw(14) << f2
              << std::setw(14) << f3
              << std::endl;
