...
void balance_init_a(Parallel *parall, const int nwavein, const int np,
  ...
  int i, j, nwave, ave, below, above, ishort, ilong, done, npacket, receiver,
      sender;
...
Balance::Balance(Parallel *inparall, const int maxsize0, const int *nidb,
                 int *nidb_out) {
  parall = inparall;
  maxsize = maxsize0;
 ...
 *                                 *
 ***********************************/
void Balance::c_balance_start(const int nb, double *a, const int request_indx,
                              const int msgtype) {
  int j, pto, pfrom, msglen, indx;
