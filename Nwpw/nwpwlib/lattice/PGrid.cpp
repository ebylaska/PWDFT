...
void PGrid::cr_pfft3b(const int nb, double *a)
{
   ...
   else
   {
       ...
       ************************************************/
       gdevice_batch_cfftz_tmpz_zero(false,nz,nq3,n2ft3d,a,d3db::tmpz,zero_row3[nb]);
       /*
       ...
       ************************************************/
       gdevice_batch_cffty_tmpy_zero(false,ny,nq2,n2ft3d,a,d3db::tmpy,zero_row2[nb]);
       /*
#if (defined NWPW_SYCL) || (defined NWPW_CUDA)
...
void PGrid::rc_pfft3f(const int nb, double *a)
{
   ...
   else
   {
       ...
       ********************************************/
       gdevice_batch_cffty_tmpy_zero(true,ny,nq2,n2ft3d,a,d3db::tmpy,zero_row2[nb]);
       /*
#if (defined NWPW_SYCL) || (defined NWPW_CUDA)
       ...
       ********************************************/
       gdevice_batch_cfftz_tmpz_zero(true,nz,nq3,n2ft3d,a,d3db::tmpz,zero_row3[nb]);
       /*
#if (defined NWPW_SYCL) || (defined NWPW_CUDA)
 ...
 *                              *
 ********************************/
void PGrid::c_unpack_start(const int nb, double *tmp1, double *tmp2, const int request_indx, const int msgtype)
{
   if (balanced)
 ...
 *                              *
 ********************************/
void PGrid::c_unpack_mid(const int nb, double *tmp1, double *tmp2, const int request_indx, const int msgtype)
{
   if (balanced)
 ...
 *                              *
 ********************************/
void PGrid::c_unpack_end(const int nb, double *tmp1, double *tmp2, const int request_indx)
{
...
void PGrid::pfftbz(const int nb, double *tmp1, double *tmp2, int request_indx)
{
   ...
   else
   {
       ...
       ***   A(nz,kx,ky) <- fft1d^(-1)[A(kz,kx,ky)] ***
       ************************************************/
       gdevice_batch_cfftz_tmpz_zero(false,nz,nq3,n2ft3d,tmp1,d3db::tmpz,zero_row3[nb]);
 ...
 *                              *
 ********************************/
void PGrid::pfftb_step(const int step, const int nb, double *a, double *tmp1, double *tmp2, const int request_indx)
{
    if (step==0)
    {
...
void PGrid::pfftfx(const int nb, double *tmp1, double *tmp2, int request_indx)
{
   ...
   else
   {
      // do fft along nx dimension    
      // A(kx,ny,nz) <- fft1d[A(nx,ny,nz)] 
       gdevice_batch_cfftx_tmpx(true,nx,nq1,n2ft3d,tmp1,d3db::tmpx);

       d3db::c_ptranspose_ijk_start(nb,0,tmp1,tmp2,tmp1,request_indx,40);
...
void PGrid::pfftfy(const int nb, double *tmp1, double *tmp2, int request_indx)
{
   ...
   else
   {
      ...
      // do fft along ny dimension       
      // A(ky,nz,kx) <- fft1d[A(ny,nz,kx)]
      gdevice_batch_cffty_tmpy_zero(true,ny,nq2,n2ft3d,tmp1,d3db::tmpy,zero_row2[nb]);
...
void PGrid::pfftfz(const int nb, double *tmp1, double *tmp2, int request_indx)
{
   ...
   else
   {
      ...
      // do fft along nz dimension      
      // A(kz,kx,ky) <- fft1d[A(nz,kx,ky)]
      gdevice_batch_cfftz_tmpz_zero(true,nz,nq3,n2ft3d,tmp1,d3db::tmpz,zero_row3[nb]);
   }
...
void PGrid::pfftf_step(const int step, const int nb, double *a, double *tmp1, double *tmp2, int request_indx)
{
    ...
    else if (step==2)
    {
       ...
       this->c_pack_start(nb,tmp1,tmp2,request_indx,47);
 ...
 *                              *
 ********************************/
void PGrid::c_pack_start(const int nb, double *a, double *tmp1, const int request_indx, const int msgtype)
{
   //int one=1;
...
void PGrid::rc_pfft3f_queuein(const int nb, double *a)
{
   ...
   for (auto q=0; q<bqsize; ++q)
   {
       ...
       shift1 = n2ft3d*(2*indx);
       shift2 = n2ft3d*(2*indx+1);
       pfftf_step(status,nb,a,atmp+shift1,atmp+shift2,indx+3);
       ++bqstatus[indx];
   }

   ++blast_index;
   if (blast_index>=bqmax) blast_index = 0;
   ++bqsize;
   ...
   pfftf_step(0,nb,a,atmp+shift1,atmp+shift2,blast_index+3);
}
