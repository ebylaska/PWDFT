...
public:
  ...
  std::vector<sycl::queue *> syclQueues; // TODO: queues per device
  sycl::queue *device_queue = nullptr;   // default SYCL queue for now
  ...
  Gdevices() {
    ...
    device_queue =
        new sycl::queue(sycl::gpu_selector{}, asyncHandler,
  ...
  ~Gdevices() {
    ...
    // free GPU memory
    for (std::map<size_t, std::set<double *>>::iterator it =
             free_list_gpu.begin();
         it != free_list_gpu.end(); ++it) {
      for (std::set<double *>::iterator it2 = it->second.begin();
           it2 != it->second.end(); ++it2) {
        sycl::free(*it2, *device_queue);
      }
    }
    free_list_gpu.clear();
    live_ptrs_gpu.clear();
  ...
  resurrect_from_free_list(std::map<size_t, std::set<double *>> &free_map,
                           size_t bytes, std::map<double *, size_t> &liveset) {
    double *ptr = nullptr;
    ...
    if (free_list_gpu.find(bytes) != free_list_gpu.end()) {
      ...
      for (std::map<size_t, std::set<double *>>::iterator it =
        ...
        if (it->first >= bytes && it->second.size() > 0) {
          ptr =
              resurrect_from_free_list(free_list_gpu, it->first, live_ptrs_gpu);
          return ptr;
        }
      }
    }
  ...
  double *getHostMem(size_t bytes) {
    ...
    if (free_list_host.find(bytes) != free_list_host.end()) {
      ...
      for (std::map<size_t, std::set<double *>>::iterator it =
           ...
           it != free_list_host.end(); ++it) {
        if (it->first >= bytes && it->second.size() > 0) {
          ptr = resurrect_from_free_list(free_list_host, it->first,
                                         live_ptrs_host);
          return ptr;
        }
      }
  ...
  int fetch_dev_mem_indx(const size_t ndsize) {
    ...
    if (ii < ndev_mem) {
      ...
      dev_mem[ii] = sycl::malloc_device<double>(ndsize, *device_queue);
      ndev_mem += 1;
    }
  ...
  void TN3_dgemm(int npack, int ne, double alpha, double *host_a,
    ...
    try {
      ...
      device_queue->memcpy(dev_mem[ia_psi], host_a,
                           npack * ne * sizeof(double));
      device_queue->memcpy(dev_mem[ia_hpsi], host_b,
                           npack * ne * sizeof(double));

      oneapi::mkl::blas::gemm(*device_queue, matT, matN, ne, ne, npack, alpha,
                              dev_mem[ia_psi], npack, dev_mem[ia_psi], npack,
                              beta, dev_mem[ic11], ne);
      oneapi::mkl::blas::gemm(*device_queue, matT, matN, ne, ne, npack, alpha,
                              dev_mem[ia_psi], npack, dev_mem[ia_hpsi], npack,
                              beta, dev_mem[ic12], ne);
      oneapi::mkl::blas::gemm(*device_queue, matT, matN, ne, ne, npack, alpha,
                              dev_mem[ia_hpsi], npack, dev_mem[ia_hpsi], npack,
  ...
  void NN_dgemm(int npack, int ne, double alpha, double *host_a, double *host_b,
    ...
    try {
      ...
      oneapi::mkl::blas::gemm(*device_queue, matN, matN, npack, ne, ne, alpha,
                              dev_mem[ia_psi], npack, dev_mem[ib], ne, beta,
                              dev_mem[ia_hpsi], npack);

      device_queue->memcpy(host_c, dev_mem[ia_hpsi],
                           npack * ne * sizeof(double));
      device_queue->wait();
    } catch (sycl::exception const &e) {
      std::cout << "\t\tSYCL exception during GEMM\n"
  ...
  void TN_dgemm(int ne, int nprj, int npack, double alpha, double *host_a,
    ...
    try {
      ...
      device_queue->memcpy(dev_mem[ib_prj], host_b,
                           npack * nprj * sizeof(double));

      oneapi::mkl::blas::gemm(*device_queue, matT, matN, ne, nprj, npack, alpha,
                              dev_mem[ia_psi], npack, dev_mem[ib_prj], npack,
                              beta, dev_mem[ic], ne);
  ...
  void NT_dgemm(int npack, int ne, int nprj, double alpha, double *host_a,
    ...
    try {
      ...
      oneapi::mkl::blas::gemm(*device_queue, matN, matT, npack, ne, nprj, alpha,
                              dev_mem[ib_prj], npack, dev_mem[ib], ne, beta,
                              dev_mem[ia_hpsi], npack);
  ...
  void hpsi_copy_host2gpu(int npack, int ne, double *hpsi) {
    device_queue->memcpy(dev_mem[ia_hpsi], hpsi,
                         2 * npack * ne * sizeof(double));
  }
  ...
  void hpsi_copy_gpu2host(int npack, int ne, double *hpsi) {
    device_queue->wait();
    device_queue->memcpy(hpsi, dev_mem[ia_hpsi],
                         2 * ne * npack * sizeof(double));
    device_queue->wait();
  }
    ...
    try {
      ...
      if (forward)
        fftevent = compute_forward(*desc_x, dev_mem[ia_dev],
                                   std::vector<sycl::event>{h2d_event});
      else
      ...
      //               indx += (nx+2);
      //           }
      d2h_event = device_queue->memcpy(a, dev_mem[ia_dev],
                                       n2ft3d * sizeof(double), fftevent);
      d2h_event.wait();
    } catch (sycl::exception const &e) {
