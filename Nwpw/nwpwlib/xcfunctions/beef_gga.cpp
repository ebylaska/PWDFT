   ...
   to nup, ndn, |grad nup|, |grad ndn|, and |grad n|.
    ...
    Exit - xce(*)  : PBE96 energy density
         ...
         - fdn(*,3): d(n*xce)/d|grad nup|, d(n*xce)/d|grad ndn|
                     d(n*xce)/d|grad n|
*/
void gen_BEEF_BW_unrestricted(const int n2ft3d, double *dn_in, double *agr_in,
                              const double x_parameter,
                              const double c_parameter, double alphac,
 ...
 ************************************/
/*
    This routine calculates the non-Langreth terms of the BEEF-vdw
   exchange-correlation potential(xcp) and energy density(xce).
...
*/
void gen_BEEF_BW_restricted(const int n2ft3d, double *rho_in, double *agr_in,
                            const double x_parameter, const double c_parameter,
                            const double alphac, double *xce, double *fn,
                            double *fdn) {
  ...
  for (int i = 0; i < n2ft3d; ++i) {
    ...
    fnc = ec - alphac * (onethird * rs * ec_lda_rs) -
          malphac * (onethird * rs * Hrs) - malphac * (sevensixths * t * Ht);
