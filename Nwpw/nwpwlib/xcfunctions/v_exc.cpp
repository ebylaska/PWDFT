...
void v_exc(const int ispin, const int n2ft3d, double *dn, double *xcp, double *xce, double *x) 
{
   nwpw_timing_start(4);
   ...
   rhoup = dn;
   rhodn = dn + (ispin - 1) * n2ft3d;
   xcpup = xcp;
   xcpdn = xcp + (ispin - 1) * n2ft3d;
   xceup = xce;
   xcedn = xce + (ispin - 1) * n2ft3d;
   ...
   for (k = 0; k < n2ft3d; ++k) {
     ...
     xx1 = x[k] + cp3;
     xx1 *= xx1;
     xceup[k] = cp1 * std::log(xx * x[k] * x[k]) + cp2 * std::log(xx * xx1) +
     ...
     xcpup[k] =
         xceup[k] - one6th * x[k] *
                        (dp1 / x[k] + dp2 / (x[k] + dp3) +
                         dp4 * xx * (2.00 * x[k] + bp) + dp5 / (xx1 + dp7));
   }
   ...
   if (ispin == 2) {
     ...
     for (k = 0; k < n2ft3d; ++k) {
       ...
       xx1 = x[k] + cf3;
       xx1 *= xx1;
       xcedn[k] = cf1 * std::log(xx * x[k] * x[k]) + cf2 * std::log(xx * xx1) +
       ...
       xcpdn[k] =
           xcedn[k] - one6th * x[k] *
                          (df1 / x[k] + df2 / (x[k] + df3) +
                           df4 * xx * (2.00 * x[k] + bf) + df5 / (xx1 + df7));
     }
     ...
     for (k = 0; k < n2ft3d; ++k) {
       ...
       f = (zup * (std::pow(zup, one3rd)) + zdw * (std::pow(zdw, one3rd)) -
            2.00) *
           fc;
       xcpup[k] = (1.00 - f) * xcpup[k] + f * xcpdn[k];
       df = (std::pow(zup, one3rd) - std::pow(zdw, one3rd)) *
            (xcedn[k] - xceup[k]) * fd;
