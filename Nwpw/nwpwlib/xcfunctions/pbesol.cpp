...
/* PBEsol GGA correlation constants */
#define GAMMA 0.031090690869655e0
#define BETA 0.0460e0
...
/****************************************
 *					*
 *	    gen_PBEsol_unrestricted	*
 *					*
 ****************************************/
/*
     This function returns the PBEsol exchange-correlation
   ...
   to nup, ndn, |grad nup|, |grad ndn|, and |grad n|.
    ...
    Exit - xce(*)  : PBEsol energy density
         - fn(*,2) : d(n*xce)/dnup, d(n*xce)/dndn
         - fdn(*,3): d(n*xce)/d|grad nup|, d(n*xce)/d|grad ndn|
                     d(n*xce)/d|grad n|
*/
void gen_PBEsol_BW_unrestricted(const int n2ft3d, double *dn_in, double *agr_in,
  ...
  for (int i = 0; i < n2ft3d; ++i) {
    ...
    /********************************************
     **** calculate PBEsol correlation energy ****
     ********************************************/
    phi3 = phi * phi * phi;
    phi4 = phi3 * phi;
...
/****************************************
 *					*
 *	    gen_PBEsol_BW_restricted	*
 *					*
 ****************************************/
/*
   This routine calculates the PBEsol exchange-correlation
   ...
   Entry - n2ft3d     : number of grid points
     ...
     Exit  - xce(n2ft3d) : PBEsol exchange correlation energy density
             fn(n2ft3d)  : d(n*xce)/dn
             fdn(n2ft3d) : d(n*xce/d|grad n|
*/

void gen_PBEsol_BW_restricted(const int n2ft3d, double *rho_in, double *agr_in,
                              const double x_parameter,
  ...
  for (int i = 0; i < n2ft3d; ++i) {
    ...
    /* PBEsol correlation energy  corrections */
    t2 = t * t;
    t4 = t2 * t2;
    B = -ec_lda / GAMMA;
    ...
    /* PBEsol correlation fdn and fdnc derivatives */
    t6 = t4 * t2;
