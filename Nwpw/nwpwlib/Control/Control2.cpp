...
Control2::Control2(const int np0, const std::string rtdbstring) {
  ...
  if (permanent_dir_str.size() > 0) {
    ...
    if (input_v_movecs[0] != '/')
      input_v_movecs = permanent_dir_str + "/" + input_v_movecs;
    if (output_v_movecs[0] != '/')
      output_v_movecs = permanent_dir_str + "/" + output_v_movecs;
  }
  ...
  if (ptask == 5)
    if (rtdbjson["nwpw"]["steepest_descent"]["loop"][0].is_number_integer())
      ploop[0] = rtdbjson["nwpw"]["steepest_descent"]["loop"][0];
  if (ptask == 5)
    if (rtdbjson["nwpw"]["steepest_descent"]["loop"][1].is_number_integer())
      ploop[1] = rtdbjson["nwpw"]["steepest_descent"]["loop"][1];
  if (ptask == 6)
    if (rtdbjson["nwpw"]["car-parrinello"]["loop"][0].is_number_integer())
      ploop[0] = rtdbjson["nwpw"]["car-parrinello"]["loop"][0];
  if (ptask == 6)
    if (rtdbjson["nwpw"]["car-parrinello"]["loop"][1].is_number_integer())
      ploop[1] = rtdbjson["nwpw"]["car-parrinello"]["loop"][1];

  pscaling[0] = 1.0;
  pscaling[1] = 1.0;
  if (ptask == 6)
    if (rtdbjson["nwpw"]["car-parrinello"]["scaling"][0].is_number_float())
      pscaling[0] = rtdbjson["nwpw"]["car-parrinello"]["scaling"][0];
  if (ptask == 6)
    if (rtdbjson["nwpw"]["car-parrinello"]["scaling"][0].is_number_integer())
      pscaling[0] = rtdbjson["nwpw"]["car-parrinello"]["scaling"][0];
  if (ptask == 6)
    if (rtdbjson["nwpw"]["car-parrinello"]["scaling"][1].is_number_float())
      pscaling[1] = rtdbjson["nwpw"]["car-parrinello"]["scaling"][1];
  if (ptask == 6)
    if (rtdbjson["nwpw"]["car-parrinello"]["scaling"][1].is_number_integer())
      pscaling[1] = rtdbjson["nwpw"]["car-parrinello"]["scaling"][1];
  ...
  if (ptask == 5)
    if (rtdbjson["nwpw"]["steepest_descent"]["tolerances"][0].is_number_float())
      ptolerances[0] = rtdbjson["nwpw"]["steepest_descent"]["tolerances"][0];
  if (ptask == 5)
    if (rtdbjson["nwpw"]["steepest_descent"]["tolerances"][1].is_number_float())
  ...
  if (ptask == 5)
    if (rtdbjson["nwpw"]["steepest_descent"]["cutoff"][0].is_number_float())
      pwcut = rtdbjson["nwpw"]["steepest_descent"]["cutoff"][0];
  if (ptask == 5)
    if (rtdbjson["nwpw"]["steepest_descent"]["cutoff"][1].is_number_float())
  ...
  if (!rtdbjson["nwpw"]["generalized_poisson"].is_null()) {
    ...
    if (gpoissonjson["on"].is_boolean())
      pgpoisson_on = gpoissonjson["on"];
    if (gpoissonjson["dielec"].is_number_float())
      pgpoisson_dielec = gpoissonjson["dielec"];
    if (gpoissonjson["rho0"].is_number_float())
      pgpoisson_rho0 = gpoissonjson["rho0"];
    if (gpoissonjson["beta"].is_number_float())
      pgpoisson_beta = gpoissonjson["beta"];
    if (gpoissonjson["rhomin"].is_number_float())
  ...
  if (!rtdbjson["nwpw"]["car-parrinello"]["Nose-Hoover"].is_null()) {
    ...
    if (nosejson["on"].is_boolean())
      pnose_on = nosejson["on"];
    if (nosejson["restart"].is_boolean())
      pnose_restart = nosejson["restart"];

    if (nosejson["Mchain"].is_number_integer())
      pnose_mchain = nosejson["Mchain"];
    if (nosejson["Nchain"].is_number_integer())
      pnose_nchain = nosejson["Nchain"];

    if (nosejson["Te"].is_number_float())
      pnose_te = nosejson["Te"];
    if (nosejson["Tr"].is_number_float())
      pnose_tr = nosejson["Tr"];
    if (nosejson["Pe"].is_number_float())
      pnose_pe = nosejson["Pe"];
    if (nosejson["Pr"].is_number_float())
      pnose_pr = nosejson["Pr"];

    if (nosejson["Ne_chain"].is_number_float())
      pnose_ne_chain = nosejson["Ne_chain"];
    if (nosejson["eke0"].is_number_float())
      pnose_eke0 = nosejson["eke0"];
...
int Control2::cubetype_cubefiles(const int i) {
  ...
  if (!rtdbjson["nwpw"]["dplot"].is_null()) {
    ...
    if (mystring_contains(key, "orbital2-"))
      cubetype = -99;
    if (mystring_contains(key, "density-total"))
      cubetype = -1;
    if (mystring_contains(key, "density-diff"))
      cubetype = -2;
    if (mystring_contains(key, "density-alpha"))
      cubetype = -3;
    if (mystring_contains(key, "density-up"))
      cubetype = -3;
    if (mystring_contains(key, "density-beta"))
      cubetype = -4;
    if (mystring_contains(key, "density-down"))
      cubetype = -4;
    if (mystring_contains(key, "density-laplacian"))
      cubetype = -5;
    if (mystring_contains(key, "density-potential"))
      cubetype = -6;
    if (mystring_contains(key, "density-fullpotential"))
      cubetype = -7;
    if (mystring_contains(key, "elf"))
      cubetype = -8;
  }
  return cubetype;
}
