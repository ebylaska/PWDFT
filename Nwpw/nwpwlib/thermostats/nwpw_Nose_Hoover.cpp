 ...
 *                                         *
 *******************************************/
nwpw_Nose_Hoover::nwpw_Nose_Hoover(Ion &myion, int nemax0, double eke0,
                                   Control2 &control) {
  ...
  if (nose_on) {
    ...
    for (auto n = 1; n < Nchain; ++n)
      Er0[n] = 0.5 * kb * Tr;

    Ee0[0] = 4.0 * kb * Te * fmass * (nion / total_mass) * eke0_init;
    ;
    double betae = Ee0[0] / Ne_chain;
    for (auto m = 1; m < Mchain; ++m)
      Ee0[m] = betae;

    double pi = 4.0 * std::atan(1.0);
    if (!nosers) {
      for (auto m = 0; m < Mchain; ++m)
        Qe[m] = Ee0[m] * std::pow((Pe[m] / pi), 2);
      for (auto n = 0; n < Nchain; ++n)
        Qr[n] = Er0[n] * std::pow((Pr[n] / pi), 2);
    } else {
      for (auto m = 0; m < Mchain; ++m)
        Pe[m] = pi * std::sqrt(Qe[m]) / Ee0[m];
      for (auto n = 0; n < Nchain; ++n)
        Pr[n] = pi * std::sqrt(Qr[n]) / Er0[n];
    }
  }
}
...
void nwpw_Nose_Hoover::writejsonstr(std::string &rtdbstring) {
  if (nose_on) {
    ...
    for (auto m = 0; m < Mchain; ++m)
      Xem[m] = (Xe2[m] - Xe0[m]) / (2.0 * time_step);
    for (auto n = 0; n < Nchain; ++n)
      Xrm[n] = (Xr2[n] - Xr0[n]) / (2.0 * time_step);
  ...
  if (nose_on) {
    ...
    for (auto m = 0; m < Mchain; ++m)
      stream << "     link = " << std::setw(3) << (m + 1)
             << " Te =" << std::fixed << std::setprecision(2) << std::setw(8)
             << Te << " Qe =" << std::scientific << std::setprecision(3)
             << std::setw(10) << Qe[m] << " 2*pi/we =" << std::scientific
             << std::setprecision(3) << std::setw(10) << Pe[m]
             << " Ee0 =" << std::scientific << std::setprecision(3)
             << std::setw(10) << Ee0[m] << std::endl;
    for (auto n = 0; n < Nchain; ++n)
      stream << "     link = " << std::setw(3) << (n + 1)
             << " Tr =" << std::fixed << std::setprecision(2) << std::setw(8)
             << Tr << " Qr =" << std::scientific << std::setprecision(3)
             << std::setw(10) << Qr[n] << " 2*pi/wr =" << std::scientific
             << std::setprecision(3) << std::setw(10) << Pr[n]
             << " Er0 =" << std::scientific << std::setprecision(3)
             << std::setw(10) << Er0[n] << std::endl;
...
std::string nwpw_Nose_Hoover::thermostatprint() {
  ...
  if (nose_on) {
    ...
    double er = r_energy();
    stream << std::endl;
    stream << " thermostat energy (elc) : " << ee << " ( " << ee / nemax
           << "/elc)" << std::endl;
    stream << " thermostat energy (ion) : " << er << " ( " << er / nion
           << "/ion)" << std::endl;
