...
Paw_xc::Paw_xc(Ion *myion0, Pneb *mypneb0,  Control2& control,
               const int nprj[], const int nbasis[], const int n1dgrid[], const int psp_type[], const int lmax0[],
               int* l_prj[], int* m_prj[], int* b_prj[])
{
   ...
   for (int ia=0; ia<nkatm; ++ia)
   {
      ...
      if (nbasis[ia] >nbasis_max)  nbasis_max  = nbasis[ia];
      if (n1dgrid[ia]>n1dgrid_max) n1dgrid_max = n1dgrid[ia];
   }
   ...
   mtr_size = 2*(nbasis_max*nbasis_max)*((mult_l_max+1)*(mult_l_max+1));
   ...
   paw_xc_tlm       = new (std::nothrow) double [paw_xc_ntheta*paw_xc_nphi*(mult_l_max+1)*(mult_l_max+1)]();


   //**** used for generating derivatives of tlm's ****
   if (gga>=10)
   {
      paw_xc_dtlm_theta = new (std::nothrow) double [paw_xc_ntheta*paw_xc_nphi*(mult_l_max+1)*(mult_l_max+1)]();
      paw_xc_dtlm_phi   = new (std::nothrow) double [paw_xc_ntheta*paw_xc_nphi*(mult_l_max+1)*(mult_l_max+1)]();
   }

   Paw_get_spher_grid(paw_xc_ntheta,paw_xc_nphi,paw_xc_angle_phi,paw_xc_cos_theta,paw_xc_w_theta,paw_xc_w_phi);
   ...
   if (gga>=10) 
   {
      ...
      for (auto i_p=0; i_p<paw_xc_nphi; ++i_p)
      {
         ...
         for (auto m=-l; m<=l; ++m)
         {
            ...
            {
               double tmp_phi;
               double tmp_theta = util_rtheta_lm_div(l,m,paw_xc_cos_theta[i_t]);
               double angle_phi = paw_xc_angle_phi[i_p];
