...
/*
     Calculates the two electron two center Gaussian integral
                                            ...
                                            //
    WGaussian(la,ma,sa,lb,Ra,mb,sb,Rb)   =  || g(la,ma,sa;r-Ra) *
   g(lb,mb,sb;r'-Rb)
                                            ||
   ------------------------------------  dr dr'
                                            //                |r-r'|
     ...
     The normalization constant C_l is defined such at
            ...
            /
*/
double util_WGaussian(const int la, const int ma, const double sa, const int lb,
                      const int mb, const double sb, const double Rab[]) {
  ...
  for (auto l = abs(la - lb); l <= (la + lb); l += 2) {
    double mtmp = 0.0;
    for (auto m = -l; m <= l; ++m) {
      mtmp += util_gaunt(false, l, m, la, ma, lb, mb) *
              util_Tesseral_lm(l, m, cos_theta, phi);
    }
 ...
 ******************************************************/
/*
     Calculates the two electron two center Gaussian integral and it's
   derivative wrt to Rab

                                            //
   dWGaussian(la,ma,sa,lb,Ra,mb,sb,Rb)   =  || g(la,ma,sa;r-Ra) *
   g(lb,mb,sb;r'-Rb)
                                            ||
   ------------------------------------  dr dr'
                                            //                |r-r'|
     ...
     The normalization constant C_l is defined such at
            ...
            /
*/
void util_dWGaussian(const int la, const int ma, const double sa, const int lb,
                     const int mb, const double sb, const double Rab[],
                     double &W, double dW[]) {
...
*/
double util_UGaussian(const int la, const int ma, const double sa, const int lb,
                      const int mb, const double sb) {
  double U = 0.0;
...
/*
   ...
   ------------------------------------  dr dr'
     ...
     The normalization constant C_l is defined such at
            ...
            /
*/
double util_WGaussian3(const int la, const int ma, const double sa,
...
/*
   ...
   ------------------------------------  dr dr'
     ...
     The normalization constant C_l is defined such at
            ...
            /
*/
double util_WGaussian2(const int la, const int ma, const double sa,
                       const int lb, const int mb, const double sm,
                       const double Rab[]) {
  double pi = 4.0 * atan(1.0);
  double x = util_double_factorial(2 * la + 1);
  double y = util_double_factorial(2 * lb + 1);
...
/*
     ...
     of the two electron two center Gaussian integral and it's derivative wrt to
   Rab
   ...
   ------------------------------------  dr dr'
     ...
     The normalization constant C_l is defined such at
            ...
            /
*/
void util_dWGaussian3(const int la, const int ma, const double sa, const int lb,
                      const int mb, const double sb, const double sm,
                      const double Rab[], double &W, double dW[]) {
  double pi = 4.0 * atan(1.0);
  double x = util_double_factorial(2 * la + 1);
  double y = util_double_factorial(2 * lb + 1);
  double alpha_ab = sqrt(0.25 * (sa * sa + sb * sb));
...
/*
     Calculates the two electron two center Gaussian integral
                                             ...
                                             //
    CWGaussian(la,ma,sa,lb,Ra,mb,sb,Rb)   =  || g(la,ma,sa;r-Ra) *
   g(lb,mb,sb;r'-Rb)
                                             ||
   ------------------------------------  dr dr'
                                             //                |r-r'|
     ...
     The normalization constant C_l is defined such at
            ...
            /
*/
std::complex<double> util_CWGaussian(const int la, const int ma,
                                     const double sa, const int lb,
                                     const int mb, const double sb,
                                     const double Rab[]) {
  int m = ma + mb;
  double pi = 4.0 * atan(1.0);
  ...
  for (auto l = abs(la - lb); l <= (la + lb); l += 2) {
    if (std::abs(m) <= l) {
      std::complex<double> mtmp = util_gaunt(true, l, m, la, ma, lb, -mb) *
                                  util_YSpherical_lm(l, m, cos_theta, phi);
      tmp += fac * mtmp * util_GaussBessel(la + lb, l, alpha, R);
    }
    fac = -fac;
  }
...
/*
   ...
   derivative wrt to Rab
                                             ...
                                             //
   dCWGaussian(la,ma,sa,lb,Ra,mb,sb,Rb)   =  || g(la,ma,sa;r-Ra) *
   g(lb,mb,sb;r'-Rb)
                                             ||
   ------------------------------------  dr dr'
                                             //                |r-r'|
     ...
     The normalization constant C_l is defined such at
            ...
            /
*/
void util_dCWGaussian(const int la, const int ma, const double sa, const int lb,
                      const int mb, const double sb, const double Rab[],
  ...
  for (auto l = abs(la - lb); l <= (la + lb); l += 2) {
    if (std::abs(m) <= l) {
      ...
      std::complex<double> mtmp =
          gg1 * util_YSpherical_lm(l, m, cos_theta, phi);
...
/*
   ...
   ------------------------------------  dr dr'
     ...
     The normalization constant C_l is defined such at
            ...
            /
*/
std::complex<double> util_CWGaussian3(const int la, const int ma,
                                      const double sa, const int lb,
                                      const int mb, const double sb,
                                      const double sm, const double Rab[]) {
  int m = ma + mb;
...
/*
   ...
   ------------------------------------  dr dr'
     ...
     The normalization constant C_l is defined such at
            ...
            /
*/
std::complex<double> util_CWGaussian2(const int la, const int ma,
                                      const double sa, const int lb,
                                      const int mb, const double sm,
                                      const double Rab[]) {
  int m = ma + mb;
  double pi = 4.0 * atan(1.0);
  double x = util_double_factorial(2 * la + 1);
