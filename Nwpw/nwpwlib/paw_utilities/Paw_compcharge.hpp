...
class Paw_compcharge {
  ...
  double
      *glm; // glm(k, lm=1:(max_mult_l+1)**2) =  Tlm(k) * |k|**l / (2*l+1)!!
            //    - Note that (-i)**l factor will be assumed in the calculation.
            //      Also note that  the Tlm and gaussian terms are rearranged to
            //      save space It is more natural to define g_lm, gk and
            //      gk_smooth
            //           g_lm      =  4*pi*Tlm(k)
            //           gk        = |k|**l * Exp[-k*k*sigma(ia)**2 / 4] /
            //           (2*l+1)!! gk_smooth = |k|**l * Exp[-k*k*sigma_smooth**2
            //           / 4] / (2*l+1)!!
            //       but this requires more space

  double *Qlm;  // Qlm(lm=1:(mult_l+1)**2,2,1:nion_paw) = compensation charge
                // coefficients
  double *Qlmx; // Qlmx(lm=1:(mult_l+1)**2,2,1:nion_paw) = compensation charge
                // coefficients
  double *Qlmy; // Qlmy(lm=1:(mult_l+1)**2,2,1:nion_paw) = compensation charge
                // coefficients
  double *Qlmz; // Qlmz(lm=1:(mult_l+1)**2,2,1:nion_paw) = compensation charge
                // coefficients
  double
      *dEmult_Qlm; // dEmult_Qlm(lm=1:(mult_l+1)**2,2,1:nion_paw) = dEmult/dQlm
  double *dElocal_Qlm; // dElocal_Qlm(lm=1:(mult_l+1)**2,2,1:nion_paw) =
                       // dElocal/dQlm
  double *dE_Qlm; // dE_Qlm(lm=1:(mult_l+1)**2,2,1:nion_paw) = dE/dQlm where E
...
public:
  /* Constructors */
  Paw_compcharge(Ion *, Pneb *, Control2 &, const int *, const int *,
                 const int *, const int *, const double *, const int, int **,
