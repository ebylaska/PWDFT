 ...
 *      /2pi  / pi
 *     |     |
 *   = |     | dconjg(Y_lm(theta,phi)) * Y_l1m1(theta,phi) *
 * dconjg(Y_l2m2(theta,phi))  sin(theta) dtheta dphi |     | /0    / 0
 *
 *      /2pi  / pi
 *     |     |
 *   = |     | Y_lm(theta,phi) * dconjg(Y_l1m1(theta,phi)) * Y_l2m2(theta,phi)
 * sin(theta) dtheta dphi |     | /0    / 0
 *
 *
 *   = sqrt( (2*l+1)*(2*l2+1)/(4*pi*(2*l1+1)) ) * Clebsch(l l2 l1; m m2 m1) *
 * Clebsch(l l2 l1; 0 0 0)
 */
static double gaunt_sub_cmplx(const int l1, const int m1, const int l2,
                              const int m2, const int l3, const int m3) {
  double coeff;
  ...
  // **** Error Checking ****
  if ((l1 < 0) || (l2 < 0) || (l3 < 0))
    std::cout << "Invalid parameter in gen_gaunt_coeff, negative l"
              << std::endl;
  ...
  for (auto i = 0; i < order; ++i)
    gg += w[i] * util_ytheta_lm(l1, m1, x[i]) * util_ytheta_lm(l2, m2, x[i]) *
 ...
 *      /2pi  / pi
 *     |     |
 *   = |     | T_lm(theta,phi)) * T_l1m1(theta,phi) * T_l2m2(theta,phi))
 * sin(theta) dtheta dphi |     | /0    / 0
 */
static double gaunt_sub_real(const int l1, const int m1, const int l2,
                             const int m2, const int l3, const int m3) {
  double coeff;
  double pi = 4.0 * atan(1.0);
  double twopi = 2.00 * pi;
  ...
  double gg = 0.0;
  for (auto i = 0; i < order; ++i)
    gg += w[i] * util_rtheta_lm(l1, m1, x[i]) * util_rtheta_lm(l2, m2, x[i]) *
...
double Paw_gaunt::gaunt(const int l1, const int m1, const int l2, const int m2,
                        const int l3, const int m3) {
 ...
 *      /2pi  / pi
 *     |     |
 *   = |     | dconjg(dY_lm(theta,phi)/dtheta) * dY_l1m1(theta,phi)/dtheta *
 * dconjg(Y_l2m2(theta,phi))  sin(theta) dtheta dphi |     | /0    / 0
 *
 *      /2pi  / pi
 *     |     |
 *   = |     | dY_lm(theta,phi)/dtheta * dconjg(dY_l1m1(theta,phi)/dtheta) *
 * Y_l2m2(theta,phi)  sin(theta) dtheta dphi |     | /0    / 0
 *
 *
 *
 * or if iscmplx=.false. this routine computes the Taunt coefficient
 ...
 *      /2pi  / pi
 *     |     |
 *   = |     | dT_lm(theta,phi))/dtheta * dT_l1m1(theta,phi)/dtheta *
 * T_l2m2(theta,phi))  sin(theta) dtheta dphi |     | /0    / 0
 */
double Paw_gaunt::gaunt2(const int l1, const int m1, const int l2, const int m2,
                         const int l3, const int m3) {
  double coeff;
  double pi = 4.0 * atan(1.0);
  double twopi = 2.0 * pi;
  ...
  if (gaunt_iscmplx)
    for (auto i = 0; i < order; ++i)
      gg += w[i] * util_ytheta2_lm(l1, m1, x[i]) *
            util_ytheta2_lm(l2, m2, x[i]) * util_ytheta_lm(l3, m3, x[i]);
  else
    for (auto i = 0; i < order; ++i)
      gg += w[i] * util_rtheta2_lm(l1, m1, x[i]) *
            util_rtheta2_lm(l2, m2 + 1, x[i]) * util_rtheta_lm(l3, m3, x[i]);
  return (gg * coeff);
}
 ...
 *      /2pi  / pi
 *     |     |
 *   = |     | dconjg(dY_lm(theta,phi)/dphi) * dY_l1m1(theta,phi)/dphi *
 * dconjg(Y_l2m2(theta,phi)) * 1/(sin(theta))**2 *  sin(theta) dtheta dphi | |
 *    /0    / 0
 *
 *      /2pi  / pi
 *     |     |
 *   = |     | dY_lm(theta,phi)/dphi * dconjg(dY_l1m1(theta,phi)/dphi) *
 * Y_l2m2(theta,phi) 1/(sin(theta))**2  sin(theta) dtheta dphi |     | /0    / 0
 *
 *
 *
 * or if iscmplx=.false. this routine computes the Taunt coefficient
 ...
 *      /2pi  / pi
 *     |     |
 *   = |     | (1/sin(theta))**2 * dT_lm(theta,phi))/dphi *
 * dT_l1m1(theta,phi)/dphi * T_l2m2(theta,phi))  sin(theta) dtheta dphi |     |
 *    /0    / 0
 */

double Paw_gaunt::gaunt3(const int l1, const int m1, const int l2, const int m2,
                         const int l3, const int m3) {
  double coeff;
  double pi = 4.0 * atan(1.0);
  double twopi = 2.0 * pi;
  ...
  if (gaunt_iscmplx)
    for (auto i = 0; i < order; ++i)
      gg -= w[i] * util_theta_lm_div(l1, m1, x[i]) *
            (m1)*util_theta_lm_div(l2, m2, x[i]) *
            (m2)*util_ytheta_lm(l3, m3, x[i]);
  else
    for (auto i = 0; i < order; ++i)
      gg += w[i] * util_rtheta_lm_div(l1, -m1, x[i]) *
            (m1)*util_rtheta_lm_div(l2, -m2, x[i]) *
            (m2)*util_rtheta_lm(l3, m3, x[i]);
  return (gg * coeff);
}
