 ...
 *                                 *
 ***********************************/
static void Paw_compcharge_gen_smoothpsp(const int nray, const double Gray[],
                                         const double vlray[], const int npack0,
                                         const double Gx[], const double Gy[],
                                         const double Gz[], double vlsmooth[]) {
  double dG = Gray[2] - Gray[1];
  for (auto k = 0; k < npack0; ++k) {
    double Q = sqrt(Gx[k] * Gx[k] + Gy[k] * Gy[k] + Gz[k] * Gz[k]);
    if (Q > 1.0e-9) {
      double nxray = (Q / dG) + 1.0;
      double P = util_splint(&Gray[1], &vlray[1], &vlray[1 + nray], nray - 1,
                             nxray - 1, Q);
      vlsmooth[k] = P;
    } else
      vlsmooth[k] = vlray[0];
...
static void Paw_compcharge_gen_vlray(const double sigma_smooth,
                                     const double log_amesh, const int nrho,
                                     const double rho[], const double f[],
                                     const int nray, const double Gray[],
                                     double vlray[], double tmpray[]) {
  ...
  //**** five point formula ***
  double yp1 = (-50.0 * vlray[1] + 96.0 * vlray[2] - 72.0 * vlray[3] +
                32.0 * vlray[4] - 6.0 * vlray[5]) /
               (24.0 * dG);

  std::cout << "into util_spline vlray" << std::endl;

  util_spline(&Gray[1], &vlray[1], nray - 1, yp1, 0.0, &vlray[1 + nray],
              tmpray);
...
static void Paw_compcharge_gen_vk_smooth(const int nray, const double *Gray,
                                         ...
                                         const int npack0, const double Gx[],
  ...
  Paw_compcharge_gen_vlray(sigma_smooth, log_bmesh, nrho, rho, f, nray, Gray,
                           vlray, tmpray);
...
Paw_compcharge::Paw_compcharge(Ion *myion0, Pneb *mypneb0, Control2 &control,
                               const int nprj[], const int nbasis[],
                               const int psp_type[], const int lmax0[],
                               const double sigma[], const int nprj_max,
                               int *l_prj[], int *m_prj[], int *b_prj[],
  ...
  for (auto l = 0; l <= mult_l_max; ++l) {
    ...
    if (l > 0) {
      for (auto k = 0; k < npack0; ++k) {
        double gg = Gall[0][k] * Gall[0][k] + Gall[1][k] * Gall[1][k] +
                    Gall[2][k] * Gall[2][k];
    ...
    for (auto m = -l; m <= l; ++m) {
      util_Tesseral3_vector_lm(l, m, npack0, Gall[0], Gall[1], Gall[2],
                               &glm[lm * npack0]);
  ...
  Paw_compcharge_gen_vk_smooth(n_ray, G_ray, sigma_smooth, npack0, Gall[0],
                               Gall[1], Gall[2], vk_smooth);
      ...
      for (auto m = -l; m <= l; ++m) {
        for (auto j = 0; j < nprj[ia]; ++j) {
          ...
          for (auto i = 0; i < nprj[ia]; ++i) {
            ...
            int bi = b_prj[ia][i];
            double taunt =
                util_gaunt(false, l, m, li, mi, lj, mj) *
                comp_charge_matrix[ia][matindx2(bi, bj, l, nbasis[ia])];
            ...
            for (auto l = 0; l <= mult_l[iia]; ++l) {
              for (auto m = -l; m <= l; ++m) {
                double taunt = util_gaunt(false, l, m, li, mi, lj, mj) *
                               ...
                               hartree_matrix[ia][matindx4(bi, bj, bi1, bj1, l,
                                                           nbasis[ia])];
                if (std::abs(taunt) > 1.0e-15)
                  ++indx;
              }
