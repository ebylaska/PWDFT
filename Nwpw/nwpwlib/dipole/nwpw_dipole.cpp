 ...
 *                                   *
 *************************************/
nwpw_dipole::nwpw_dipole(Ion *myion0, Pneb *mypneb0, Strfac *mystrfac0,
                         Control2 &) {
  mypneb = mypneb0;
  myion = myion0;
  mystrfac = mystrfac0;
    ...
    for (auto n2 = -ncut; n2 <= ncut; ++n2)
      for (auto n1 = -ncut; n1 <= ncut; ++n1)
        for (auto n0 = -ncut; n0 <= ncut; ++n0) {
          x = myion->rion(0, ii) + n0 * mypneb->lattice->unita(0, 0) +
              n1 * mypneb->lattice->unita(0, 1) +
              n2 * mypneb->lattice->unita(0, 2);
          y = myion->rion(1, ii) + n0 * mypneb->lattice->unita(1, 0) +
              n1 * mypneb->lattice->unita(1, 1) +
              n2 * mypneb->lattice->unita(1, 2);
          z = myion->rion(2, ii) + n0 * mypneb->lattice->unita(2, 0) +
