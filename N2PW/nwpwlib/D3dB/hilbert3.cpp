...
int     hilbert3d(int *p0, int *p1, int *p2, int *p3)
{
   ...
   d1[0] = p1[0]-p0[0]; d1[1] = p1[1]-p0[1]; d1[2] = p1[2]-p0[2];
   d2[0] = p2[0]-p0[0]; d2[1] = p2[1]-p0[1]; d2[2] = p2[2]-p0[2];
   d3[0] = p3[0]-p0[0]; d3[1] = p3[1]-p0[1]; d3[2] = p3[2]-p0[2];

   if (d1[0] ==1) 
   {
      indx = p0[0] + p0[1]*count + p0[2]*count*count;
      map3[indx] = length;
      ++length;
   }
   else
   ...
   {
      parent_direction = hilbert3_dir(parent(i),parent(j),parent(k),
                                     level-1,high,
                                     start);
      crnr = corner(i,j,k);
...
void hilbert3d_map(const int sizex, const int sizey, const int sizez, int *map)
{
    int i,j,size;
    int ii,jj,iii,jjj;
    int level,count;
   ...
   for (ii=0; ii<count; ++ii)
   {
      ...
      map3[ii+jj*count + kk*count*count] = hilbert3d(ii,jj,kk,level);
      olist3.insert(map3[ii+jj*count+kk*count*count]);
   }
   for (kk=0; kk<count; ++kk)
   for (jj=0; jj<count; ++jj)
   for (ii=0; ii<count; ++ii)
      map3[ii+jj*count + kk*count*count] = olist3.index(map3[ii+jj*count+kk*count*count]);
   ...
   for (j=0; j<sizex*sizey*sizez; ++j) map[j] = -9;
