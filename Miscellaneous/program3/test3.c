...
int main(int nargc, char *argc[]) 
{
   ...
   for (int i=0; i<Ksize*Nsize; ++i) B[i] = sin(((float) i))+4.0;
...
*/
   ...
   printf("Gpu total time                       = time6-time2 = %f ms\n",1000.0*(float)(time6-time2)/CLOCKS_PER_SEC);

   float dt = (float)(time6-time2)/CLOCKS_PER_SEC;
   ...
   if (runserial)
   {
      ...
      dt = (float)(time8-time7)/CLOCKS_PER_SEC;
