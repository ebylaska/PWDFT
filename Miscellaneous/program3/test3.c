                               ...
                               const int iswrite[])
{
   ...
   gpuprogram->bufsize = (cl_uint *) malloc(nbuf*sizeof(cl_uint));
   gpuprogram->buf_mem_obj = (cl_mem *) malloc(sizeof(cl_mem)*nbuf);
   for (int i=0; i<nbuf; ++i)
   {
      gpuprogram->bufsize[i] = (cl_uint) bufsize[i];
      if (iswrite[i]==1) 
      {
      ...
      else if (iswrite[i]==2) 
      {
         gpuprogram->buf_mem_obj[i] = clCreateBuffer(gpuprogram->context, 
                                                     CL_MEM_WRITE_ONLY, 
                                                     gpuprogram->bufsize[i]*sizeof(int),
                                                     NULL, &ret);
      }
      else if (iswrite[i]==3) 
      {
...
int main(int nargc, char *argc[]) 
{
   ...
   int Msize,Nsize,Ksize,runserial;
   printf("nargc=%d\n",nargc);

   // set the platform and device to use
   // initially set to amd gpu on my macbook pro
   int iplat = 0;
   int idev  = 2;
   if (nargc>2)
   {
      sscanf(argc[1],"%d",&iplat);
      sscanf(argc[2],"%d",&idev);
   }
   ...
   if (nargc<6) {
      ...
      runserial = 1;
       ...
       sscanf(argc[3],"%d",&Msize);
       sscanf(argc[4],"%d",&Nsize);
       sscanf(argc[5],"%d",&Ksize);
       runserial = 1;
    } else  {
       sscanf(argc[3],"%d",&Msize);
       sscanf(argc[4],"%d",&Nsize);
       sscanf(argc[5],"%d",&Ksize);
       sscanf(argc[6],"%d",&runserial);
    }
   ...
   printf("+--------------------------------+\n");
   printf("| OpenCL MatMul-like GPU timings |\n");
   printf("+--------------------------------+\n\n");
   ...
   for (int i=0; i<Ksize*Nsize; ++i) B[i] = sin(((double) i))+4.0;
...
*/
   ...
   printf("Gpu total time                       = time6-time2 = %lf ms\n",1000.0*(double)(time6-time2)/CLOCKS_PER_SEC);

   double dt = (double)(time6-time2)/CLOCKS_PER_SEC;
   printf("\nGpu GFlops = %lf\n", (1.0*Msize*Nsize*Ksize/1.9e9)/dt);
   ...
   if (runserial)
   {
      ...
      {
         acc = 0.0;
         for(int k=0; k<Ksize; ++k) acc += A[i+k*Msize]*B[k+j*Nsize];
         CC[i+j*Msize] = acc;
      }
      time8 = clock();
      printf("\nSerial time = %lf ms\n",1000.0*(double)(time8-time7)/CLOCKS_PER_SEC);
      dt = (double)(time8-time7)/CLOCKS_PER_SEC;
      printf("\nSerial GFlops = %lf\n", (Msize*Nsize*Ksize/1.9e9)/dt);
      ...
      for(int i=0; i<(Msize*Nsize); ++i)
         err += (C[i]-CC[i])*(C[i]-CC[i]);
      printf("Error = %20.15le\n",sqrt(err));
   
      for(int i=0; i<8; ++i)
         printf("i=%d C = %20.15le %20.15le diff = %20.15le\n",i,C[i],CC[i],C[i]-CC[i]);
      printf("\n");
   }
