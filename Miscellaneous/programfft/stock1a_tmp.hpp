...
static void fft0_fac2(int isgn, int n, int s, bool eo, complex_t* x, complex_t* y)
...
static void fft0_fac3(int isgn, int n, int s, bool eo, complex_t* x, complex_t* y)
// isgn: -1 forward fft, 1 inverse fft
// n  : sequence length
// s  : stride
// eo : x is output if eo == 0, y is output if eo == 1
...
static void fft0_fac4(int isgn, int n, int s, bool eo, complex_t* x, complex_t* y)
// isgn: -1 forward fft, 1 inverse fft
// n  : sequence length
// s  : stride
// eo : x is output if eo == 0, y is output if eo == 1
...
static void fft0_fac5(int isgn, int n, int s, bool eo, complex_t* x, complex_t* y)
// isgn: -1 forward fft, 1 inverse fft
// n  : sequence length
// s  : stride
// eo : x is output if eo == 0, y is output if eo == 1
...
static void fft0_fac6(int isgn, int n, int s, bool eo, complex_t* x, complex_t* y)
// isgn: -1 forward fft, 1 inverse fft
// n  : sequence length
// s  : stride
// eo : x is output if eo == 0, y is output if eo == 1
...
static void fft0_fac7(int isgn, int n, int s, bool eo, complex_t* x, complex_t* y)
// isgn: -1 forward fft, 1 inverse fft
// n  : sequence length
// s  : stride
// eo : x is output if eo == 0, y is output if eo == 1
...
static void fft0_fac8(int isgn, int n, int s, bool eo, complex_t* x, complex_t* y)
// isgn: -1 forward fft, 1 inverse fft
// n  : sequence length
// s  : stride
// eo : x is output if eo == 0, y is output if eo == 1
...
static void fft0_fac9(int isgn, int n, int s, bool eo, complex_t* x, complex_t* y)
// isgn: -1 forward fft, 1 inverse fft
// n  : sequence length
// s  : stride
// eo : x is output if eo == 0, y is output if eo == 1
...
static void fft0_fac11(int isgn, int n, int s, bool eo, complex_t* x, complex_t* y)
// isgn: -1 forward fft, 1 inverse fft 
// n  : sequence length
// s  : stride
// eo : x is output if eo == 0, y is output if eo == 1
   ...
   if (n == 1) 
       if (eo) 
          for (int b=0; b<nbatch; ++b)
             for (int q=0; q<(s); q++) y[b*nfft+q] = x[b*nfft+q];
   ...
   else 
   {
      for (int p = 0; p < m; p++) 
      {
         ...
         for (int b=0; b<nbatch; ++b)
            for (int q=0; q<s; ++q) 
            {
               const complex_t a00= x[b*nfft + q + s*(p +  0)];
               const complex_t a01= x[b*nfft + q + s*(p +  1*m)];
               const complex_t a02= x[b*nfft + q + s*(p +  2*m)];
               const complex_t a03= x[b*nfft + q + s*(p +  3*m)];
               const complex_t a04= x[b*nfft + q + s*(p +  4*m)];
               const complex_t a05= x[b*nfft + q + s*(p +  5*m)];
               const complex_t a06= x[b*nfft + q + s*(p +  6*m)];
               const complex_t a07= x[b*nfft + q + s*(p +  7*m)];
               const complex_t a08= x[b*nfft + q + s*(p +  8*m)];
               const complex_t a09= x[b*nfft + q + s*(p +  9*m)];
               const complex_t a10= x[b*nfft + q + s*(p + 10*m)];
               const complex_t a11= x[b*nfft + q + s*(p + 11*m)];
               const complex_t a12= x[b*nfft + q + s*(p + 12*m)];
               const complex_t a13= x[b*nfft + q + s*(p + 13*m)];
               const complex_t a14= x[b*nfft + q + s*(p + 14*m)];
               const complex_t a15= x[b*nfft + q + s*(p + 15*m)];
               const complex_t a16= x[b*nfft + q + s*(p + 16*m)];
            
               y[b*nfft + q + s*(17*p + 0)]  = a00 + a01 + a02 + a03 + a04 + a05 + a06 + a07 + a08 + a09 + a10 + a11 + a12 + a13 + a14 + a15 + a16;
            
               y[b*nfft + q + s*(17*p + 1)]  = (a00  + a01*u01_17 + a02*u02_17 + a03*u03_17 + a04*u04_17 + a05*u05_17 + a06*u06_17 
                                                     + a07*u07_17 + a08*u08_17 + a09*u09_17 + a10*u10_17 + a11*u11_17 + a12*u12_17 
                                                     + a13*u13_17 + a14*u14_17 + a15*u15_17 + a16*u16_17)*wp01;
            
               y[b*nfft + q + s*(17*p + 2)]  = (a00  + a01*u02_17 + a02*u04_17 + a03*u06_17 + a04*u08_17 + a05*u10_17 + a06*u12_17
                                                     + a07*u14_17 + a08*u16_17 + a09*u01_17 + a10*u03_17 + a11*u05_17 + a12*u07_17
                                                     + a13*u09_17 + a14*u11_17 + a15*u13_17 + a16*u15_17)*wp02;
            
               y[b*nfft + q + s*(17*p + 3)]  = (a00  + a01*u03_17 + a02*u06_17 + a03*u09_17 + a04*u12_17 + a05*u15_17 + a06*u01_17
                                                     + a07*u04_17 + a08*u07_17 + a09*u10_17 + a10*u13_17 + a11*u16_17 + a12*u02_17 
                                                     + a13*u05_17 + a14*u08_17 + a15*u11_17 + a16*u14_17)*wp03;
            
               y[b*nfft + q + s*(17*p + 4)]  = (a00  + a01*u04_17 + a02*u08_17 + a03*u12_17 + a04*u16_17 + a05*u03_17 + a06*u07_17 
                                                     + a07*u11_17 + a08*u15_17 + a09*u02_17 + a10*u06_17 + a11*u10_17 + a12*u14_17 
                                                     + a13*u01_17 + a14*u05_17 + a15*u09_17 + a16*u13_17)*wp04;
            
               y[b*nfft + q + s*(17*p + 5)]  = (a00  + a01*u05_17 + a02*u10_17 + a03*u15_17 + a04*u03_17 + a05*u08_17 + a06*u13_17 
                                                     + a07*u01_17 + a08*u06_17 + a09*u11_17 + a10*u16_17 + a11*u04_17 + a12*u09_17 
                                                     + a13*u14_17 + a14*u02_17 + a15*u07_17 + a16*u12_17)*wp05;
            
               y[b*nfft + q + s*(17*p + 6)]  = (a00  + a01*u06_17 + a02*u12_17 + a03*u01_17 + a04*u07_17 + a05*u13_17 + a06*u02_17 
                                                     + a07*u08_17 + a08*u14_17 + a09*u03_17 + a10*u09_17 + a11*u15_17 + a12*u04_17 
                                                     + a13*u10_17 + a14*u16_17 + a15*u05_17 + a16*u11_17)*wp06;
            
               y[b*nfft + q + s*(17*p + 7)]  = (a00  + a01*u07_17 + a02*u14_17 + a03*u04_17 + a04*u11_17 + a05*u01_17 + a06*u08_17 
                                                     + a07*u15_17 + a08*u05_17 + a09*u12_17 + a10*u02_17 + a11*u09_17 + a12*u16_17 
                                                     + a13*u06_17 + a14*u13_17 + a15*u03_17 + a16*u10_17)*wp07;
            
               y[b*nfft + q + s*(17*p + 8)]  = (a00  + a01*u08_17 + a02*u16_17 + a03*u07_17 + a04*u15_17 + a05*u06_17 + a06*u14_17 
                                                     + a07*u05_17 + a08*u13_17 + a09*u04_17 + a10*u12_17 + a11*u03_17 + a12*u11_17 
                                                     + a13*u02_17 + a14*u10_17 + a15*u01_17 + a16*u09_17)*wp08;
            
               y[b*nfft + q + s*(17*p + 9)]  = (a00  + a01*u09_17 + a02*u01_17 + a03*u10_17 + a04*u02_17 + a05*u11_17 + a06*u03_17 
                                                     + a07*u12_17 + a08*u04_17 + a09*u13_17 + a10*u05_17 + a11*u14_17 + a12*u06_17 
                                                     + a13*u15_17 + a14*u07_17 + a15*u16_17 + a16*u08_17)*wp09;
            
               y[b*nfft + q + s*(17*p + 10)] = (a00  + a01*u10_17 + a02*u03_17 + a03*u13_17 + a04*u06_17 + a05*u16_17 + a06*u09_17 
                                                     + a07*u02_17 + a08*u12_17 + a09*u05_17 + a10*u15_17 + a11*u08_17 + a12*u01_17 
                                                     + a13*u11_17 + a14*u04_17 + a15*u14_17 + a16*u07_17)*wp10;
            
               y[b*nfft + q + s*(17*p + 11)] = (a00  + a01*u11_17 + a02*u05_17 + a03*u16_17 + a04*u10_17 + a05*u04_17 + a06*u15_17 
                                                     + a07*u09_17 + a08*u03_17 + a09*u14_17 + a10*u08_17 + a11*u02_17 + a12*u13_17 
                                                     + a13*u07_17 + a14*u01_17 + a15*u12_17 + a16*u06_17)*wp11;
            
               y[b*nfft + q + s*(17*p + 12)] = (a00  + a01*u12_17 + a02*u07_17 + a03*u02_17 + a04*u14_17 + a05*u09_17 + a06*u04_17 
                                                     + a07*u16_17 + a08*u11_17 + a09*u06_17 + a10*u01_17 + a11*u13_17 + a12*u08_17 
                                                     + a13*u03_17 + a14*u15_17 + a15*u10_17 + a16*u05_17)*wp12;
            
               y[b*nfft + q + s*(17*p + 13)] = (a00  + a01*u13_17 + a02*u09_17 + a03*u05_17 + a04*u01_17 + a05*u14_17 + a06*u10_17 
                                                     + a07*u06_17 + a08*u02_17 + a09*u15_17 + a10*u11_17 + a11*u07_17 + a12*u03_17 
                                                     + a13*u16_17 + a14*u12_17 + a15*u08_17 + a16*u04_17)*wp13;
            
               y[b*nfft + q + s*(17*p + 14)] = (a00  + a01*u14_17 + a02*u11_17 + a03*u08_17 + a04*u05_17 + a05*u02_17 + a06*u16_17 
                                                     + a07*u13_17 + a08*u10_17 + a09*u07_17 + a10*u04_17 + a11*u01_17 + a12*u15_17  
                                                     + a13*u12_17 + a14*u09_17 + a15*u06_17 + a16*u03_17)*wp14;
            
               y[b*nfft + q + s*(17*p + 15)] = (a00  + a01*u15_17 + a02*u13_17 + a03*u11_17 + a04*u09_17 + a05*u07_17 + a06*u05_17 
                                                     + a07*u03_17 + a08*u01_17 + a09*u16_17 + a10*u14_17 + a11*u12_17 + a12*u10_17 
                                                     + a13*u08_17 + a14*u06_17 + a15*u04_17 + a16*u02_17)*wp15;
            
               y[b*nfft + q + s*(17*p + 16)] = (a00  + a01*u16_17 + a02*u15_17 + a03*u14_17 + a04*u13_17 + a05*u12_17 + a06*u11_17 
...
void fft(int isgn, int n, complex_t* x, complex_t* y) // Fourier transform
// isgn: -1 forward fft, 1 inverse fft
// n : sequence length
