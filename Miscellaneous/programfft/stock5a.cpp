...
// y  : work area(or output sequence if eo == 1)
{
   ...
   std::cout << "FAC5 m=" << m << " theta0=" << theta0 << std::endl;
   ...
   std::cout << "FAC5 Up=" << p << " " << wp << " " << wp2 << " " << wp3 << " " << wp4 <<  std::endl;
          for (int q = 0; q < s; q++) {
              const complex_t a = x[q + s*(p + 0)];
              const complex_t b = x[q + s*(p + 1*m)];
              const complex_t c = x[q + s*(p + 2*m)];
      ...
      for (int p=0; p<m; ++p) 
         for (int q=0; q<s; ++q) 
            for (int r1=0; r1<radix; ++r1)
            {
               ...
               y[q + s*(radix*p+r1)] *= twiddle[radix*(p+1)+r1];
            }
   }
}
...
void set_sub_fft_twiddle(const int isgn, const int n, const int radix, complex_t *twiddle)
{
   ...
   for (int p=0; p<m; ++p)
      for (int r=0; r<radix; ++r)
         twiddle[(p+1)*radix+r] = complex_t(cos(r*p*theta0), isgn*sin(r*p*theta0));
...
void set_fft_twiddle(const int isgn, const int n, complex_t *twiddle)
{
   ...
   int nn = n;
   while (s<=n) {
      if ((nn%17)==0)      { set_sub_fft_twiddle(isgn,nn,17,twiddle+nsize); nsize += (nn+17); nn /= 17; s *= 17; }
