...
void fft_radix(const int isgn, const int n, const int s, bool eo, const int radix, complex_t* twiddle, complex_t* x, complex_t *y)
{
   const int m = n/radix;
   const double theta0 = 2*M_PI/n;
   if (n == 1) { if (eo) for (int q = 0; q < s; q++) y[q] = x[q]; }
         ...
         for (int q = 0; q < s; ++q) 
            for (int r1=0; r1<radix; ++r1)
            {
               ...
               for (int r2=0; r2<radix; ++r2)
                  y[q + s*(radix*p+r1)] += x[q + s*(p+r2*m)] * twiddle[radix*p + (r1*r2)%radix];
               y[q + s*(radix*p+r1)] *= twiddle[n+r1];
            }
   }
}
 ...
 */
void set_sub_fft_twiddle(const int n, const int radix, complex_t *twiddle)
{
   const int m = n/radix;
   const int isgn = 1;
...
int size_fft_twiddle(const int n)
{
   ...
   int nn = n;
   while (s<=n) {
      if ((nn%17)==0)      { nsize += (n+17); nn /= 17; s *= 17; }
      else if ((nn%11)==0) { nsize += (n+11); nn /= 11; s *= 11; }
      else if ((nn%9)==0)  { nsize += (n+9);  nn /= 9;  s *= 9; }
      else if ((nn%8)==0)  { nsize += (n+8);  nn /= 8;  s *= 8; }
      else if ((nn%7)==0)  { nsize += (n+7);  nn /= 7;  s *= 7; }
      else if ((nn%6)==0)  { nsize += (n+6);  nn /= 6;  s *= 6; }
      else if ((nn%5)==0)  { nsize += (n+5);  nn /= 5;  s *= 5; }
      else if ((nn%4)==0)  { nsize += (n+4);  nn /= 4;  s *= 4; }
      else if ((nn%3)==0)  { nsize += (n+3);  nn /= 3;  s *= 3; }
      else if ((nn%2)==0)  { nsize += (n+2);  nn /= 2;  s *= 2; }
      else
         throw std::runtime_error("Unsupported FFT size. No valid factorization is possible.");
   }
   return nsize;
}
 ...
 *                                       *
 *****************************************/
void set_fft_twiddle(const int n, complex_t *twiddle)
{
   int nsize = 0;
   int s  = 1;
   int nn = n;
   while (s<=n) {
      if ((nn%17)==0) {
         //const int m = n/17;
         set_sub_fft_twiddle(n,17,twiddle+nsize);
         //nsize += 17*m;
         nsize += n+17;
         nn /= 17; s *= 17;
      }
      else if ((nn%11)==0) { set_sub_fft_twiddle(n,11,twiddle+nsize); nsize += (n+11); nn /= 11; s *= 11; }
      else if ((nn%9)==0)  { set_sub_fft_twiddle(n,9, twiddle+nsize); nsize += (n+9);  nn /= 9;  s *= 9; }
      else if ((nn%8)==0)  { set_sub_fft_twiddle(n,8, twiddle+nsize); nsize += (n+8);  nn /= 8;  s *= 8; }
      else if ((nn%7)==0)  { set_sub_fft_twiddle(n,7, twiddle+nsize); nsize += (n+7);  nn /= 7;  s *= 7; }
      else if ((nn%6)==0)  { set_sub_fft_twiddle(n,6, twiddle+nsize); nsize += (n+6);  nn /= 6;  s *= 6; }
      else if ((nn%5)==0)  { set_sub_fft_twiddle(n,5, twiddle+nsize); nsize += (n+5);  nn /= 5;  s *= 5; }
      else if ((nn%4)==0)  { set_sub_fft_twiddle(n,4, twiddle+nsize); nsize += (n+4);  nn /= 4;  s *= 4; }
      else if ((nn%3)==0)  { set_sub_fft_twiddle(n,3, twiddle+nsize); nsize += (n+3);  nn /= 3;  s *= 3; }
      else if ((nn%2)==0)  { set_sub_fft_twiddle(n,2, twiddle+nsize); nsize += (n+2);  nn /= 2;  s *= 2; }
      else
         throw std::runtime_error("Unsupported FFT size. No valid factorization is possible.");
   }
}
