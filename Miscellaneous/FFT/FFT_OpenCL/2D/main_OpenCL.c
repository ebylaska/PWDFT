...
int FFT_2D_OpenCL(float *tab[], const char* direction, int sizex, int sizey) {
...
int main(void) {
 ...
 FFT_2D = fopen("FFT_2D_OpenCL_Input.dat","w");
 for(j=0; j<sizey; j++)
 {
  for(i=0; i<sizex; i++)
  {
   Array[0][j*sizex+i] = cos(2*M_PI*frequency_signalx*hx)*
    cos(2*M_PI*frequency_signaly*hy);
   Array[1][j*sizex+i] = 0.0f;
 ...
 // Perform Forward FFT
 if (FFT_2D_OpenCL(Array,"forward", sizex, sizey) == 0)
  printf("FFT passed !\n");

 // Save Output Array
 FFT_2D = fopen("FFT_2D_OpenCL_Forward.dat","w");
 for(j=0; j<sizey; j++)
  for(i=0; i<sizex; i++)
 ...
 if (FFT_2D_OpenCL(Array,"backward", sizex, sizey) == 0)
  printf("IFFT passed !\n");

 // Save Output Array
 FFT_2D = fopen("FFT_2D_OpenCL_Backward.dat","w");
 for(j=0; j<sizey; j++)
  for(i=0; i<sizex; i++)
   fprintf(FFT_2D,"%f %f %e\n", i/(frequency_signalx*sizex),
     j/(frequency_signaly*sizey),
